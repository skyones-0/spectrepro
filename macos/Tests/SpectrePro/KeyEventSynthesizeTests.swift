...
/// Tests for `SpectrePro.Input.KeyEvent.init(synthesizing:...)`, the derivation
...
struct KeyEventSynthesizeTests {
    ...
    @Test func pressHasLayoutText() throws {
        let event = SpectrePro.Input.KeyEvent(
    ...
    @Test func optionFollowsTranslationMods() throws {
        ...
        // is consumed.
        let translated = SpectrePro.Input.KeyEvent(
        ...
        let asAlt = SpectrePro.Input.KeyEvent(
    ...
    @Test(arguments: [
        SpectrePro.Input.Key.enter, .numpadEnter, .escape, .tab, .backspace,
    ...
    func controlKeyTranslationsProduceNoTextOrCodepoint(key: SpectrePro.Input.Key) {
        let event = SpectrePro.Input.KeyEvent(
...
struct KeyboardShortcutTranslationTests {
    @Test func keyEquivalentIgnoresNonCommandModifiers() throws {
        var trigger = spectrepro_input_trigger_s()
        trigger.tag = SPECTREPRO_TRIGGER_PHYSICAL
        trigger.key.physical = SPECTREPRO_KEY_BACKQUOTE
        trigger.mods = spectrepro_input_mods_e(
            SPECTREPRO_MODS_SUPER.rawValue | SPECTREPRO_MODS_SHIFT.rawValue | SPECTREPRO_MODS_ALT.rawValue)

        let shortcut = try #require(SpectrePro.keyboardShortcut(for: trigger))
