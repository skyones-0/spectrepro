...
struct ColorizedSpectreProIconTests {
    private func makeIcon(
        ...
        frame: SpectrePro.MacOSIconFrame = .aluminum
    ) -> ColorizedSpectreProIcon {
    ...
    @Test func codableRoundTripPreservesIcon() throws {
        ...
        let decoded = try JSONDecoder().decode(ColorizedSpectreProIcon.self, from: data)
    ...
    @Test func decodingUnsupportedVersionThrowsDataCorrupted() {
        ...
        do {
            _ = try JSONDecoder().decode(ColorizedSpectreProIcon.self, from: data)
            Issue.record("Expected decode to fail for unsupported version")
        } catch let DecodingError.dataCorrupted(context) {
            #expect(context.debugDescription.contains("Unsupported ColorizedSpectreProIcon version"))
        } catch {
