...
/// `SpectrePro.Input.KeyEvent` delivered to the surface.
///
...
struct ScriptKeyEventCommandTests {
    ...
    @Test(
        arguments: [
            ("shift", SpectrePro.Input.Mods.shift),
            ("control", .ctrl),
            ("option", .alt),
            ("command", .super),
            ...
            ("", []),
        ] as [(String, SpectrePro.Input.Mods)]
    )
    func parsesModifiers(string: String, expected: SpectrePro.Input.Mods) throws {
    ...
    @Test(arguments: [
        SpectrePro.Input.Key.enter, .escape, .tab, .backspace, .arrowUp, .f1,
        .home, .delete,
    ])
    func functionalKeysCarryNoTextOrCodepoint(key: SpectrePro.Input.Key) throws {
