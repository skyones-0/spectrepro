...
extension SpectrePro {
    ...
    enum ClipboardRequest {
        ...
        static func from(request: spectrepro_clipboard_request_e) -> ClipboardRequest? {
            switch request {
            case SPECTREPRO_CLIPBOARD_REQUEST_PASTE:
                return .paste
            case SPECTREPRO_CLIPBOARD_REQUEST_OSC_52_READ:
                return .osc_52_read
            case SPECTREPRO_CLIPBOARD_REQUEST_OSC_52_WRITE:
                return .osc_52_write
            case SPECTREPRO_CLIPBOARD_REQUEST_KITTY_READ:
                return .kitty_read
            case SPECTREPRO_CLIPBOARD_REQUEST_KITTY_WRITE:
                return .kitty_write
            default:
                return nil
            }
    ...
    /// A one-shot clipboard confirmation originating from libspectrepro.
    ///
    ...
    /// libspectrepro state cannot leak when no UI is available to handle the
    /// notification. Cancellation is deferred because notification delivery
    /// occurs from inside the libspectrepro callback that created the request.
