...
extension SpectrePro {
    ...
    struct InspectableSurface: View {
        @EnvironmentObject var spectrepro: SpectrePro.App
        ...
        var body: some View {
            ...
            ZStack {
                if !surfaceView.inspectorVisible {
                    ...
                    SplitView(.vertical, $split, dividerColor: spectrepro.config.splitDividerColor, left: {
                        SurfaceWrapper(surfaceView: surfaceView, isSplit: isSplit)
                    }, right: {
        ...
        private func onControlInspector(_ notification: SwiftUI.Notification) {
            ...
            guard let mode = modeAny as? spectrepro_action_inspector_e else { return }

            switch mode {
            case SPECTREPRO_INSPECTOR_TOGGLE:
                surfaceView.inspectorVisible = !surfaceView.inspectorVisible

            case SPECTREPRO_INSPECTOR_SHOW:
                surfaceView.inspectorVisible = true

            case SPECTREPRO_INSPECTOR_HIDE:
    ...
    class InspectorView: MTKView, NSTextInputClient {
        ...
        private var inspector: SpectrePro.Inspector? {
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            inspector.mouseButton(SPECTREPRO_MOUSE_PRESS, button: SPECTREPRO_MOUSE_LEFT, mods: mods)
        }

        override func mouseUp(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = SpectrePro.spectreproMods(event.modifierFlags)
            inspector.mouseButton(SPECTREPRO_MOUSE_RELEASE, button: SPECTREPRO_MOUSE_LEFT, mods: mods)
        }

        override func rightMouseDown(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = SpectrePro.spectreproMods(event.modifierFlags)
            inspector.mouseButton(SPECTREPRO_MOUSE_PRESS, button: SPECTREPRO_MOUSE_RIGHT, mods: mods)
        }

        override func rightMouseUp(with event: NSEvent) {
            guard let inspector = self.inspector else { return }
            let mods = SpectrePro.spectreproMods(event.modifierFlags)
            inspector.mouseButton(SPECTREPRO_MOUSE_RELEASE, button: SPECTREPRO_MOUSE_RIGHT, mods: mods)
        }
        ...
        override func scrollWheel(with event: NSEvent) {
            ...
            var momentum: spectrepro_input_mouse_momentum_e = SPECTREPRO_MOUSE_MOMENTUM_NONE
            switch event.momentumPhase {
            case .began:
                momentum = SPECTREPRO_MOUSE_MOMENTUM_BEGAN
            case .stationary:
                momentum = SPECTREPRO_MOUSE_MOMENTUM_STATIONARY
            case .changed:
                momentum = SPECTREPRO_MOUSE_MOMENTUM_CHANGED
            case .ended:
                momentum = SPECTREPRO_MOUSE_MOMENTUM_ENDED
            case .cancelled:
                momentum = SPECTREPRO_MOUSE_MOMENTUM_CANCELLED
            case .mayBegin:
                momentum = SPECTREPRO_MOUSE_MOMENTUM_MAY_BEGIN
            default:
                break
            }
        ...
        override func keyUp(with event: NSEvent) {
            keyAction(SPECTREPRO_ACTION_RELEASE, event: event)
        }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            if mods.rawValue & mod != 0 { action = SPECTREPRO_ACTION_PRESS }

            keyAction(action, event: event)
        }

        private func keyAction(_ action: spectrepro_input_action_e, event: NSEvent) {
            guard let inspector = self.inspector else { return }
            guard let key = SpectrePro.Input.Key(keyCode: event.keyCode) else { return }
            let mods = SpectrePro.spectreproMods(event.modifierFlags)
