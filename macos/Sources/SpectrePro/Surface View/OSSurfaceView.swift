...
extension SpectrePro {
    class OSSurfaceView: NSView, ObservableObject {
        ...
        @Published var surfaceSize: spectrepro_surface_size_s?
        ...
        /// A message sent from `spectrepro_surface_t` when a child process exited
        @Published private(set) var childExitedMessage: ChildExitedMessage?

        var surface: spectrepro_surface_t? {
            nil
        }
        ...
        init(id: UUID?, frame: CGRect) {
            ...
            center.addObserver(
                self,
                selector: #selector(spectreproDidChangeReadonly(_:)),
        ...
        @objc private func spectreproDidChangeReadonly(_ notification: Foundation.Notification) {
...
extension SpectrePro.OSSurfaceView {
    @MainActor class SearchState: ObservableObject {
        ...
        init(
            from startSearch: SpectrePro.Action.StartSearch,
