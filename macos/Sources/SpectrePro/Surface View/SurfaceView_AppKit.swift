...
extension SpectrePro {
    ...
    class SurfaceView: OSSurfaceView, Codable, Identifiable {
        ...
        // detected with the set_password_input_cb on the SpectrePro state.
        ...
        var needsConfirmQuit: Bool {
            ...
            return spectrepro_surface_needs_confirm_quit(surface)
        }

        // Returns true if the process in this surface has exited.
        var processExited: Bool {
            guard let surface = self.surface else { return true }
            return spectrepro_surface_process_exited(surface)
        }

        // Returns the inspector instance for this surface, or nil if the
        // surface has been closed or no inspector is active.
        var inspector: SpectrePro.Inspector? {
            guard let surface = self.surface else { return nil }
            guard let cInspector = spectrepro_surface_inspector(surface) else { return nil }
            return SpectrePro.Inspector(cInspector: cInspector)
        }
        ...
        @Published var inspectorVisible: Bool = false {
            didSet {
                if oldValue && !inspectorVisible {
                    guard let surface = self.surface else { return }
                    spectrepro_inspector_free(surface)
                }
            }
        }
        ...
        var scrollbar: SpectrePro.Action.Scrollbar?
        ...
        init(_ app: spectrepro_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            self.markedText = NSMutableAttributedString()

            // Our initial config always is our application wide config.
            if let appDelegate = NSApplication.shared.delegate as? AppDelegate {
                self.derivedConfig = DerivedConfig(appDelegate.spectrepro.config)
            } else {
            ...
            cachedScreenContents = .init(duration: .milliseconds(500)) { [weak self] in
                ...
                let sel = spectrepro_selection_s(
                    top_left: spectrepro_point_s(
                        tag: SPECTREPRO_POINT_SCREEN,
                        coord: SPECTREPRO_POINT_COORD_TOP_LEFT,
                        x: 0,
                        y: 0),
                    bottom_right: spectrepro_point_s(
                        tag: SPECTREPRO_POINT_SCREEN,
                        coord: SPECTREPRO_POINT_COORD_BOTTOM_RIGHT,
                        x: 0,
                        y: 0),
                    rectangle: false)
                guard spectrepro_surface_read_text(surface, sel, &text) else { return "" }
            ...
            accessibilitySelectionCancellable = NotificationCenter.default
                ...
                // comment below)
                .publisher(for: .spectreproSelectionDidChange)
            ...
            center.addObserver(
                ...
                name: SpectrePro.Notification.didUpdateRendererHealth,
                object: self)
            center.addObserver(
                self,
                selector: #selector(spectreproDidContinueKeySequence),
                name: SpectrePro.Notification.didContinueKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(spectreproDidEndKeySequence),
                name: SpectrePro.Notification.didEndKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(spectreproDidChangeKeyTable),
                name: SpectrePro.Notification.didChangeKeyTable,
                object: self)
            center.addObserver(
                self,
                selector: #selector(spectreproConfigDidChange(_:)),
                name: .spectreproConfigDidChange,
                object: self)
            center.addObserver(
                self,
                selector: #selector(spectreproColorDidChange(_:)),
                name: .spectreproColorDidChange,
                object: self)
            center.addObserver(
                self,
                selector: #selector(spectreproBellDidRing(_:)),
            ...
            guard let surface = surface else {
                self.error = SpectrePro.Error.apiFailed
                return
            }
            self.surfaceModel = SpectrePro.Surface(cSurface: surface)
        ...
        override func focusDidChange(_ focused: Bool) {
            ...
            // Notify libspectrepro
            spectrepro_surface_set_focus(surface, focused)
        ...
        override func sizeDidChange(_ size: CGSize) {
            // SpectrePro wants to know the actual framebuffer size... It is very important
        ...
        private func setSurfaceSize(width: UInt32, height: UInt32) {
            ...
            spectrepro_surface_set_size(surface, width, height)

            // Update our cached size metrics
            let size = spectrepro_surface_size(surface)
        ...
        func setCursorShape(_ shape: spectrepro_action_mouse_shape_e) {
            switch shape {
            case SPECTREPRO_MOUSE_SHAPE_DEFAULT:
                pointerStyle = .default

            case SPECTREPRO_MOUSE_SHAPE_TEXT:
                pointerStyle = .horizontalText

            case SPECTREPRO_MOUSE_SHAPE_GRAB:
                pointerStyle = .grabIdle

            case SPECTREPRO_MOUSE_SHAPE_GRABBING:
                pointerStyle = .grabActive

            case SPECTREPRO_MOUSE_SHAPE_POINTER:
                pointerStyle = .link

            case SPECTREPRO_MOUSE_SHAPE_W_RESIZE:
                pointerStyle = .resizeLeft

            case SPECTREPRO_MOUSE_SHAPE_E_RESIZE:
                pointerStyle = .resizeRight

            case SPECTREPRO_MOUSE_SHAPE_N_RESIZE:
                pointerStyle = .resizeUp

            case SPECTREPRO_MOUSE_SHAPE_S_RESIZE:
                pointerStyle = .resizeDown

            case SPECTREPRO_MOUSE_SHAPE_NS_RESIZE:
                pointerStyle = .resizeUpDown

            case SPECTREPRO_MOUSE_SHAPE_EW_RESIZE:
                pointerStyle = .resizeLeftRight

            case SPECTREPRO_MOUSE_SHAPE_VERTICAL_TEXT:
                pointerStyle = .verticalText

            case SPECTREPRO_MOUSE_SHAPE_CONTEXT_MENU:
                pointerStyle = .contextMenu

            case SPECTREPRO_MOUSE_SHAPE_CROSSHAIR:
                pointerStyle = .crosshair

            case SPECTREPRO_MOUSE_SHAPE_NOT_ALLOWED:
        ...
        @objc private func onUpdateRendererHealth(notification: SwiftUI.Notification) {
            ...
            guard let health = healthAny as? spectrepro_action_renderer_health_e else { return }
            DispatchQueue.main.async { [weak self] in
                self?.healthy = health == SPECTREPRO_RENDERER_HEALTH_HEALTHY
            }
        }

        @objc private func spectreproDidContinueKeySequence(notification: SwiftUI.Notification) {
            guard let keyAny = notification.userInfo?[SpectrePro.Notification.KeySequenceKey] else { return }
        ...
        @objc private func spectreproDidEndKeySequence(notification: SwiftUI.Notification) {
            DispatchQueue.main.async { [weak self] in
                self?.keySequence = []
            }
        }

        @objc private func spectreproDidChangeKeyTable(notification: SwiftUI.Notification) {
            guard let action = notification.userInfo?[SpectrePro.Notification.KeyTableKey] as? SpectrePro.Action.KeyTable else { return }
        ...
        @objc private func spectreproConfigDidChange(_ notification: SwiftUI.Notification) {
            // Get our managed configuration object out
            guard let config = notification.userInfo?[
                SwiftUI.Notification.Name.SpectreProConfigChangeKey
        ...
        @objc private func spectreproColorDidChange(_ notification: SwiftUI.Notification) {
            guard let change = notification.userInfo?[
                SwiftUI.Notification.Name.SpectreProColorChangeKey
            ] as? SpectrePro.Action.ColorChange else { return }
        ...
        @objc private func spectreproBellDidRing(_ notification: SwiftUI.Notification) {
        ...
        @objc private func windowDidChangeScreen(notification: SwiftUI.Notification) {
            ...
            // When the window changes screens, we need to update libspectrepro with the screen
            // ID. If vsync is enabled, this will be used with the CVDisplayLink to ensure
            // the proper refresh rate is going.
            spectrepro_surface_set_display_id(surface, screen.displayID ?? 0)
            ...
            // Issue: https://github.com/spectrepro-org/spectrepro/issues/2731
            DispatchQueue.main.async { [weak self] in
        ...
        override func viewDidChangeBackingProperties() {
            ...
            spectrepro_surface_set_content_scale(surface, xScale, yScale)
        ...
        override func mouseDown(with event: NSEvent) {
            ...
            guard let surface = self.surface else { return }
            let mods = SpectrePro.spectreproMods(event.modifierFlags)
            spectrepro_surface_mouse_button(surface, SPECTREPRO_MOUSE_PRESS, SPECTREPRO_MOUSE_LEFT, mods)
        }
        ...
        override func mouseUp(with event: NSEvent) {
            ...
            guard let surface = self.surface else { return }
            let mods = SpectrePro.spectreproMods(event.modifierFlags)
            spectrepro_surface_mouse_button(surface, SPECTREPRO_MOUSE_RELEASE, SPECTREPRO_MOUSE_LEFT, mods)

            // Release pressure
            spectrepro_surface_mouse_pressure(surface, 0, 0)
        }
        ...
        override func otherMouseDown(with event: NSEvent) {
            ...
            guard let surface = self.surface else { return }
            let mods = SpectrePro.spectreproMods(event.modifierFlags)
            let button = SpectrePro.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
            spectrepro_surface_mouse_button(surface, SPECTREPRO_MOUSE_PRESS, button.cMouseButton, mods)
        }
        ...
        override func otherMouseUp(with event: NSEvent) {
            ...
            let mods = SpectrePro.spectreproMods(event.modifierFlags)
            let button = SpectrePro.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
            spectrepro_surface_mouse_button(surface, SPECTREPRO_MOUSE_RELEASE, button.cMouseButton, mods)
        }
        ...
        override func rightMouseDown(with event: NSEvent) {
            ...
            let mods = SpectrePro.spectreproMods(event.modifierFlags)
            if spectrepro_surface_mouse_button(
                surface,
        ...
        override func mouseEntered(with event: NSEvent) {
            ...
            let mouseEvent = SpectrePro.Input.MousePosEvent(
                x: pos.x,
        ...
        override func scrollWheel(with event: NSEvent) {
            ...
            let scrollEvent = SpectrePro.Input.MouseScrollEvent(
                x: x,
                y: y,
        ...
        override func pressureChange(with event: NSEvent) {
            ...
            // Notify SpectrePro first. We do this because this will let SpectrePro handle
            // state setup that we'll need for later pressure handling (such as
            // QuickLook)
            spectrepro_surface_mouse_pressure(surface, UInt32(event.stage), Double(event.pressure))
            ...
            guard UserDefaults.spectrepro.bool(forKey: "com.apple.trackpad.forceClick") else { return }
        ...
        override func keyDown(with event: NSEvent) {
            ...
            let translationModsSpectrePro = SpectrePro.eventModifierFlags(
                mods: spectrepro_surface_key_translation_mods(
                    surface,
                    SpectrePro.spectreproMods(event.modifierFlags)
                )
            )

            // There are hidden bits set in our event that matter for certain dead keys
            // so we can't use translationModsSpectrePro directly. Instead, we just check
            // for exact states and set them.
            var translationMods = event.modifierFlags
            for flag in [NSEvent.ModifierFlags.shift, .control, .option, .command] {
                if translationModsSpectrePro.contains(flag) {
            ...
            let action = event.isARepeat ? SPECTREPRO_ACTION_REPEAT : SPECTREPRO_ACTION_PRESS
            ...
            if markedTextBefore,
               ...
               list.count > 0 {
                for text in list {
                    if SpectrePro.SurfaceView.shouldSuppressComposingControlInput(
                        text,
                        composing: composing
                    ) {
                        continue
            ...
            if let list = keyTextAccumulator, list.count > 0 {
                ...
                _ = keyAction(
                    ...
                    event: event,
                    translationEvent: translationEvent,
                    text: translationEvent.spectreproCharacters,
                    composing: composing
                )
            }
        }
        ...
        override func keyUp(with event: NSEvent) {
            _ = keyAction(SPECTREPRO_ACTION_RELEASE, event: event)
        }
        ...
        override func performKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            let bindingFlags = surfaceModel.flatMap { surface in
                var spectreproEvent = event.spectreproKeyEvent(SPECTREPRO_ACTION_PRESS)
                return (event.characters ?? "").withCString { ptr in
                    spectreproEvent.text = ptr
                    return surface.keyIsBinding(spectreproEvent)
                }
            }
                ...
                if keySequence.isEmpty,
                   ...
                   bindingFlags.contains(.consumed) {
                    if let appDelegate = NSApp.delegate as? AppDelegate,
                       appDelegate.performSpectreProBindingMenuKeyEquivalent(with: event) {
                        return true
                    }
                }
        ...
        override func flagsChanged(with event: NSEvent) {
            ...
            case 0x39: mod = SPECTREPRO_MODS_CAPS.rawValue
            case 0x38, 0x3C: mod = SPECTREPRO_MODS_SHIFT.rawValue
            case 0x3B, 0x3E: mod = SPECTREPRO_MODS_CTRL.rawValue
            case 0x3A, 0x3D: mod = SPECTREPRO_MODS_ALT.rawValue
            case 0x37, 0x36: mod = SPECTREPRO_MODS_SUPER.rawValue
            default: return
            }
            ...
            var action = SPECTREPRO_ACTION_RELEASE
            if mods.rawValue & mod != 0 {
                ...
                if sidePressed {
                    action = SPECTREPRO_ACTION_PRESS
                }
            }
        ...
        private func keyAction(
            ...
            var key_ev = event.spectreproKeyEvent(action, translationMods: translationEvent?.modifierFlags)
            key_ev.composing = composing

            if let text = text?.keyEventText {
                return text.withCString { ptr in
                    key_ev.text = ptr
                    return spectrepro_surface_key(surface, key_ev)
                }
            } else {
                return spectrepro_surface_key(surface, key_ev)
            }
        ...
        private func shouldReplayCommittedPreeditKey(_ event: NSEvent) -> Bool {
            guard let key = SpectrePro.Input.Key(keyCode: event.keyCode) else { return false }
            switch key {
        ...
        private func committedTextAction(
            ...
            var key_ev = spectrepro_input_key_s()
            key_ev.action = action
            key_ev.keycode = 0
            key_ev.text = nil
            key_ev.composing = false
            key_ev.mods = SPECTREPRO_MODS_NONE
            key_ev.consumed_mods = SPECTREPRO_MODS_NONE
        ...
        override func quickLook(with event: NSEvent) {
            ...
            var text = spectrepro_text_s()
            guard spectrepro_surface_quicklook_word(surface, &text) else { return super.quickLook(with: event) }
            defer { spectrepro_surface_free_text(surface, &text) }
            ...
            if let fontRaw = spectrepro_surface_quicklook_font(surface) {
                // Memory management here is wonky: spectrepro_surface_quicklook_font
            ...
            // SpectrePro coordinate system is top-left, convert to bottom-left for AppKit
        ...
        @IBAction func splitRight(_ sender: Any) {
            guard let surface = self.surface else { return }
            spectrepro_surface_split(surface, SPECTREPRO_SPLIT_DIRECTION_RIGHT)
        }

        @IBAction func splitLeft(_ sender: Any) {
            guard let surface = self.surface else { return }
            spectrepro_surface_split(surface, SPECTREPRO_SPLIT_DIRECTION_LEFT)
        }

        @IBAction func splitDown(_ sender: Any) {
            guard let surface = self.surface else { return }
            spectrepro_surface_split(surface, SPECTREPRO_SPLIT_DIRECTION_DOWN)
        }

        @IBAction func splitUp(_ sender: Any) {
            guard let surface = self.surface else { return }
            spectrepro_surface_split(surface, SPECTREPRO_SPLIT_DIRECTION_UP)
        }
        ...
        func showUserNotification(title: String, body: String, requireFocus: Bool = true) {
            ...
            content.categoryIdentifier = SpectrePro.userNotificationCategory
            content.userInfo = [
        ...
        struct DerivedConfig {
            ...
            let backgroundBlur: SpectrePro.Config.BackgroundBlur
            let macosWindowShadow: Bool
            let windowTitleFontFamily: String?
            let windowAppearance: NSAppearance?
            let scrollbar: SpectrePro.Config.Scrollbar
            ...
            init(_ config: SpectrePro.Config) {
                ...
                self.windowAppearance = .init(spectreproConfig: config)
        ...
        required convenience init(from decoder: Decoder) throws {
            // Decoding uses the global SpectrePro app
            guard let del = NSApplication.shared.delegate,
                  let appDel = del as? AppDelegate,
                  let app = appDel.spectrepro.app else {
...
extension SpectrePro.SurfaceView {
    ...
    private func pendingClipboardConfirmationDidChange(
        from previous: SpectrePro.ClipboardConfirmationRequest?
    ) {
...
extension SpectrePro.SurfaceView: NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        // Get our range from the SpectrePro API. There is a race condition between getting the
        ...
        var text = spectrepro_text_s()
        guard spectrepro_surface_read_selection(surface, &text) else { return NSRange() }
        defer { spectrepro_surface_free_text(surface, &text) }
    ...
    func attributedSubstring(forProposedRange range: NSRange, actualRange: NSRangePointer?) -> NSAttributedString? {
        // SpectrePro.logger.warning("pressure substring range=\(range) selectedRange=\(self.selectedRange())")
        guard let surface = self.surface else { return nil }
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        // SpectrePro will tell us where it thinks an IME keyboard should render.
        var x: Double = 0
        var y: Double = 0
        ...
        if range.length > 0 && range != self.selectedRange() {
            // QuickLook
            var text = spectrepro_text_s()
            if spectrepro_surface_read_selection(surface, &text) {
                ...
                // Free our text
                spectrepro_surface_free_text(surface, &text)
            } else {
                spectrepro_surface_ime_point(surface, &x, &y, &width, &height)
            }
        } else {
            spectrepro_surface_ime_point(surface, &x, &y, &width, &height)
        }
        ...
        // SpectrePro coordinates are in top-left (0, 0) so we have to convert to
    ...
    func insertText(_ string: Any, replacementRange: NSRange) {
        ...
        if !chars.isEmpty {
            _ = committedTextAction(SPECTREPRO_ACTION_PRESS, text: chars)
        }
    }
    ...
    /// Sync the preedit state based on the markedText value to libspectrepro
...
extension SpectrePro.SurfaceView: NSServicesMenuRequestor {
    override func validRequestor(
        ...
        if (returnType == nil || receivable.contains(returnType!)) &&
            ...
            // performance hits because `spectrepro_surface_has_selection` isn't free.
            if let sendType, sendableRequiresSelection.contains(sendType) {
                if surface == nil || !spectrepro_surface_has_selection(surface) {
    ...
    func readSelection(from pboard: NSPasteboard) -> Bool {
        ...
        str.withCString { ptr in
            ...
            spectrepro_surface_text(surface, ptr, UInt(len - 1))
        }
...
extension SpectrePro.SurfaceView: NSMenuItemValidation {
    func validateMenuItem(_ item: NSMenuItem) -> Bool {
        ...
        case #selector(pasteSelection):
            let pb = NSPasteboard.spectreproSelection
...
extension SpectrePro.SurfaceView {
    ...
    /// this to copy styling information as well but we need to augment SpectrePro core to
    /// expose that.
    ///
