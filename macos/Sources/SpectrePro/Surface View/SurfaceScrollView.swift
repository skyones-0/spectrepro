...
/// Wraps a SpectrePro surface view in an NSScrollView to provide native macOS scrollbar support.
///
/// ## Coordinate System
...
/// - `surfaceView`: The actual SpectrePro renderer, positioned to fill the visible rect
class SurfaceScrollView: NSView {
    private let scrollView: NSScrollView
    private let documentView: NSView
    private let surfaceView: SpectrePro.SurfaceView
    ...
    init(contentSize: CGSize, surfaceView: SpectrePro.SurfaceView) {
        ...
        // so that our primary SpectrePro renderer only needs to render the viewport.
        ...
        // Listen for scrollbar updates from SpectrePro
        observers.append(NotificationCenter.default.addObserver(
            forName: .spectreproDidUpdateScrollbar,
            object: surfaceView,
            queue: .main
        ) { [weak self] notification in
    ...
    private func handleScrollbarUpdate(_ notification: Notification) {
        guard let scrollbar = notification.userInfo?[SwiftUI.Notification.Name.ScrollbarKey] as? SpectrePro.Action.Scrollbar else {
            return
        }
