...
extension NSEvent {
    /// Create a SpectrePro key event for a given keyboard action.
    ///
    ...
    func spectreproKeyEvent(
        ...
        key_ev.mods = SpectrePro.spectreproMods(modifierFlags)
        key_ev.consumed_mods = SpectrePro.spectreproMods(
    ...
    /// Returns the text to set for a key event for SpectrePro.
    ///
    /// This namely contains logic to avoid control characters, since we handle control character
    /// mapping manually within SpectrePro.
    var spectreproCharacters: String? {
        ...
        if characters.count == 1,
           let scalar = characters.unicodeScalars.first {
            ...
            // encoding directly within SpectrePro's KeyEncoder.
            if scalar.value < 0x20 {
                return self.characters(byApplyingModifiers: modifierFlags.subtracting(.control))
            }

            // If we have a single value in the PUA, then it's a function key and
            // we don't want to send PUA ranges down to SpectrePro.
