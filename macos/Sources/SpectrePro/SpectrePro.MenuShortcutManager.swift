...
extension SpectrePro {
    /// The manager that's responsible for updating shortcuts of SpectrePro's app menu
    @MainActor
    class MenuShortcutManager {

        /// SpectrePro menu items indexed by their normalized shortcut. This avoids traversing
        ...
        /// action string used for the SpectrePro configuration.
        func syncMenuShortcut(_ config: SpectrePro.Config, action: String?, menuItem: NSMenuItem?) {
        ...
        /// SpectrePro keybind actions. This is important because it lets our surface dispatch
        /// bindings through the menu so they flash but also lets our surface override macOS built-ins
        /// like Cmd+H.
        func performSpectreProBindingMenuKeyEquivalent(with event: NSEvent) -> Bool {
        ...
        private func performMenuItem(for key: MenuShortcutKey) -> Bool? {
            // If we don't have an entry for this key combo, no SpectrePro-owned
...
private extension SpectrePro.MenuShortcutManager {
    /// Syncs a single menu shortcut for the given action. The action string is the same
    /// action string used for the SpectrePro configuration.
    ///
    /// - Returns: Whether the menu item is updated and saved in ``menuItemsByShortcut``
    func updateMenuShortcut(_ config: SpectrePro.Config, action: String?, menuItem menu: NSMenuItem) -> Bool {
        guard
            let action,
            let trigger = config.keybindTrigger(for: action),
            let shortcut = SpectrePro.keyboardShortcut(for: trigger)
        else { return false }

        let isPhysical = trigger.tag == SPECTREPRO_TRIGGER_PHYSICAL
        let physicalKeyCode = isPhysical ? SpectrePro.Input.Key(cKey: trigger.key.physical)?.keyCode : nil
...
extension SpectrePro.MenuShortcutManager {
