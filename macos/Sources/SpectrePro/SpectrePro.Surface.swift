...
extension SpectrePro {
    ...
    final class Surface: Sendable {
        ...
        nonisolated(unsafe) private let surface: spectrepro_surface_t
        ...
        var unsafeCValue: spectrepro_surface_t {
            surface
        }

        /// Initialize from the C structure.
        init(cSurface: spectrepro_surface_t) {
        ...
        deinit {
            guard !Thread.isMainThread else {
                ...
                spectrepro_surface_free(surface)
                return
            }
            // deinit is not guaranteed to happen on the main actor and our API
            // calls into libspectrepro must happen there so we capture the surface
        ...
        func keyTranslationMods(_ mods: Input.Mods) -> Input.Mods {
            Input.Mods(cMods: spectrepro_surface_key_translation_mods(surface, mods.cMods))
        }
        ...
        func sendKeyEvent(_ event: Input.KeyEvent) {
            event.withCValue { cEvent in
                spectrepro_surface_key(surface, cEvent)
            }
        }
        ...
        @MainActor
        func keyIsBinding(_ event: spectrepro_input_key_s) -> Input.BindingFlags? {
            var flags = spectrepro_binding_flags_e(0)
            guard spectrepro_surface_key_is_binding(surface, event, &flags) else { return nil }
            return Input.BindingFlags(cFlags: flags)
        }

        /// See `keyIsBinding(_ event: spectrepro_input_key_s)`.
        @MainActor
        ...
        var mouseCaptured: Bool {
            spectrepro_surface_mouse_captured(surface)
        }

        /// The PID of the foreground process group attached to the PTY.
        @MainActor
        var foregroundPID: Int? {
            let pid = spectrepro_surface_foreground_pid(surface)
        ...
        @MainActor
        var ttyName: String? {
            let ttyName = AllocatedString(spectrepro_surface_tty_name(surface)).string
        ...
        func sendMouseButton(_ event: Input.MouseButtonEvent) {
            spectrepro_surface_mouse_button(
                surface,
        ...
        func sendMouseScroll(_ event: Input.MouseScrollEvent) {
            spectrepro_surface_mouse_scroll(
                surface,
                event.x,
                event.y,
