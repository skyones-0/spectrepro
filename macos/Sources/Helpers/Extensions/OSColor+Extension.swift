...
// MARK: SpectrePro Types
#if !DOCK_TILE_PLUGIN
extension NSColor {
    /// Create a color from a SpectrePro color.
    convenience init(spectrepro: spectrepro_config_color_s) {
        let red = Double(spectrepro.r) / 255
        let green = Double(spectrepro.g) / 255
        let blue = Double(spectrepro.b) / 255
