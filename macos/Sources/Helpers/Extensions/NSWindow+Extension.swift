...
extension NSWindow {
    ...
    func addTabbedWindowSafely(
        ...
        var error: NSError?
        let success = SpectreProAddTabbedWindowSafely(self, child, ordered.rawValue, &error)
        if let error {
            SpectrePro.logger.error("addTabbedWindow failed: \(error.localizedDescription, privacy: .public)")
        }
...
extension NSWindowController {
    ...
    func showWindowSafely(_ sender: Any?) -> Bool {
        var error: NSError?
        let success = SpectreProShowWindowSafely(self, sender, &error)
        if let error {
            SpectrePro.logger.error("showWindow failed: \(error.localizedDescription, privacy: .public)")
        }
