...
extension NSPasteboard {
    /// The pasteboard to used for SpectrePro selection.
    static var spectreproSelection: NSPasteboard = {
        NSPasteboard(name: .init("co.skyones.spectrepro.selection"))
    }()
    ...
    func getOpinionatedStringContents() -> String? {
        let strings = (pasteboardItems ?? []).compactMap { item in
            if let plist = item.propertyList(forType: .fileURL),
               ...
               fileURL.isFileURL {
                return SpectrePro.Shell.escape(fileURL.path)
            } else {
    ...
    private var spectreproFileURLs: [URL] {
    ...
    func spectreproData(forMime mime: String) -> Data? {
        switch mime {
        case "text/plain":
    ...
    func spectreproAvailableMimes() -> [String] {
        var result: [String] = []
        var seen = Set<String>()
    ...
    /// The pasteboard for the SpectrePro enum type. Returns nil for locations
    /// macOS can't serve; callers report those as unsupported.
    static func spectrepro(_ clipboard: spectrepro_clipboard_e) -> NSPasteboard? {
        switch clipboard {
        case SPECTREPRO_CLIPBOARD_STANDARD:
            return Self.general

        case SPECTREPRO_CLIPBOARD_SELECTION:
            return Self.spectreproSelection

        case SPECTREPRO_CLIPBOARD_PRIMARY:
