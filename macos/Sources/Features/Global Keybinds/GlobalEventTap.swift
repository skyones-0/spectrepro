...
private func cgEventFlagsChangedHandler(
    ...
    // We need an app delegate to get the SpectrePro app instance
    guard let appDelegate = NSApplication.shared.delegate as? AppDelegate else { return result }
    guard let spectrepro = appDelegate.spectrepro.app else { return result }
    ...
    let key_ev = event.spectreproKeyEvent(SPECTREPRO_ACTION_PRESS)
    if spectrepro_app_key(spectrepro, key_ev) {
