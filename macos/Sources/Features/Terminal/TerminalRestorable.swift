...
protocol TerminalRestorable: Codable {
    ...
    var baseConfig: SpectrePro.SurfaceConfiguration? { get }
}
...
extension TerminalRestorable {
    ...
    var baseConfig: SpectrePro.SurfaceConfiguration? { nil }
...
final class TerminalRestorableState: TerminalRestorable {
    ...
    var surfaceTree: SplitTree<SpectrePro.SurfaceView> {
    ...
    private let internalState: InternalState<SpectrePro.SurfaceView>
    ...
    init(from decoder: any Decoder) throws {
        self.internalState = try InternalState<SpectrePro.SurfaceView>(from: decoder)
    }
...
class TerminalWindowRestoration: NSObject, NSWindowRestoration {
    static func restoreWindow(
        ...
        // no matter what. Note its safe to use "spectrepro.config" directly here
        // because window restoration is only ever invoked on app start so we
        // don't have to deal with config reloads.
        if appDelegate.spectrepro.config.windowSaveState == "never" {
        ...
        if let focusedStr = state.focusedSurface {
            var foundView: SpectrePro.SurfaceView?
    ...
    private static func restoreFocus(to: SpectrePro.SurfaceView, inWindow: NSWindow, attempts: Int = 0) {
