...
class TerminalController: BaseTerminalController, TabGroupCloseCoordinator.Controller {
    override var windowNibName: NSNib.Name? {
        ...
        let config = appDelegate.spectrepro.config
    ...
    init(_ spectrepro: SpectrePro.App,
         withBaseConfig base: SpectrePro.SurfaceConfiguration? = nil,
         withSurfaceTree tree: SplitTree<SpectrePro.SurfaceView>? = nil,
         parent: NSWindow? = nil
    ) {
        ...
        super.init(spectrepro, baseConfig: base, surfaceTree: tree)
        ...
        center.addObserver(
            ...
            name: SpectrePro.Notification.spectreproToggleFullscreen,
            object: nil)
        center.addObserver(
            self,
        ...
        center.addObserver(
            ...
            name: SpectrePro.Notification.spectreproGotoTab,
            object: nil)
        center.addObserver(
            self,
    ...
    override func surfaceTreeDidChange(from: SplitTree<SpectrePro.SurfaceView>, to: SplitTree<SpectrePro.SurfaceView>) {
    ...
    static func newWindow(
        _ spectrepro: SpectrePro.App,
        withBaseConfig baseConfig: SpectrePro.SurfaceConfiguration? = nil,
        withParent explicitParent: NSWindow? = nil
    ) -> TerminalController {
        let c = TerminalController.init(spectrepro, withBaseConfig: baseConfig)
        ...
        } else if let fullscreenMode = spectrepro.config.windowFullscreen {
        ...
        c.scheduleInitialPresentation {
            ...
            // - Creating a window via `New SpectrePro Window Here` service.
            c.showWindowSafely(self)

            // Only cascade if we aren't fullscreen.
            if let window = c.window {
                if !window.styleMask.contains(.fullScreen) && spectrepro.config.windowFullscreen == nil {
    ...
    /// - Parameters:
    ///   - spectrepro: The SpectrePro app instance.
    ...
    static func newWindow(
        _ spectrepro: SpectrePro.App,
        tree: SplitTree<SpectrePro.SurfaceView>,
        ...
        let c = TerminalController.init(spectrepro, withSurfaceTree: tree)
    ...
    static func newTab(
        ...
        guard let parent,
            ...
            return newWindow(spectrepro, withBaseConfig: baseConfig, withParent: parent)
        }

        // If our parent is in non-native fullscreen, then new tabs do not work.
        // See: https://github.co/skyones/spectrepro/issues/392
        ...
        let controller = TerminalController.init(spectrepro, withBaseConfig: baseConfig)
        ...
        if window.tabbingMode != .disallowed {
            ...
            switch spectrepro.config.windowNewTabPosition {
            case "end":
    ...
    @objc private func spectreproConfigDidChange(_ notification: Notification) {
        ...
        /// ``SpectrePro/SpectrePro/SurfaceView/derivedConfig`` then
    ...
    func relabelTabs() {
        ...
        if let windows = window?.tabbedWindows as? [TerminalWindow] {
            for (tab, window) in zip(1..., windows) {
                ...
                if let equiv = spectrepro.config.keyboardShortcut(for: "goto_tab:\(tab)") {
    ...
    private func syncAppearance(_ surfaceConfig: SpectrePro.SurfaceView.DerivedConfig) {
        ...
        terminalViewContainer?.spectreproConfigDidChange(spectrepro.config, preferredBackgroundColor: window.preferredBackgroundColor)
    }
    ...
    func closeTabImmediately(registerRedo: Bool = true) {
        ...
        if let undoManager, let undoState {
            ...
            undoManager.registerUndo(
                ...
                let newController = TerminalController(spectrepro, with: undoState)
    ...
    private func registerUndoForCloseWindow() {
        ...
        guard let tabGroup = window.tabGroup,
            ...
            if let undoState {
                ...
                undoManager.registerUndo(
                    withTarget: spectrepro,
                    expiresAfter: undoExpiration) { spectrepro in
        ...
        undoManager.registerUndo(
            ...
            let controllers = undoStates.map { undoState in
                TerminalController(spectrepro, with: undoState)
            }
    ...
    struct UndoState {
        let frame: NSRect
        let surfaceTree: SplitTree<SpectrePro.SurfaceView>
    ...
    convenience init(_ spectrepro: SpectrePro.App, with undoState: UndoState) {
        self.init(spectrepro, withSurfaceTree: undoState.surfaceTree)
        ...
        if let window {
            ...
            if let focusedUUID = undoState.focusedSurface,
               let focusTarget = surfaceTree.first(where: { $0.id == focusedUUID }) {
                DispatchQueue.main.async {
                    SpectrePro.moveFocus(to: focusTarget, from: nil)
                }
                ...
                DispatchQueue.main.async {
                    SpectrePro.moveFocus(to: focusedSurface, from: nil)
                }
            }
        }
    }
    ...
    override func windowDidLoad() {
        ...
        let container = TerminalViewContainer {
            TerminalView(spectrepro: spectrepro, viewModel: self, delegate: self)
        }
        ...
        // In various situations, macOS automatically tabs new windows. SpectrePro handles
        ...
        if NSWindow.userTabbingPreference == .always,
           !window.styleMask.contains(.fullScreen) {
            ...
            // Since SpectrePro manages tabbing manually this will never be more than one
    ...
    override func newWindowForTab(_ sender: Any?) {
        // Trigger the spectrepro core event logic for a new tab.
        guard let surface = self.focusedSurface?.surface else { return }
        spectrepro.newTab(surface: surface)
    }
    ...
    override func windowWillClose(_ notification: Notification) {
        ...
        if let focusedWindow = NSApplication.shared.keyWindow {
            ...
            if focusedWindow != window {
                ...
                // https://github.com/spectrepro-org/spectrepro/issues/2565
    ...
    @IBAction func newWindow(_ sender: Any?) {
        ...
        spectrepro.newWindow(surface: surface)
    }
    ...
    @IBAction func toggleSpectreProFullScreen(_ sender: Any?) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.toggleFullscreen(surface: surface)
    }

    @IBAction func toggleTerminalInspector(_ sender: Any?) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.toggleTerminalInspector(surface: surface)
    }

    // MARK: - TerminalViewDelegate

    override func focusedSurfaceDidChange(to: SpectrePro.SurfaceView?) {
    ...
    private func syncAppearanceOnPropertyChange(_ surface: SpectrePro.SurfaceView?) {
    ...
    @objc private func onMoveTab(notification: SwiftUI.Notification) {
        ...
        // Get the move action
        guard let action = notification.userInfo?[Notification.Name.SpectreProMoveTabKey] as? SpectrePro.Action.MoveTab else { return }
    ...
    @objc private func onGotoTab(notification: SwiftUI.Notification) {
        ...
        guard let tabEnumAny = notification.userInfo?[SpectrePro.Notification.GotoTabKey] else { return }
        guard let tabEnum = tabEnumAny as? spectrepro_action_goto_tab_e else { return }
        ...
        if tabIndex <= 0 {
            ...
            if tabIndex == SPECTREPRO_GOTO_TAB_PREVIOUS.rawValue {
                if selectedIndex == 0 {
                    finalIndex = tabbedWindows.count - 1
                } else {
                    finalIndex = selectedIndex - 1
                }
            } else if tabIndex == SPECTREPRO_GOTO_TAB_NEXT.rawValue {
                if selectedIndex == tabbedWindows.count - 1 {
                    finalIndex = 0
                } else {
                    finalIndex = selectedIndex + 1
                }
            } else if tabIndex == SPECTREPRO_GOTO_TAB_LAST.rawValue {
                finalIndex = tabbedWindows.count - 1
            } else {
                return
            }
    ...
    @objc private func onToggleFullscreen(notification: SwiftUI.Notification) {
        ...
        if let any = notification.userInfo?[SpectrePro.Notification.FullscreenModeKey],
           let mode = any as? FullscreenMode {
            fullscreenMode = mode
        } else {
            SpectrePro.logger.warning("no fullscreen mode specified or invalid mode, doing nothing")
            return
        }
    ...
    struct DerivedConfig {
        ...
        let macosWindowButtons: SpectrePro.MacOSWindowButtons
        let macosTitlebarStyle: SpectrePro.Config.MacOSTitlebarStyle
        let maximize: Bool
