...
/// A base class for windows that can contain SpectrePro windows. This base class implements
/// the bare minimum functionality that every terminal window in SpectrePro should implement.
///
...
class BaseTerminalController: NSWindowController,
    ...
    private static let surfaceControllers =
        NSMapTable<SpectrePro.SurfaceView, BaseTerminalController>.weakToWeakObjects()

    /// The app instance that this terminal view will represent.
    let spectrepro: SpectrePro.App

    /// The currently focused surface.
    var focusedSurface: SpectrePro.SurfaceView? {
        didSet { syncFocusToSurfaceTree() }
    }

    /// The tree of splits within this terminal window.
    @Published var surfaceTree: SplitTree<SpectrePro.SurfaceView> = .init() {
        didSet {
    ...
    private var appliedColorScheme: spectrepro_color_scheme_e?

    /// The configuration derived from the SpectrePro config so we don't need to rely on references.
    ...
    init(_ spectrepro: SpectrePro.App,
         baseConfig base: SpectrePro.SurfaceConfiguration? = nil,
         surfaceTree tree: SplitTree<SpectrePro.SurfaceView>? = nil
    ) {
        self.spectrepro = spectrepro
        self.derivedConfig = DerivedConfig(spectrepro.config)

        super.init(window: nil)

        // Initialize our initial surface.
        guard let spectrepro_app = spectrepro.app else { preconditionFailure("app must be loaded") }
        self.surfaceTree = tree ?? .init(view: SpectrePro.SurfaceView(spectrepro_app, baseConfig: base))
        ...
        center.addObserver(
            self,
            selector: #selector(spectreproConfigDidChangeBase(_:)),
            name: .spectreproConfigDidChange,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(spectreproQuickCommandsDidToggle(_:)),
            name: .spectreproQuickCommandsDidToggle,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(spectreproCommandPaletteDidToggle(_:)),
            name: .spectreproCommandPaletteDidToggle,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(spectreproMaximizeDidToggle(_:)),
        ...
        center.addObserver(
            self,
            selector: #selector(spectreproDidCloseSurface(_:)),
            name: SpectrePro.Notification.spectreproCloseSurface,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(spectreproDidNewSplit(_:)),
        ...
        center.addObserver(
            self,
            selector: #selector(spectreproDidFocusSplit(_:)),
            name: SpectrePro.Notification.spectreproFocusSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(spectreproDidToggleSplitZoom(_:)),
            name: SpectrePro.Notification.didToggleSplitZoom,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(spectreproDidResizeSplit(_:)),
            name: SpectrePro.Notification.didResizeSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(spectreproDidPresentTerminal(_:)),
            name: SpectrePro.Notification.spectreproPresentTerminal,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(spectreproSurfaceDragEndedNoTarget(_:)),
            name: .spectreproSurfaceDragEndedNoTarget,
            object: nil)
    ...
    /// view relationship.
    static func controller(owning surface: SpectrePro.SurfaceView) -> BaseTerminalController? {
    ...
    private static func updateSurfaceControllers(
        ...
        from oldTree: SplitTree<SpectrePro.SurfaceView>,
        to newTree: SplitTree<SpectrePro.SurfaceView>
    ) {
    ...
    @discardableResult
    func newSplit(
        at oldView: SpectrePro.SurfaceView,
        direction: SplitTree<SpectrePro.SurfaceView>.NewDirection,
        baseConfig config: SpectrePro.SurfaceConfiguration? = nil
    ) -> SpectrePro.SurfaceView? {
        // We can only create new splits for surfaces in our tree.
        guard surfaceTree.root?.node(view: oldView) != nil else { return nil }

        // Create a new surface view
        guard let spectrepro_app = spectrepro.app else { return nil }
        let newView = SpectrePro.SurfaceView(spectrepro_app, baseConfig: config)

        // Do the split
        let newTree: SplitTree<SpectrePro.SurfaceView>
        do {
            ...
            // no big deal.
            SpectrePro.logger.warning("failed to insert split: \(error, privacy: .public)")
            return nil
        }
    ...
    func surfaceTreeDidChange(from: SplitTree<SpectrePro.SurfaceView>, to: SplitTree<SpectrePro.SurfaceView>) {
    ...
    /// Update all surfaces with the focus state. This ensures that libspectrepro has an accurate view about
    /// what surface is focused. This must be called whenever a surface OR window changes focus.
    func syncFocusToSurfaceTree() {
        var newlyFocused: SpectrePro.SurfaceView?
    ...
    func closeSurface(
        _ node: SplitTree<SpectrePro.SurfaceView>.Node,
        withConfirmation: Bool = true
    ) {
        ...
        // due to SwiftUI bugs (see SpectrePro #560). To repeat from #560, the bug is that
    ...
    private func findNextFocusTargetAfterClosing(node: SplitTree<SpectrePro.SurfaceView>.Node) -> SpectrePro.SurfaceView? {
    ...
    private func removeSurfaceNode(_ node: SplitTree<SpectrePro.SurfaceView>.Node) {
        // Move focus if the closed surface was focused and we have a next target
        let nextFocus: SpectrePro.SurfaceView? = if node.contains(
    ...
    func replaceSurfaceTree(
        _ newTree: SplitTree<SpectrePro.SurfaceView>,
        moveFocusTo newView: SpectrePro.SurfaceView? = nil,
        moveFocusFrom oldView: SpectrePro.SurfaceView? = nil,
        ...
        if let newView {
            DispatchQueue.main.async {
                SpectrePro.moveFocus(to: newView, from: oldView)
            }
        }
        ...
        undoManager.registerUndo(
            ...
            if let oldView {
                DispatchQueue.main.async {
                    SpectrePro.moveFocus(to: oldView, from: target.focusedSurface)
                }
            }
    ...
    @objc private func spectreproConfigDidChangeBase(_ notification: Notification) {
        ...
        guard let config = notification.userInfo?[
            Notification.Name.SpectreProConfigChangeKey
        ] as? SpectrePro.Config else { return }

        // Update our derived config
        self.derivedConfig = DerivedConfig(config)
    }

    @objc private func spectreproQuickCommandsDidToggle(_ notification: Notification) {
        guard let surfaceView = notification.object as? SpectrePro.SurfaceView,
    ...
    @IBAction func toggleQuickCommands(_ sender: Any?) {
        ...
        if !QuickCommandsState.shared.isShowing, let focusedSurface, surfaceTree.contains(focusedSurface) {
            SpectrePro.moveFocus(to: focusedSurface)
        }
    }
    ...
    func sendQuickCommand(_ command: QuickCommand, customText: String? = nil, execute: Bool, broadcast: Bool = false) {
        ...
        if broadcast {
            ...
            performAction(action, on: target)
            SpectrePro.moveFocus(to: target)
        }
    }

    @objc private func spectreproCommandPaletteDidToggle(_ notification: Notification) {
        guard let surfaceView = notification.object as? SpectrePro.SurfaceView else { return }
        guard surfaceTree.contains(surfaceView) else { return }
        toggleCommandPalette(nil)
    }

    @objc private func spectreproMaximizeDidToggle(_ notification: Notification) {
        guard let window else { return }
        guard let surfaceView = notification.object as? SpectrePro.SurfaceView else { return }
        guard surfaceTree.contains(surfaceView) else { return }
        window.zoom(nil)
    ...
    @objc private func spectreproDidCloseSurface(_ notification: Notification) {
        guard let target = notification.object as? SpectrePro.SurfaceView else { return }
    ...
    @objc private func spectreproDidNewSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let oldView = notification.object as? SpectrePro.SurfaceView else { return }
        guard surfaceTree.root?.node(view: oldView) != nil else { return }

        // Notification must contain our base config
        let configAny = notification.userInfo?[SpectrePro.Notification.NewSurfaceConfigKey]
        let config = configAny as? SpectrePro.SurfaceConfiguration

        // Determine our desired direction
        guard let directionAny = notification.userInfo?["direction"] else { return }
        guard let direction = directionAny as? spectrepro_action_split_direction_e else { return }
        let splitDirection: SplitTree<SpectrePro.SurfaceView>.NewDirection
        switch direction {
        case SPECTREPRO_SPLIT_DIRECTION_RIGHT: splitDirection = .right
        case SPECTREPRO_SPLIT_DIRECTION_LEFT: splitDirection = .left
        case SPECTREPRO_SPLIT_DIRECTION_DOWN: splitDirection = .down
        case SPECTREPRO_SPLIT_DIRECTION_UP: splitDirection = .up
        default: return
        }
    ...
    @objc private func spectreproDidFocusSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? SpectrePro.SurfaceView else { return }
        guard surfaceTree.root?.node(view: target) != nil else { return }

        // Get the direction from the notification
        guard let directionAny = notification.userInfo?[SpectrePro.Notification.SplitDirectionKey] else { return }
        guard let direction = directionAny as? SpectrePro.SplitFocusDirection else { return }
        ...
        DispatchQueue.main.async {
            SpectrePro.moveFocus(to: nextSurface, from: target)
        }
    }

    @objc private func spectreproDidToggleSplitZoom(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? SpectrePro.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }
    ...
    @objc private func spectreproDidResizeSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? SpectrePro.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }

        // Extract direction and amount from notification
        guard let directionAny = notification.userInfo?[SpectrePro.Notification.ResizeSplitDirectionKey] else { return }
        guard let direction = directionAny as? SpectrePro.SplitResizeDirection else { return }

        guard let amountAny = notification.userInfo?[SpectrePro.Notification.ResizeSplitAmountKey] else { return }
        guard let amount = amountAny as? UInt16 else { return }

        // Convert SpectrePro.SplitResizeDirection to SplitTree.Spatial.Direction
        let spatialDirection: SplitTree<SpectrePro.SurfaceView>.Spatial.Direction
        switch direction {
        ...
        do {
            ...
            SpectrePro.logger.warning("failed to resize split: \(error, privacy: .public)")
        }
    }

    @objc private func spectreproDidPresentTerminal(_ notification: Notification) {
        guard let target = notification.object as? SpectrePro.SurfaceView else { return }
        guard surfaceTree.contains(target) else { return }
        ...
        SpectrePro.moveFocus(to: target)
        SpectrePro.moveFocus(to: target, delay: 0.1)

        // Show a brief highlight to help the user locate the presented terminal.
        target.highlight()
    }

    @objc private func spectreproSurfaceDragEndedNoTarget(_ notification: Notification) {
        guard let target = notification.object as? SpectrePro.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }
        ...
        let newTree = SplitTree<SpectrePro.SurfaceView>(view: target)
        ...
        _ = TerminalController.newWindow(
            spectrepro,
            tree: newTree,
            position: notification.userInfo?[Notification.Name.spectreproSurfaceDragEndedNoTargetPointKey] as? NSPoint,
            confirmUndo: false,
    ...
    private func localEventFlagsChanged(_ event: NSEvent) -> NSEvent? {
        var surfaces: [SpectrePro.SurfaceView] = surfaceTree.map { $0 }
    ...
    private func computeTitle(title: String, bell: Bool) -> String {
        var result = title
        if bell && spectrepro.config.bellFeatures.contains(.title) {
            result = "🔔 \(result)"
        }
    ...
    private func splitDidResize(node: SplitTree<SpectrePro.SurfaceView>.Node, to newRatio: Double) {
        let resizedNode = node.resizing(to: newRatio)
        do {
            surfaceTree = try surfaceTree.replacing(node: node, with: resizedNode)
        } catch {
            SpectrePro.logger.warning("failed to replace node during split resize: \(error, privacy: .public)")
        }
    }

    private func splitDidDrop(
        source: SpectrePro.SurfaceView,
        destination: SpectrePro.SurfaceView,
        zone: TerminalSplitDropZone
    ) {
        // Map drop zone to split direction
        let direction: SplitTree<SpectrePro.SurfaceView>.NewDirection = switch zone {
        case .top: .up
        case .bottom: .down
        case .left: .left
        case .right: .right
        ...
        if let sourceNode = surfaceTree.root?.node(view: source) {
            ...
            do {
                ...
                SpectrePro.logger.warning("failed to insert surface during drop: \(error, privacy: .public)")
                return
            }
        ...
        var sourceNode: SplitTree<SpectrePro.SurfaceView>.Node?
        ...
        guard let sourceController, let sourceNode else {
            SpectrePro.logger.warning("source surface not found in any window during drop")
            return
        }
        ...
        do {
            ...
            SpectrePro.logger.warning("failed to insert surface during cross-window drop: \(error, privacy: .public)")
            return
        }
    ...
    func performAction(_ action: String, on surfaceView: SpectrePro.SurfaceView) {
        guard let surface = surfaceView.surface else { return }
        let len = action.utf8CString.count
        if len == 0 { return }
        _ = action.withCString { cString in
            spectrepro_surface_binding_action(surface, cString, UInt(len - 1))
        }
    }
    ...
    func toggleBackgroundOpacity() {
        ...
        guard spectrepro.config.backgroundOpacity < 1 else { return }
    ...
    private func syncSurfaceTreeOcclusionState() {
        ...
        for view in surfaceTree {
            if let surface = view.surface, view.isWindowVisible != visible {
                spectrepro_surface_set_occlusion(surface, visible)
    ...
    @IBAction func close(_ sender: Any) {
        ...
        spectrepro.requestClose(surface: surface)
    }
    ...
    @IBAction func splitRight(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.split(surface: surface, direction: SPECTREPRO_SPLIT_DIRECTION_RIGHT)
    }

    @IBAction func splitLeft(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.split(surface: surface, direction: SPECTREPRO_SPLIT_DIRECTION_LEFT)
    }

    @IBAction func splitDown(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.split(surface: surface, direction: SPECTREPRO_SPLIT_DIRECTION_DOWN)
    }

    @IBAction func splitUp(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.split(surface: surface, direction: SPECTREPRO_SPLIT_DIRECTION_UP)
    }

    @IBAction func splitZoom(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.splitToggleZoom(surface: surface)
    }
    ...
    @IBAction func equalizeSplits(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.splitEqualize(surface: surface)
    }

    @IBAction func moveSplitDividerUp(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.splitResize(surface: surface, direction: .up, amount: 10)
    }

    @IBAction func moveSplitDividerDown(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.splitResize(surface: surface, direction: .down, amount: 10)
    }

    @IBAction func moveSplitDividerLeft(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.splitResize(surface: surface, direction: .left, amount: 10)
    }

    @IBAction func moveSplitDividerRight(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.splitResize(surface: surface, direction: .right, amount: 10)
    }

    private func splitMoveFocus(direction: SpectrePro.SplitFocusDirection) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.splitMoveFocus(surface: surface, direction: direction)
    }

    @IBAction func increaseFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.changeFontSize(surface: surface, .increase(1))
    }

    @IBAction func decreaseFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.changeFontSize(surface: surface, .decrease(1))
    }

    @IBAction func resetFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.changeFontSize(surface: surface, .reset)
    }
    ...
    @objc func resetTerminal(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        spectrepro.resetTerminal(surface: surface)
    }

    private struct DerivedConfig {
        let macosTitlebarProxyIcon: SpectrePro.MacOSTitlebarProxyIcon
        let windowStepResize: Bool
        let focusFollowsMouse: Bool
        let splitPreserveZoom: SpectrePro.Config.SplitPreserveZoom
        ...
        init(_ config: SpectrePro.Config) {
...
extension BaseTerminalController: NSMenuItemValidation {
    ...
    ///
    /// Calling ``spectrepro_surface_set_color_scheme`` triggers
    ...
    func updateColorSchemeForSurfaceTree() {
        ...
        let scheme: spectrepro_color_scheme_e
        if themeAppearance.isDark {
            scheme = SPECTREPRO_COLOR_SCHEME_DARK
        } else {
            scheme = SPECTREPRO_COLOR_SCHEME_LIGHT
        }
        ...
        for surfaceView in surfaceTree {
            if let surface = surfaceView.surface {
                spectrepro_surface_set_color_scheme(surface, scheme)
            }
        }
...
extension BaseTerminalController {
    ...
    private func setupClipboardConfirmationPublisher() {
        clipboardConfirmationCancellable = $surfaceTree
            ...
            .switchToLatest()
            // Published emits synchronously from the libspectrepro callback. Hop
    ...
    private func onConfirmClipboardRequest(
        _ request: SpectrePro.ClipboardConfirmationRequest?,
        for target: SpectrePro.SurfaceView
    ) {
    ...
    private func presentPendingClipboardConfirmation(for target: SpectrePro.SurfaceView) {
    ...
    private func cancelPendingClipboardConfirmation(for target: SpectrePro.SurfaceView) {
        if let confirmation = clipboardConfirmation,
...
extension BaseTerminalController {
    ...
    func surfaceValuesPublisher<Value>(
        valueKeyPath: KeyPath<SpectrePro.SurfaceView, Value>,
        publisherKeyPath: KeyPath<SpectrePro.SurfaceView, Published<Value>.Publisher>
    ) -> AnyPublisher<[SpectrePro.SurfaceView.ID: Value], Never> {
...
extension Notification.Name {
    ...
    static let terminalWindowBellDidChangeNotification = Notification.Name("co.skyones.spectrepro.terminalWindowBellDidChange")
