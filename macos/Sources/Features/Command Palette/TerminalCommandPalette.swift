...
struct TerminalCommandPaletteView: View {
    ...
    @ObservedObject var spectreproConfig: SpectrePro.Config
                ...
                GeometryReader { geometry in
                    VStack {
                        ...
                        CommandPaletteView(
                            ...
                            backgroundColor: spectreproConfig.backgroundColor,
    ...
    private var updateOptions: [CommandOption] {
        ...
        if updateViewModel.state.isInstallable {
            ...
            if case .updateAvailable = updateViewModel.state {
                title = "Update SpectrePro and Restart"
            } else {
    ...
    private var terminalOptions: [CommandOption] {
        ...
        return appDelegate.spectrepro.config.commandPaletteEntries
            .filter(\.isSupported)
            .map { c in
                let symbols = appDelegate.spectrepro.config.keyboardShortcut(for: c.action)?.keyList
