...
enum TerminalSplitOperation {
    ...
    struct Resize {
        let node: SplitTree<SpectrePro.SurfaceView>.Node
        let ratio: Double
    }

    struct Drop {
        /// The surface being dragged.
        let payload: SpectrePro.SurfaceView

        /// The surface it was dragged onto
        let destination: SpectrePro.SurfaceView
...
struct TerminalSplitTreeView: View {
    let tree: SplitTree<SpectrePro.SurfaceView>
    ...
    var body: some View {
        if let node = tree.zoomed ?? tree.root {
            ...
            // See: https://github.com/spectrepro-org/spectrepro/issues/7546
    ...
    var body: some View {
        ...
        case .split(let split):
            ...
            SplitView(
                ...
                dividerColor: spectrepro.config.splitDividerColor,
...
private struct TerminalSplitLeaf: View {
    let surfaceView: SpectrePro.SurfaceView
    let isSplit: Bool
    let action: (TerminalSplitOperation) -> Void
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .background {
                ...
                if !isSelfDragging {
                    Color.clear
                        .onDrop(of: [.spectreproSurfaceId], delegate: SplitDropDelegate(
                            dropState: $dropState,
            ...
            .onPreferenceChange(SpectrePro.DraggingSurfaceKey.self) { value in
    ...
    private struct SplitDropDelegate: DropDelegate {
        ...
        let viewSize: CGSize
        let destinationSurface: SpectrePro.SurfaceView
        let action: (TerminalSplitOperation) -> Void

        func validateDrop(info: DropInfo) -> Bool {
            info.hasItemsConforming(to: [.spectreproSurfaceId])
        }
        ...
        func performDrop(info: DropInfo) -> Bool {
            ...
            let providers = info.itemProviders(for: [.spectreproSurfaceId])
            guard let provider = providers.first else { return false }

            // Capture action before the async closure
            _ = provider.loadTransferable(type: SpectrePro.SurfaceView.self) { [weak destinationSurface] result in
                switch result {
