...
class QuickTerminalController: BaseTerminalController {
    ...
    /// The previously running application when the terminal is shown. This is NEVER SpectrePro.
    ...
    init(_ spectrepro: SpectrePro.App,
        ...
        super.init(spectrepro, baseConfig: base, surfaceTree: .init())
    ...
    override func focusSurface(_ view: SpectrePro.SurfaceView) {
        if visible {
    ...
    override func newSplit(
        ...
        var config = config ?? SpectrePro.SurfaceConfiguration()
        config.environmentVariables["SPECTREPRO_QUICK_TERMINAL"] = "1"
    ...
    func animateIn() {
        ...
        if surfaceTree.isEmpty,
           let spectrepro_app = spectrepro.app {
            if let tree = restorationState?.surfaceTree, !tree.isEmpty {
                ...
                let view = SpectrePro.SurfaceView(spectrepro_app, baseConfig: config)
        ...
        NSAnimationContext.runAnimationGroup({ context in
            ...
            DispatchQueue.main.async {
                ...
                if !NSApp.isActive {
                    ...
                    // shown on a screen that has no other SpectrePro windows, it takes
                    // a few (variable) event loop ticks until we can actually focus it.
                    // https://github.com/spectrepro-org/spectrepro/issues/2409
                    //
    ...
    override func syncAppearance() {
        ...
        if !isBackgroundOpaque && (self.derivedConfig.backgroundOpacity < 1 || derivedConfig.backgroundBlur.isGlassStyle) {
            ...
            if !derivedConfig.backgroundBlur.isGlassStyle {
                spectrepro_set_window_background_blur(spectrepro.app, Unmanaged.passUnretained(window).toOpaque())
            }
        } else {
        ...
        terminalViewContainer?.spectreproConfigDidChange(spectrepro.config, preferredBackgroundColor: nil)
    }
    ...
    @objc private func spectreproConfigDidChange(_ notification: Notification) {
        ...
        terminalViewContainer?.spectreproConfigDidChange(config, preferredBackgroundColor: nil)
    }
