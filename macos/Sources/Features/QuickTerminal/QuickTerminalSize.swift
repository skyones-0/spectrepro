...
/// Represents the SpectrePro `quick-terminal-size` configuration. See the documentation for
...
struct QuickTerminalSize {
    ...
    init(from cStruct: spectrepro_config_quick_terminal_size_s) {
    ...
    enum Size {
        ...
        init?(from cStruct: spectrepro_quick_terminal_size_s) {
            switch cStruct.tag {
            case SPECTREPRO_QUICK_TERMINAL_SIZE_NONE:
                return nil
            case SPECTREPRO_QUICK_TERMINAL_SIZE_PERCENTAGE:
                self = .percentage(cStruct.value.percentage)
            case SPECTREPRO_QUICK_TERMINAL_SIZE_PIXELS:
