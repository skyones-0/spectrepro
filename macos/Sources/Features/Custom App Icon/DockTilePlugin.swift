...
class DockTilePlugin: NSObject, NSDockTilePlugIn {
    // WARNING: An instance of this class is alive as long as SpectrePro's icon is
    ...
    #if DEBUG
    private let spectreproUserDefaults = UserDefaults(suiteName: "co.skyones.spectrepro.debug")
    #else
    private let spectreproUserDefaults = UserDefaults(suiteName: "co.skyones.spectrepro")
    #endif
    ...
    func setDockTile(_ dockTile: NSDockTile?) {
        // If no dock tile or no access to SpectrePro defaults, we can't do anything.
        guard let dockTile, let spectreproUserDefaults else {
            iconChangeObserver = nil
            return
        }

        // Try to restore the previous icon on launch.
        iconDidChange(spectreproUserDefaults.appIcon, dockTile: dockTile)

        // Setup a new observer for when the icon changes so we can update. This message
        // is sent by the primary SpectrePro app.
        iconChangeObserver = DistributedNotificationCenter
            .default()
            .publisher(for: .spectreproIconDidChange)
            .map { [weak self] _ in self?.spectreproUserDefaults?.appIcon }
