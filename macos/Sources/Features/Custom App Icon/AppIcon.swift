...
/// The icon style for the SpectrePro App.
enum AppIcon: Equatable, Codable, Sendable {
    ...
    case customStyle(_ icon: ColorizedSpectreProIcon)

#if !DOCK_TILE_PLUGIN
    init?(config: SpectrePro.Config) {
        ...
        case .customStyle:
            ...
            self = .customStyle(ColorizedSpectreProIcon(screenColors: screenColors, ghostColor: ghostColor, frame: config.macosIconFrame))
        }
    }
#endif
...
actor AppIconUpdater {
    func update(icon: AppIcon?) {
        UserDefaults.spectrepro.appIcon = icon
        // Notify DockTilePlugin to update dock icon
        DistributedNotificationCenter.default()
            .postNotificationName(
                .spectreproIconDidChange,
                object: nil,
                userInfo: nil,
