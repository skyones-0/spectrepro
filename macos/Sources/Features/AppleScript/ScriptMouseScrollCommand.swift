...
/// Handler for the `send mouse scroll` AppleScript command defined in `SpectrePro.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="SpectreProScriptMouseScrollCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(SpectreProScriptMouseScrollCommand)
final class ScriptMouseScrollCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let momentum: SpectrePro.Input.Momentum
