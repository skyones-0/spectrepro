...
/// AppleScript-facing wrapper around a live SpectrePro terminal surface.
///
...
///
/// Mapping from `SpectrePro.sdef`:
/// - `class terminal` -> this class (`@objc(SpectreProAppleScriptTerminal)`).
...
final class ScriptTerminal: NSObject {
    ...
    weak var surfaceView: SpectrePro.SurfaceView?

    init(surfaceView: SpectrePro.SurfaceView) {
...
/// Converts four-character codes from the `split direction` enumeration in `SpectrePro.sdef`
...
enum ScriptSplitDirection {
    ...
    var splitDirection: SplitTree<SpectrePro.SurfaceView>.NewDirection {
        switch self {
        case .right: .right
        case .left: .left
        case .down: .down
