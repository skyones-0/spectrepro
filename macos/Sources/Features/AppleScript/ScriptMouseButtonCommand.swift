...
/// Handler for the `send mouse button` AppleScript command defined in `SpectrePro.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="SpectreProScriptMouseButtonCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(SpectreProScriptMouseButtonCommand)
final class ScriptMouseButtonCommand: NSScriptCommand {
    override func performDefaultImplementation() -> Any? {
        ...
        let action: SpectrePro.Input.MouseState
        ...
        let mods: SpectrePro.Input.Mods
        if let modsString = evaluatedArguments?["modifiers"] as? String {
            guard let parsed = SpectrePro.Input.Mods(scriptModifiers: modsString) else {
        ...
        let mouseEvent = SpectrePro.Input.MouseButtonEvent(
            action: action,
            button: button.spectreproButton,
            mods: mods
        )
...
/// Four-character codes matching the `mouse button` enumeration in `SpectrePro.sdef`.
private enum ScriptMouseButtonValue {
    ...
    var spectreproButton: SpectrePro.Input.MouseButton {
        switch self {
        case .left: .left
        case .right: .right
        case .middle: .middle
