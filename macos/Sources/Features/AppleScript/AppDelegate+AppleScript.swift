...
// Application-level Cocoa scripting hooks for the SpectrePro AppleScript dictionary.
//
...
// the runtime object behind the `application` class in `SpectrePro.sdef`.
...
extension NSApplication {
    ...
    /// the frontmost logical SpectrePro window.
    @objc(frontWindow)
...
extension NSApplication {
    ...
    func handleNewSurfaceConfigurationScriptCommand(_ command: NSScriptCommand) -> NSDictionary? {
        ...
        do {
            let configuration = try SpectrePro.SurfaceConfiguration(
    ...
    func handleNewWindowScriptCommand(_ command: NSScriptCommand) -> ScriptWindow? {
        ...
        guard let appDelegate = delegate as? AppDelegate else {
            ...
            command.scriptErrorString = "SpectrePro app delegate is unavailable."
            return nil
        }

        let baseConfig: SpectrePro.SurfaceConfiguration?
        if let scriptRecord = command.evaluatedArguments?["configuration"] as? NSDictionary {
            do {
                baseConfig = try SpectrePro.SurfaceConfiguration(scriptRecord: scriptRecord)
            } catch {
                command.scriptErrorNumber = errAECoercionFail
                command.scriptErrorString = error.localizedDescription
                return nil
...
@MainActor
extension NSApplication {
    /// Whether SpectrePro should currently accept AppleScript interactions.
    var isAppleScriptEnabled: Bool {
        guard let appDelegate = delegate as? AppDelegate else { return true }
        return appDelegate.spectrepro.config.macosAppleScript
    }
    ...
    fileprivate var allSurfaceViews: [SpectrePro.SurfaceView] {
