...
/// Handler for the `send key` AppleScript command defined in `SpectrePro.sdef`.
///
/// Cocoa scripting instantiates this class because the command's `<cocoa>` element
/// specifies `class="SpectreProScriptKeyEventCommand"`. The runtime calls
/// `performDefaultImplementation()` to execute the command.
@MainActor
@objc(SpectreProScriptKeyEventCommand)
...
extension ScriptKeyEventCommand {
    ...
    static func parse(
        ...
        translationMods: (SpectrePro.Input.Mods) -> SpectrePro.Input.Mods = { $0 },
    ) throws -> SpectrePro.Input.KeyEvent {
        guard let keyName = directParameter as? String else {
            throw ArgumentError.missingKey
        }

        guard let key = SpectrePro.Input.Key(rawValue: keyName) else {
            throw ArgumentError.unknownKey(keyName)
        }

        let action: SpectrePro.Input.Action
        ...
        return SpectrePro.Input.KeyEvent(
            synthesizing: key,
            action: action,
            mods: mods,
