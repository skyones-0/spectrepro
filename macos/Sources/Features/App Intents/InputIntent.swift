...
struct InputTextIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        guard await requestIntentPermission() else {
            throw SpectreProIntentError.permissionDenied
        }

        guard let surface = terminal.surfaceModel else {
            throw SpectreProIntentError.surfaceNotFound
        }
...
struct KeyEventIntent: AppIntent {
    ...
    var action: SpectrePro.Input.Action
    ...
    func perform() async throws -> some IntentResult {
        ...
        // Convert KeyEventMods array to SpectrePro.Input.Mods
        let spectreproMods = mods.reduce(SpectrePro.Input.Mods()) { result, mod in
            result.union(mod.spectreproMod)
        }

        let keyEvent = SpectrePro.Input.KeyEvent(
            key: key,
            action: action,
            mods: spectreproMods
        )
...
struct MouseButtonIntent: AppIntent {
    ...
    var button: SpectrePro.Input.MouseButton
    ...
    var action: SpectrePro.Input.MouseState
    ...
    func perform() async throws -> some IntentResult {
        ...
        let mouseEvent = SpectrePro.Input.MouseButtonEvent(
            action: action,
            button: button,
            mods: spectreproMods
        )
...
struct MousePosIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        let mousePosEvent = SpectrePro.Input.MousePosEvent(
            x: x,
            y: y,
            mods: spectreproMods
        )
...
struct MouseScrollIntent: AppIntent {
    ...
    @Parameter(
        ...
        default: SpectrePro.Input.Momentum.none
    )
    var momentum: SpectrePro.Input.Momentum
...
enum KeyEventMods: String, AppEnum, CaseIterable {
    ...
    var spectreproMod: SpectrePro.Input.Mods {
        switch self {
        case .shift: .shift
        case .control: .ctrl
        case .option: .alt
