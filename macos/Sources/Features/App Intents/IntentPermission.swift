...
/// Requests permission for Shortcuts app to interact with SpectrePro
///
/// This function displays a permission dialog asking the user to allow Shortcuts
/// to interact with SpectrePro. The permission is automatically cached for 10 minutes
...
/// permission for one intent allows all SpectrePro intents to execute without additional
    ...
    await withCheckedContinuation { continuation in
        Task { @MainActor in
            if let delegate = NSApp.delegate as? AppDelegate {
                switch delegate.spectrepro.config.macosShortcuts {
                case .allow:
            ...
            PermissionRequest.show(
                "co.skyones.spectrepro.shortcutsPermission",
                message: "Allow Shortcuts to interact with SpectrePro?",
