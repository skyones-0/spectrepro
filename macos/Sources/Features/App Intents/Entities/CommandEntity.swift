...
struct CommandEntity: AppEntity {
    ...
    let command: SpectrePro.Command
    ...
    init(_ command: SpectrePro.Command, for terminal: TerminalEntity) {
...
struct CommandQuery: EntityQuery {
    ...
    func entities(for identifiers: [CommandEntity.ID]) async throws -> [CommandEntity] {
        ...
        let commands = appDelegate.spectrepro.config.commandPaletteEntries
    ...
    func suggestedEntities() async throws -> [CommandEntity] {
        ...
        return appDelegate.spectrepro.config.commandPaletteEntries.map { CommandEntity($0, for: terminal) }
    }
}
