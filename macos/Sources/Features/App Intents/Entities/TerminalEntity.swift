...
struct TerminalEntity: AppEntity {
    ...
    @MainActor
    var surfaceView: SpectrePro.SurfaceView? {
        Self.defaultQuery.all.first { $0.id == self.id }
    }

    @MainActor
    var surfaceModel: SpectrePro.Surface? {
    ...
    @MainActor
    init(_ view: SpectrePro.SurfaceView) {
        self.id = view.id
    ...
    @MainActor
    init(view: SpectrePro.SurfaceView) async {
        self.id = view.id
        self.tty = view.surfaceModel?.ttyName
...
struct TerminalQuery: EntityStringQuery, EnumerableEntityQuery {
    ...
    @MainActor
    var all: [SpectrePro.SurfaceView] {
