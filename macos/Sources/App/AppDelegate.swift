...
class AppDelegate: NSObject,
    ...
    /// Various menu items so that we can programmatically sync the keyboard shortcut with the SpectrePro config
    ...
    /// This is the current configuration from the SpectrePro configuration that we need.
    private var derivedConfig: DerivedConfig = DerivedConfig()

    /// The spectrepro global state. Only one per process.
...
#else
        spectrepro = SpectrePro.App()
#endif
        super.init()

        spectrepro.delegate = self
    }
    ...
    func applicationWillFinishLaunching(_ notification: Notification) {
        ...
        if
            ...
            let clear = ProcessInfo.processInfo.environment["SPECTREPRO_CLEAR_USER_DEFAULTS"],
            (clear as NSString).boolValue {
            UserDefaults.spectrepro.removePersistentDomain(forName: suite)
        }
        #endif
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if UserDefaults.spectrepro.bool(forKey: "SecureInput") != SecureInput.shared.enabled {
            toggleSecureInput(self)
        }

        // Initial config loading
        spectreproConfigDidChange(config: spectrepro.config)
        ...
        // This registers the SpectrePro => Services menu to exist.
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(spectreproNewWindow(_:)),
            name: SpectrePro.Notification.spectreproNewWindow,
            object: nil)
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(spectreproNewTab(_:)),
            name: SpectrePro.Notification.spectreproNewTab,
            object: nil)

        // Configure user notifications
        let actions = [
            UNNotificationAction(identifier: SpectrePro.userNotificationActionShow, title: "Show")
        ]
        ...
        center.setNotificationCategories([
            UNNotificationCategory(
                identifier: SpectrePro.userNotificationCategory,
                actions: actions,
                intentIdentifiers: [],
        ...
        // Observe our appearance so we can report the correct value to libspectrepro.
        self.appearanceObserver = NSApplication.shared.observe(
            \.effectiveAppearance,
             options: [.new, .initial]
        ) { _, change in
            guard let appearance = change.newValue else { return }
            guard let app = self.spectrepro.app else { return }
            ...
            spectrepro_app_set_color_scheme(app, scheme)
        }
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        why: if let event = NSAppleEventManager.shared().currentAppleEvent {
            // If all SpectrePro windows are in the background (i.e. you Cmd-Q from the Cmd-Tab
        ...
        if !spectrepro.needsConfirmQuit { return .terminateNow }
    ...
    func application(_ sender: NSApplication, openFile filename: String) -> Bool {
        ...
        // SpectrePro will validate as well but we can avoid creating an entirely new
        ...
        if isDirectory.boolValue {
            ...
            config.initialInput = "\(SpectrePro.Shell.quote(filename)); exit\n"
        ...
        if requiresConfirm {
            ...
            let alert = NSAlert()
            alert.messageText = "Allow SpectrePro to execute \"\(filename)\"?"
        ...
        switch spectrepro.config.macosDockDropBehavior {
        case .new_tab:
        ...
        case .new_window: _ = TerminalController.newWindow(spectrepro, withBaseConfig: config)
        }
    ...
    private func setupSignals() {
        ...
        sigusr2.setEventHandler { [weak self] in
            ...
            SpectrePro.logger.info("reloading configuration in response to SIGUSR2")
            self.spectrepro.reloadConfig()
        }
    ...
    private func localEventKeyDown(_ event: NSEvent) -> NSEvent? {
        ...
        if let app = spectrepro.app, let config = spectrepro.config.config {
            var spectreproEvent = event.spectreproKeyEvent(SPECTREPRO_ACTION_PRESS)
            let match = (event.characters ?? "").withCString { ptr in
                spectreproEvent.text = ptr
                if !spectrepro_config_key_is_binding(config, spectreproEvent) {
                    return false
                }

                return spectrepro_app_key(app, spectreproEvent)
            }

            // If the key was handled by SpectrePro we stop the event chain. If
        ...
        // through the SpectrePro key mechanism.

        // SpectrePro must be loaded
        guard let spectrepro = self.spectrepro.app else { return event }

        // Build our event input and call spectrepro
        if spectrepro_app_key(spectrepro, event.spectreproKeyEvent(SPECTREPRO_ACTION_PRESS)) {
            // The key was used so we want to stop it from going to our Mac app
            SpectrePro.logger.debug("local key event handled event=\(event, privacy: .public)")
            return nil
        }
    ...
    @objc private func spectreproConfigDidChange(_ notification: Notification) {
        ...
        spectreproConfigDidChange(config: config)
    }

    @MainActor @objc private func keyboardSelectionDidChange(_ notification: Notification) {
        syncMenuShortcuts(spectrepro.config)
        TerminalController.all.forEach { $0.relabelTabs() }
    }

    @objc private func spectreproBellDidRing(_ notification: Notification) {
        if spectrepro.config.bellFeatures.contains(.system) {
            NSSound.beep()
        }

        if spectrepro.config.bellFeatures.contains(.audio) {
            if let configPath = spectrepro.config.bellAudioPath,
               let sound = NSSound(contentsOfFile: configPath.path, byReference: false) {
                sound.volume = spectrepro.config.bellAudioVolume
                sound.play()
            }
        }

        if spectrepro.config.bellFeatures.contains(.attention) {
    ...
    @objc private func spectreproNewWindow(_ notification: Notification) {
    ...
    @objc private func spectreproNewTab(_ notification: Notification) {
        ...
        _ = TerminalController.newTab(spectrepro, from: window, withBaseConfig: config)
    }
    ...
    private func setDockBadge() {
        ...
        let wantsBadge = spectrepro.config.bellFeatures.contains(.attention) && bellCount > 0
        let label = wantsBadge ? (bellCount > 99 ? "99+" : String(bellCount)) : nil
        NSApp.dockTile.badgeLabel = label
        NSApp.dockTile.display()
    }

    private func spectreproConfigDidChange(config: SpectrePro.Config) {
        ...
        case "never": UserDefaults.spectrepro.setValue(false, forKey: "NSQuitAlwaysKeepsWindows")
        case "always": UserDefaults.spectrepro.setValue(true, forKey: "NSQuitAlwaysKeepsWindows")
        case "default": fallthrough
        default: UserDefaults.spectrepro.removeObject(forKey: "NSQuitAlwaysKeepsWindows")
        }
        ...
        if Bundle.main.infoDictionary?["SUEnableAutomaticChecks"] as? Bool == false {
            ...
            /*
             ...
             delete `SUEnableAutomaticChecks` in SpectrePro-Info.plist.
        ...
        // events that we care about in SpectrePro.
        if spectrepro_app_has_global_keybinds(spectrepro.app!) {
    ...
    private func syncAppearance(config: SpectrePro.Config) {
        NSApplication.shared.appearance = .init(spectreproConfig: config)
    }

    private func updateAppIcon(from config: SpectrePro.Config) {
        Task.detached {
    ...
    func application(_ app: NSApplication, willEncodeRestorableState coder: NSCoder) {
        guard spectrepro.config.windowSaveState != "never" else { return }
    ...
    func application(_ app: NSApplication, didDecodeRestorableState coder: NSCoder) {
        ...
        if spectrepro.config.windowSaveState != "never",
    ...
    func userNotificationCenter(
        ...
        spectrepro.handleUserNotification(response: didReceive)
    ...
    func userNotificationCenter(
        ...
        let shouldPresent = spectrepro.shouldPresentNotification(notification: willPresent)
        let options: UNNotificationPresentationOptions = shouldPresent ? [.banner, .sound] : []
        withCompletionHandler(options)
    }

    // MARK: - SpectreProAppDelegate
    ...
    func setSecureInput(_ mode: SpectrePro.SetSecureInput) {
        ...
        UserDefaults.spectrepro.set(input.global, forKey: "SecureInput")
    }
    ...
    @IBAction func showHelp(_ sender: Any) {
        guard let url = URL(string: "https://spectrepro.org/docs") else { return }
    ...
    @IBAction func toggleVisibility(_ sender: Any) {
        ...
        if NSApp.isActive {
            ...
            // fullscreen. This is only relevant if SpectrePro is active.
...
extension AppDelegate {
    ...
    /// Sync all of our menu item keyboard shortcuts with the SpectrePro configuration.
    @MainActor private func syncMenuShortcuts(_ config: SpectrePro.Config) {
        guard spectrepro.readiness == .ready else { return }
        ...
        // global fullscreen keyboard shortcut. The shortcut in the SpectrePro config will continue
    ...
    @MainActor private func syncMenuShortcut(_ config: SpectrePro.Config, action: String, menuItem: NSMenuItem?) {
        menuShortcutManager.syncMenuShortcut(config, action: action, menuItem: menuItem)
    }

    @MainActor func performSpectreProBindingMenuKeyEquivalent(with event: NSEvent) -> Bool {
        menuShortcutManager.performSpectreProBindingMenuKeyEquivalent(with: event)
    }
