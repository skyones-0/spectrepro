# Localizing SpectrePro: The Translators' Guide

First of all, thanks for helping us localize SpectrePro!
...
Full locale names are more complicated, but SpectrePro does not use all parts. [The
...
file called `co.skyones.spectrepro.pot`. **Do not edit this file.** The
template is generated automatically from SpectrePro's code and resources, and is
...
```console
$ msginit -i po/co.skyones.spectrepro.pot -l X -o "po/X.po"
```
...
Afterwards, you need to update the list of known locales within SpectrePro's
...
Simply run `zig build run`. SpectrePro uses your system language by default; if
...
On some desktop environments, such as KDE Plasma, SpectrePro uses server-side
decorations by default. This hides many strings from the UI, which is
undesirable when viewing your translations. You can force SpectrePro to use
...
help with SpectrePro translations.

### `CODEOWNERS`

Localization teams are represented as teams in the SpectrePro GitHub organization.
...
```diff
 # Localization
 /po/README_TRANSLATORS.md @spectrepro-org/localization
 /po/co.skyones.spectrepro.pot @spectrepro-org/localization
 /po/zh_CN.po @spectrepro-org/zh_CN
+/po/X.po @spectrepro-org/yy_ZZ
```
...
- **Make SpectrePro fit in with surrounding applications.**
  ...
  interface guidelines of each platform SpectrePro is available for, including the
