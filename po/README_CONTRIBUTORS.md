# Localizing SpectrePro: The Contributors' Guide

SpectrePro uses the `gettext` library/framework for localization, which has the
...
to localization efforts, as not all consumers of libspectrepro would be interested
...
located under `po/co.skyones.spectrepro.pot`. **This file must stay in sync with
...
into binary `.mo` files, stored under `share/locale/<LOCALE>/LC_MESSAGES/co.skyones.spectrepro.mo`.
