...
bool decode_png(void* userdata,
  ...
  uint8_t* pixels = spectrepro_alloc(allocator, pixel_len);
...
int main() {
  ...
  int decode_count = 0;
  spectrepro_sys_set(SPECTREPRO_SYS_OPT_USERDATA, &decode_count);
  spectrepro_sys_set(SPECTREPRO_SYS_OPT_DECODE_PNG, (const void*)decode_png);
  ...
  spectrepro_terminal_resize(terminal, 80, 24, 8, 16);

  /* Set a storage limit to enable Kitty graphics. */
  uint64_t storage_limit = 64 * 1024 * 1024;  /* 64 MiB */
  spectrepro_terminal_set(terminal, SPECTREPRO_TERMINAL_OPT_KITTY_IMAGE_STORAGE_LIMIT,
                       &storage_limit);
  ...
  const char* kitty_cmd =
    ...
    "\x1b\\";
  spectrepro_terminal_vt_write(terminal, (const uint8_t*)kitty_cmd,
                            strlen(kitty_cmd));

  printf("PNG decode calls: %d\n", decode_count);

  /* Query the kitty graphics storage to verify the image was stored. */
  SpectreProKittyGraphics graphics = NULL;
  if (spectrepro_terminal_get(terminal, SPECTREPRO_TERMINAL_DATA_KITTY_GRAPHICS,
                           &graphics) != SPECTREPRO_SUCCESS || !graphics) {
  ...
  spectrepro_kitty_graphics_get(graphics, SPECTREPRO_KITTY_GRAPHICS_DATA_GENERATION,
                             &generation);
  printf("Storage generation: %llu\n", (unsigned long long)generation);

  /* Iterate placements to find the image ID. */
  SpectreProKittyGraphicsPlacementIterator iter = NULL;
  if (spectrepro_kitty_graphics_placement_iterator_new(NULL, &iter) != SPECTREPRO_SUCCESS) {
    fprintf(stderr, "Failed to create placement iterator\n");
    return 1;
  }
  if (spectrepro_kitty_graphics_get(graphics,
          SPECTREPRO_KITTY_GRAPHICS_DATA_PLACEMENT_ITERATOR, &iter) != SPECTREPRO_SUCCESS) {
    fprintf(stderr, "Failed to get placement iterator\n");
    return 1;
  }

  int placement_count = 0;
  while (spectrepro_kitty_graphics_placement_next(iter)) {
    placement_count++;
    uint32_t image_id = 0;
    ...
    spectrepro_kitty_graphics_placement_get_multi(iter, 4,
        (SpectreProKittyGraphicsPlacementData[]){
            SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_IMAGE_ID,
            SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_PLACEMENT_ID,
            SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_IS_VIRTUAL,
            SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_Z,
        },
    ...
    SpectreProKittyGraphicsImage image =
        spectrepro_kitty_graphics_image(graphics, image_id);
    if (!image) {
    ...
    SpectreProKittyImageFormat format = 0;
    size_t data_len = 0;
     ...
     */
    spectrepro_kitty_graphics_image_get_multi(image, 6,
        (SpectreProKittyGraphicsImageData[]){
            SPECTREPRO_KITTY_IMAGE_DATA_NUMBER,
            SPECTREPRO_KITTY_IMAGE_DATA_WIDTH,
            SPECTREPRO_KITTY_IMAGE_DATA_HEIGHT,
            SPECTREPRO_KITTY_IMAGE_DATA_FORMAT,
            SPECTREPRO_KITTY_IMAGE_DATA_DATA_LEN,
            SPECTREPRO_KITTY_IMAGE_DATA_GENERATION,
        },
    ...
    if (spectrepro_kitty_graphics_placement_pixel_size(iter, image, terminal,
            &px_w, &px_h) == SPECTREPRO_SUCCESS) {
      printf("    rendered pixel size: %ux%u\n", px_w, px_h);
    }
    if (spectrepro_kitty_graphics_placement_grid_size(iter, image, terminal,
            &cols, &rows) == SPECTREPRO_SUCCESS) {
      printf("    grid size: %u cols x %u rows\n", cols, rows);
    }
  }
  printf("Total placements: %d\n", placement_count);
  spectrepro_kitty_graphics_placement_iterator_free(iter);

  /* Clean up. */
  spectrepro_terminal_free(terminal);

  /* Clear the sys callbacks. */
  spectrepro_sys_set(SPECTREPRO_SYS_OPT_DECODE_PNG, NULL);
  spectrepro_sys_set(SPECTREPRO_SYS_OPT_USERDATA, NULL);
