...
// SpectreProReader callbacks are synchronous. A successful zero-byte read is
...
int main(void) {
  ...
  //! [snapshot-encode]
  SpectreProTerminal source = NULL;
  ...
  result = spectrepro_terminal_new(NULL, &source, 215, 2);
  ...
  result = spectrepro_terminal_set(
      source,
      SPECTREPRO_TERMINAL_OPT_CONTINUATION_MAX_BYTES,
      &continuation_limit);
  assert(result == SPECTREPRO_SUCCESS);

  // Keep enough scrollback to demonstrate incremental history restoration.
  result = spectrepro_terminal_set(
      source, SPECTREPRO_TERMINAL_OPT_SCROLLBACK_MAX_BYTES, NULL);
  assert(result == SPECTREPRO_SUCCESS);
  ...
  result = spectrepro_snapshot_encode_alloc(
  ...
  //! [snapshot-decode]
  SpectreProSnapshotDecoder full_decoder = NULL;
  result = spectrepro_snapshot_decoder_new_buf(
      NULL, &full_decoder, snapshot, snapshot_len);
  assert(result == SPECTREPRO_SUCCESS);

  SpectreProTerminal full_terminal = NULL;
  result = spectrepro_snapshot_decoder_decode(full_decoder, &full_terminal);
  assert(result == SPECTREPRO_SUCCESS);

  spectrepro_snapshot_decoder_free(full_decoder);
  spectrepro_terminal_free(full_terminal);
  //! [snapshot-decode]
  ...
  SpectreProSnapshotDecoder incremental_decoder = NULL;
  result = spectrepro_snapshot_decoder_new(
      NULL, &incremental_decoder, reader);
  assert(result == SPECTREPRO_SUCCESS);

  // READY returns a validated, renderable terminal before old history.
  SpectreProTerminal incremental_terminal = NULL;
  result = spectrepro_snapshot_decoder_ready(
      incremental_decoder, &incremental_terminal);
  assert(result == SPECTREPRO_SUCCESS);

  uint64_t history_rows = 0;
  result = spectrepro_snapshot_decoder_get(
      incremental_decoder,
      SPECTREPRO_SNAPSHOT_DECODER_DATA_HISTORY_ROWS_PRIMARY,
      &history_rows);
  assert(result == SPECTREPRO_SUCCESS);
  ...
  size_t page_count = 0;
  while ((result = spectrepro_snapshot_decoder_next(incremental_decoder)) ==
         SPECTREPRO_SUCCESS) {
    SpectreProTerminalScreen screen;
    size_t rows = 0;
    uint32_t remaining = 0;
    const SpectreProSnapshotDecoderData keys[] = {
        SPECTREPRO_SNAPSHOT_DECODER_DATA_PROGRESS_SCREEN,
        SPECTREPRO_SNAPSHOT_DECODER_DATA_PROGRESS_ROWS,
        SPECTREPRO_SNAPSHOT_DECODER_DATA_PROGRESS_REMAINING,
    };
    void *values[] = {&screen, &rows, &remaining};
    size_t written = 0;
    result = spectrepro_snapshot_decoder_get_multi(
        incremental_decoder,
  ...
  assert(page_count > 0);
  assert(spectrepro_snapshot_decoder_next(incremental_decoder) ==
         SPECTREPRO_NO_VALUE);

  spectrepro_snapshot_decoder_free(incremental_decoder);
  spectrepro_terminal_free(incremental_terminal);
  //! [snapshot-incremental]

  spectrepro_free(NULL, snapshot, snapshot_len);
  spectrepro_terminal_free(source);
  return 0;
}
