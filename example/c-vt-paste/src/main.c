...
#define GS(s) ((SpectreProString){.ptr = (const uint8_t*)(s), .len = sizeof(s) - 1})
...
static void on_write_pty(SpectreProTerminal terminal,
                         void* userdata,
                         const uint8_t* data,
                         size_t len) {
  (void)terminal;
...
static void on_clipboard_read(SpectreProTerminal terminal,
                              void* userdata,
...
static bool read_clipboard(void* userdata, SpectreProString mime, SpectreProWriter writer) {
...
static void paste_clipboard(SpectreProTerminal terminal, const char* text) {
  clipboard_t clipboard = {.text = text};
  SpectreProString mimes[] = {
      // The first text representation is what a text paste writes.
      GS("text/plain"),
      // Listed on a paste event, never read.
      GS("image/png"),
  };
  SpectreProPaste paste = {
      .size = sizeof(paste),
      .location = SPECTREPRO_CLIPBOARD_LOCATION_STANDARD,
      .source = SPECTREPRO_PASTE_SOURCE_CLIPBOARD,
      .mimes = mimes,
  ...
  bool written = false;
  SpectreProResult result = spectrepro_terminal_paste(terminal, &paste, &written);
  if (result == SPECTREPRO_REJECTED) {
    ...
    result = spectrepro_terminal_paste(terminal, &paste, &written);
  }
...
void safety_example() {
  ...
  if (spectrepro_paste_is_safe(safe_data, strlen(safe_data))) {
    printf("Safe to paste\n");
  }

  if (!spectrepro_paste_is_safe(unsafe_data, strlen(unsafe_data))) {
...
void encode_example() {
  ...
  SpectreProResult result = spectrepro_paste_encode(
...
static void vt_write(SpectreProTerminal terminal, const char* seq) {
  spectrepro_terminal_vt_write(terminal, (const uint8_t*)seq, strlen(seq));
}
...
int main() {
  ...
  printf("IME text with mode 5522 enabled:\n");
  {
    ...
    SpectreProString mime = GS("text/plain");
    SpectreProPaste paste = {
        .size = sizeof(paste),
        .location = SPECTREPRO_CLIPBOARD_LOCATION_STANDARD,
        .source = SPECTREPRO_PASTE_SOURCE_TEXT,
        .mimes = &mime,
        .mimes_len = 1,
        .reader = {.read = read_clipboard, .userdata = &clipboard},
        .allow_unsafe = true,
    ...
    bool written = false;
    if (spectrepro_terminal_paste(terminal, &paste, &written) == SPECTREPRO_SUCCESS &&
        written) {
      printf("  written\n");
    }
  }
