...
void basic_example() {
  ...
  SpectreProResult result = spectrepro_sgr_new(NULL, &parser);
  assert(result == SPECTREPRO_SUCCESS);

  // Parse "bold, red foreground" sequence: ESC[1;31m
  uint16_t params[] = {1, 31};
  result = spectrepro_sgr_set_params(parser, params, NULL, 2);
  assert(result == SPECTREPRO_SUCCESS);

  // Iterate through attributes
  SpectreProSgrAttribute attr;
  while (spectrepro_sgr_next(parser, &attr)) {
    switch (attr.tag) {
      case SPECTREPRO_SGR_ATTR_BOLD:
        printf("Bold enabled\n");
        break;
      case SPECTREPRO_SGR_ATTR_FG_8:
...
void advanced_example() {
  ...
  result = spectrepro_sgr_set_params(parser, params, separators, sizeof(params) / sizeof(params[0]));
  assert(result == SPECTREPRO_SUCCESS);
  ...
  while (spectrepro_sgr_next(parser, &attr)) {
    ...
    switch (attr.tag) {
      case SPECTREPRO_SGR_ATTR_UNDERLINE:
        printf("Underline style = ");
        switch (attr.value.underline) {
          case SPECTREPRO_SGR_UNDERLINE_NONE:
            printf("none\n");
            break;
          case SPECTREPRO_SGR_UNDERLINE_SINGLE:
            printf("single\n");
            break;
          case SPECTREPRO_SGR_UNDERLINE_DOUBLE:
            printf("double\n");
            break;
          case SPECTREPRO_SGR_UNDERLINE_CURLY:
            printf("curly\n");
            break;
          case SPECTREPRO_SGR_UNDERLINE_DOTTED:
            printf("dotted\n");
            break;
          case SPECTREPRO_SGR_UNDERLINE_DASHED:
            printf("dashed\n");
            break;
          default:
      ...
      case SPECTREPRO_SGR_ATTR_DIRECT_COLOR_FG:
      ...
      case SPECTREPRO_SGR_ATTR_DIRECT_COLOR_BG:
      ...
      case SPECTREPRO_SGR_ATTR_UNDERLINE_COLOR:
      ...
      case SPECTREPRO_SGR_ATTR_BG_8:
        printf("Background 8-color = %d\n", attr.value.bg_8);
        break;

      case SPECTREPRO_SGR_ATTR_FG_256:
        printf("Foreground 256-color = %d\n", attr.value.fg_256);
        break;

      case SPECTREPRO_SGR_ATTR_BG_256:
      ...
      case SPECTREPRO_SGR_ATTR_ITALIC:
        printf("Italic\n");
        break;

      case SPECTREPRO_SGR_ATTR_UNSET:
        printf("Reset all attributes\n");
        break;

      case SPECTREPRO_SGR_ATTR_UNKNOWN:
