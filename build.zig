...
/// Libspectrepro version. We use a separate version from the app.
...
pub fn build(b: *std.Build) !void {
    ...
    // SpectrePro dependencies used by many artifacts.
    const deps = try buildpkg.SharedDeps.init(b, &config);

    // The modules exported for Zig consumers of libspectrepro. If you're
    // writing a Zig program that uses libspectrepro, read this file.
    const mod = try buildpkg.SpectreProZig.init(
        b,
        &config,
        &deps,
    );
    ...
    const test_lib_vt_step = b.step(
        "test-lib-vt",
        "Run libspectrepro-vt tests",
    );
    const test_lib_vt_build_step = b.step(
        "test-lib-vt-build",
        "Build libspectrepro-vt tests without running them (compile check)",
    );
    const test_lib_vt_schema_step = b.step(
        "test-lib-vt-schema",
        "Validate the libspectrepro-vt ABI type manifest",
    );
    ...
    // SpectrePro resources like terminfo, shell integration, themes, etc.
    const resources = try buildpkg.SpectreProResources.init(b, &config, &deps);
    const i18n = if (config.i18n) try buildpkg.SpectreProI18n.init(b, &config) else null;

    // SpectrePro executable, the actual runnable SpectrePro program.
    const exe = try buildpkg.SpectreProExe.init(b, &config, &deps);

    // SpectrePro docs
    const docs = try buildpkg.SpectreProDocs.init(b, &deps);
    if (config.emit_docs) {
        docs.install();
    ...
    // SpectrePro webdata
    const webdata = try buildpkg.SpectreProWebdata.init(b, &deps);
    if (config.emit_webdata) webdata.install();

    // SpectrePro bench tools
    if (config.emit_bench) {
        const bench = try buildpkg.SpectreProBench.init(b, &deps);
        bench.install();
    }

    // SpectrePro dist tarball
    const dist = try buildpkg.SpectreProDist.init(b, &config);
    {
    ...
    const libspectrepro_vt_shared: ?buildpkg.SpectreProLibVt = shared: {
        if (config.target.result.cpu.arch.isWasm()) {
            break :shared try buildpkg.SpectreProLibVt.initWasm(
                b,
                &mod,
            );
        }
        ...
        break :shared try buildpkg.SpectreProLibVt.initShared(
            b,
            &mod,
        );
    };
    if (libspectrepro_vt_shared) |shared| {
    ...
    // libspectrepro-vt static lib
    const libspectrepro_vt_static = try buildpkg.SpectreProLibVt.initStatic(
        b,
        &mod,
    );
    if (config.is_dep) {
        // If we're a dependency, we need to install everything as-is
        // so that dep.artifact("spectrepro-vt-static") works.
        libspectrepro_vt_static.install(b.getInstallStep());
    } else {
        // If we're not a dependency, we rename the static lib to
        // be idiomatic. On Windows, we use a distinct name to avoid
        // colliding with the DLL import library (spectrepro-vt.lib).
        const static_lib_name = if (config.target.result.os.tag == .windows)
            "spectrepro-vt-static.lib"
        else
            "libspectrepro-vt.a";
        b.getInstallStep().dependOn(&b.addInstallLibFile(
            libspectrepro_vt_static.output,
            static_lib_name,
        ).step);

        if (native_freestanding) {
            b.getInstallStep().dependOn(&b.addInstallDirectory(.{
                .source_dir = b.path("include/spectrepro"),
                .install_dir = .header,
                .install_subdir = "spectrepro",
                .include_extensions = &.{".h"},
            }).step);
        }
    }

    // libspectrepro-vt xcframework (Apple only, universal binary).
    ...
    {
        const apple_libs = try buildpkg.SpectreProLibVt.initStaticAppleUniversal(
            b,
            &config,
            &deps,
            &mod,
        );
        const xcframework = buildpkg.SpectreProLibVt.xcframework(&apple_libs, b);
    ...
    // Runtime "none" is libspectrepro, anything else is an executable.
    if (config.app_runtime != .none) {
        ...
        // The macOS SpectrePro Library
        //
        // This is NOT libspectrepro (even though its named that for historical
        // reasons). It is just the glue between SpectrePro GUI on macOS and
        // the full SpectrePro GUI core.
        const lib_shared = try buildpkg.SpectreProLib.initShared(b, &deps);
        const lib_static = try buildpkg.SpectreProLib.initStatic(b, &deps);
        ...
        if (!config.target.result.os.tag.isDarwin()) {
            ...
            if (config.target.result.os.tag == .windows) {
                lib_shared.install("spectrepro-internal.dll");
                lib_static.install("spectrepro-internal-static.lib");
            } else {
                lib_shared.install("spectrepro-internal.so");
                lib_static.install("spectrepro-internal.a");
            }
        }
    }
    ...
    {
        // SpectrePro xcframework
        const xcframework = try buildpkg.SpectreProXCFramework.init(
            b,
            &deps,
        ...
        // SpectrePro macOS app
        const macos_app = try buildpkg.SpectreProXcodebuild.init(
            b,
            &config,
            .{
    ...
    run: {
        if (config.app_runtime != .none) {
            ...
            // work correctly. If we're running `zig build run` in SpectrePro,
            // this also ensures it overwrites the release one with our debug
            // build.
            run_cmd.setEnvironmentVariable(
                "SPECTREPRO_RESOURCES_DIR",
                b.getInstallPath(.prefix, "share/spectrepro"),
            );
        ...
        if (!config.emit_lib_vt and
            ...
            (config.emit_xcframework or config.emit_macos_app))
        {
            const xcframework_native = try buildpkg.SpectreProXCFramework.init(
                b,
                &deps,
                .native,
            );
            const macos_app_native_only = try buildpkg.SpectreProXcodebuild.init(
                b,
                &config,
                .{
    ...
    // Tests (skip when building libspectrepro-vt)
    if (!config.emit_lib_vt) {
        // Full unit tests
        const test_exe = b.addTest(.{
            .name = "spectrepro-test",
            .filters = test_filters,
        ...
        addSpectreProH(b, test_exe.root_module, config.baselineTarget(b.graph.io), .Debug);
        ...
        // Normal tests always test our libspectrepro modules
...
fn addSpectreProH(
    ...
    module.addImport("spectrepro.h", translated.mod);
}
