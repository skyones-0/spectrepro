...
pub const Id = enum(u64) {
    ...
    test "generate ID" {
        ...
        const id: Id = .generate(Log.create("co.skyones.spectrepro", "test"));
        try std.testing.expect(id != .invalid);
    }

    test "generate ID for pointer" {
        var foo: usize = 0x1234;
        const id: Id = .forPointer(Log.create("co.skyones.spectrepro", "test"), &foo);
...
test enabled {
    _ = enabled(Log.create("co.skyones.spectrepro", "test"));
}

test "intervals" {
    init();

    const log = Log.create("co.skyones.spectrepro", "test");
    defer log.release();
