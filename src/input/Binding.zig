...
pub const Flags = packed struct {
    ...
    /// and not just while SpectrePro is focused. This may not work on all platforms.
    ...
    ///
    /// Sync with spectrepro.h for enums.
...
pub const Action = union(enum) {
    ...
    ///
    /// SpectrePro will not process this combination nor forward it to the child
    ...
    ///
    /// This cannot unbind bindings that were not bound by SpectrePro or the user
    ...
    /// found by running `spectrepro +version`.
    toggle_tab_overview,
    ...
    ///   a new SpectrePro window to edit the configuration file. GTK only. (Available
    ///   since 1.4.0.)
    ...
    /// You must manually untoggle it or quit SpectrePro entirely to disable it.
    ///
    ...
    /// version can be found by running `spectrepro +version`.
    ...
    ///     plugin in the plugin list. SpectrePro would then need to be restarted
    ...
    /// SpectrePro becomes focused. When hiding all windows, focus is yielded
    ...
    /// windows.
    ///
    /// Not every action in SpectrePro can be undone or redone. The list
    ...
    /// Crash SpectrePro in the desired thread for the focused surface.
    ///
    ...
    pub const WriteScreen = struct {
        ...
        pub fn parse(param: []const u8) !WriteScreen {
            ...
            // also very important for backwards compatibility before SpectrePro
...
///
/// This must be kept in sync with include/spectrepro.h spectrepro_input_trigger_s
pub const Trigger = struct {
    ...
    pub const Key = union(C.Tag) {
        ...
        /// registered keycode with SpectrePro.
        unicode: u21,
    ...
    pub fn parse(input: []const u8) !Trigger {
        ...
        loop: while (rem.len > 0) {
            ...
            // keys with SpectrePro 1.1.x. We do this last so its least likely
    ...
    /// The values that are backwards compatible with SpectrePro 1.1.x.
    /// SpectrePro 1.2+ doesn't support these anymore since we moved to
    /// W3C key codes.
...
// For SpectrePro 1.2+ we changed our key names to match the W3C and removed
...
test "action: format" {
    ...
    const a: Action = .{ .text = "👻SpectrePro'\"" };
