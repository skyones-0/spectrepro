//! The main entrypoint for the `spectrepro` application. This also serves
//! as the process initialization code for the `libspectrepro` library.
...
const SpectrePro = @import("main_c.zig").SpectrePro;
...
pub fn main(minimal: std.process.Init.Minimal) !MainReturn {
        ...
        switch (@as(ErrSet, @errorCast(err))) {
            ...
            error.InvalidAction => try stderr.print(
                "Error: unknown CLI action specified. CLI actions are specified with\n" ++
                    "the '+' character.\n\n" ++
                    "All valid CLI actions can be listed with `spectrepro +help`\n",
                .{},
            ),
    ...
    if (comptime builtin.mode == .Debug) {
        ...
        std.log.warn("You should only use a debug build for developing SpectrePro.", .{});
    ...
    if (comptime build_config.app_runtime == .none) {
        ...
        try stdout.print("Usage: spectrepro +<action> [flags]\n\n", .{});
        try stdout.print(
            \\This is the SpectrePro helper CLI that accompanies the graphical SpectrePro app.
            \\To launch the terminal directly, please launch the graphical app
            \\(i.e. SpectrePro.app on macOS). This CLI can be used to perform various
            \\actions such as inspecting the version, listing fonts, etc.
            \\
            \\On macOS, the terminal can also be launched using `open -na SpectrePro.app`,
            \\or `open -na SpectrePro.app --args --foo=bar --baz=qux` to pass arguments.
            \\
...
pub const std_options: std.Options = .{
    ...
    // via SPECTREPRO_LOG because our debug logs are very expensive to
