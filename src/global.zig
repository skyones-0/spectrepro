...
comptime {
    switch (@import("terminal_options").artifact) {
        .spectrepro => {},
        // This file is not allowed to be included in libspectrepro-vt
        .lib => @compileError("global state cannot be used in libspectrepro-vt"),
    }
}

/// We export the xev backend we want to use so that the rest of
/// SpectrePro can import this once and have access to the proper
/// backend.
pub const xev = @import("xev").Dynamic;

/// Global process state. This is initialized in main() for exe artifacts and
/// by spectrepro_init() for lib artifacts. Most other methods in this file will
...
pub const InitOpts = union(enum) {
    ...
    /// Same as `main` but for auxiliary tool binaries (e.g. spectrepro-bench
    /// and spectrepro-gen) that have their own CLI action namespace. This
    /// skips detection of spectrepro app CLI actions, since tool actions
...
pub fn init(opts: InitOpts) !void {
    ...
    // easy to set.
    logging: {
        const v = self.environ.getAlloc(self.alloc, "SPECTREPRO_LOG") catch |err| switch (err) {
    ...
    std.log.info("spectrepro version={s}", .{build_config.version_string});
    std.log.info("spectrepro build optimize={s}", .{build_config.mode_string});
...
///
/// Asserts that the global state is initialized.
pub fn action() ?cli.spectrepro.Action {
    return state.?.action;
}
