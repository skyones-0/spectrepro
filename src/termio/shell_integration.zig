...
pub fn setupFeatures(
    ...
    if (writer.end > 0) {
        try env.put("SPECTREPRO_SHELL_FEATURES", buf[0..writer.end]);
    }
}
...
test "setup features" {
    ...
    // Test: all features enabled
    {
        ...
        try testing.expectEqualStrings("cursor:blink,path,ssh-env,ssh-terminfo,sudo,title", env.get("SPECTREPRO_SHELL_FEATURES").?);
    }
    ...
    // Test: all features disabled
    {
        ...
        try testing.expect(env.get("SPECTREPRO_SHELL_FEATURES") == null);
    }
    ...
    // Test: mixed features
    {
        ...
        try testing.expectEqualStrings("ssh-env,sudo", env.get("SPECTREPRO_SHELL_FEATURES").?);
    }
    ...
    // Test: blinking cursor
    {
        ...
        try testing.expectEqualStrings("cursor:blink", env.get("SPECTREPRO_SHELL_FEATURES").?);
    }
    ...
    // Test: steady cursor
    {
        ...
        try testing.expectEqualStrings("cursor:steady", env.get("SPECTREPRO_SHELL_FEATURES").?);
    }
}
...
fn setupBash(
    ...
    if (env.get("ENV")) |v| {
        try env.put("SPECTREPRO_BASH_ENV", v);
    }
    ...
    const script_path = try std.fmt.bufPrint(
        &script_path_buf,
        "{s}/shell-integration/bash/spectrepro.bash",
        .{resource_dir},
    );
    if (std.Io.Dir.openFileAbsolute(global.io(), script_path, .{})) |file| {
        ...
        _ = env.swapRemove("SPECTREPRO_BASH_ENV");
        return null;
    }

    try env.put("SPECTREPRO_BASH_INJECT", buf[0..inject.end]);
    if (rcfile) |v| {
        try env.put("SPECTREPRO_BASH_RCFILE", v);
    }
    ...
    if (env.get("HISTFILE") == null) {
        ...
        if (try homedir.home(global.io(), &environ_map, &home_buf)) |home| {
            ...
            try env.put("SPECTREPRO_BASH_UNEXPORT_HISTFILE", "1");
        }
    }
...
test "bash" {
    ...
    try testing.expectEqualStrings("1", env.get("SPECTREPRO_BASH_INJECT").?);

    var path_buf: [std.fs.max_path_bytes]u8 = undefined;
    try testing.expectEqualStrings(
        try std.fmt.bufPrint(&path_buf, "{s}/spectrepro.bash", .{res.shell_path}),
        env.get("ENV").?,
    );
}
...
test "bash: inject flags" {
    ...
    // bash --norc
    {
        ...
        try testing.expectEqualStrings("bash --posix", command.?.shell);
        try testing.expectEqualStrings("1 --norc", env.get("SPECTREPRO_BASH_INJECT").?);
    }
    ...
    // bash --noprofile
    {
        ...
        try testing.expectEqualStrings("bash --posix", command.?.shell);
        try testing.expectEqualStrings("1 --noprofile", env.get("SPECTREPRO_BASH_INJECT").?);
    }
}
...
test "bash: rcfile" {
    ...
    // bash --rcfile
    {
        ...
        try testing.expectEqualStrings("bash --posix", command.?.shell);
        try testing.expectEqualStrings("profile.sh", env.get("SPECTREPRO_BASH_RCFILE").?);
    }
...
test "bash: HISTFILE" {
    ...
    // HISTFILE unset
    {
        ...
        try testing.expectEqualStrings("1", env.get("SPECTREPRO_BASH_UNEXPORT_HISTFILE").?);
    }
    ...
    // HISTFILE set
    {
        ...
        try testing.expect(env.get("SPECTREPRO_BASH_UNEXPORT_HISTFILE") == null);
    }
}
...
test "bash: ENV" {
    ...
    _ = try setupBash(alloc, .{ .shell = "bash" }, res.path, &env);
    try testing.expectEqualStrings("env.sh", env.get("SPECTREPRO_BASH_ENV").?);
...
/// It is also saved in the `SPECTREPRO_SHELL_INTEGRATION_XDG_DIR` variable
...
fn setupXdgDataDirs(
    ...
    try env.put("SPECTREPRO_SHELL_INTEGRATION_XDG_DIR", integ_path);
...
test "xdg: empty XDG_DATA_DIRS" {
    ...
    try testing.expectEqualStrings(
        ...
        env.get("SPECTREPRO_SHELL_INTEGRATION_XDG_DIR").?,
    );
    try testing.expectEqualStrings(
...
/// which Nushell will use to load `nushell/vendor/autoload/spectrepro.nu`.
///
/// We then add `--execute 'use spectrepro ...'` to the nu command line to
...
fn setupNushell(
    ...
    // makes our 'spectrepro' module automatically available, even if any
    ...
    // enabled shell features (e.g. `use spectrepro sudo`). At the moment,
    // shell features are all runtime-guarded in the nushell script.
    try cmd.appendArg("--execute 'use spectrepro *'");
...
test "nushell" {
    ...
    try testing.expectEqualStrings("nu --execute 'use spectrepro *'", command.?.shell);
...
test "nushell: unsupported options" {
    ...
    for (cmdlines) |cmdline| {
        ...
        try testing.expect(env.get("SPECTREPRO_SHELL_INTEGRATION_XDG_DIR") != null);
    }
}
...
fn setupZsh(
    ...
    if (env.get("ZDOTDIR")) |old| {
        try env.put("SPECTREPRO_ZSH_ZDOTDIR", old);
    }
...
test "zsh" {
    ...
    try testing.expect(env.get("SPECTREPRO_ZSH_ZDOTDIR") == null);
}
...
test "zsh: ZDOTDIR" {
    ...
    try testing.expectEqualStrings("zsh", command.?.shell);
    try testing.expectEqualStrings(res.shell_path, env.get("ZDOTDIR").?);
    try testing.expectEqualStrings("$HOME/.config/zsh", env.get("SPECTREPRO_ZSH_ZDOTDIR").?);
}
    ...
    fn init(shell: Shell) !TmpResourcesDir {
        ...
        switch (shell) {
            .bash => try tmp_dir.dir.writeFile(std.testing.io, .{
                .sub_path = "shell-integration/bash/spectrepro.bash",
                .data = "",
            }),
            else => {},
        }
