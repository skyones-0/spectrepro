...
const Subprocess = struct {
    ...
    pub fn init(gpa: Allocator, cfg: Config) !Subprocess {
        ...
        if (cfg.resources_dir) |dir| {
            log.info("found SpectrePro resources dir: {s}", .{dir});
            try env.put("SPECTREPRO_RESOURCES_DIR", dir);
        }

        // Set our TERM var. This is a bit complicated because we want to use
        // the spectrepro TERM value but we want to only do that if we have
        // spectrepro in the TERMINFO database.
        //
        ...
        if (cfg.resources_dir) |base| {
            ...
            if (comptime builtin.target.os.tag.isDarwin()) {
                log.warn("spectrepro terminfo not found, using xterm-256color", .{});
        ...
        spectrepro_path: {
            ...
            const exe_bin_path = exe_buf[0 .. std.process.executablePath(
                ...
                log.warn("failed to get spectrepro exe path err={}", .{err});
                break :spectrepro_path;
            }];
            const exe_dir = std.fs.path.dirname(exe_bin_path) orelse break :spectrepro_path;
            log.debug("appending spectrepro bin to path dir={s}", .{exe_dir});

            // We always set this so that if the shell overwrites the path
            // scripts still have a way to find the SpectrePro binary when
            // running in SpectrePro.
            try env.put("SPECTREPRO_BIN_DIR", exe_dir);

            // Append if we have a path. We want to append so that spectrepro is
            ...
            if (env.get("PATH")) |path| {
                ...
                while (it.next()) |entry| {
                    if (std.mem.eql(u8, entry, exe_dir)) break :spectrepro_path;
                }
        ...
        try env.put("TERM_PROGRAM", "spectrepro");
        ...
        const shell_command: configpkg.Command = shell: {
            ...
            // Always set up shell features (SPECTREPRO_SHELL_FEATURES). These are
            ...
            const force: ?shell_integration.Shell = switch (cfg.shell_integration) {
                .none => {
                    ...
                    // opt-in since it results in some SpectrePro features not
        ...
        const args: []const [:0]const u8 = execCommand(
            ...
            // SpectrePro can launch (and maybe the user can debug this further).
        ...
        // https://github.com/spectrepro-org/spectrepro/discussions/7769
    ...
    /// exec is called; it does NOT run in the main SpectrePro process.
