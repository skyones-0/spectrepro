...
/// A parsed palette entry from SpectrePro's config "N=COLOR" syntax.
pub const PaletteEntry = struct {
    index: u8,
    color: RGB,
};

/// Parse a palette entry in SpectrePro config syntax: "N=COLOR" where N is
...
pub const RGB = packed struct(u24) {
    ...
    ///    accepted for compatibility with SpectrePro config/theme color values.
    ///
    /// 4. X11 color names
