...
// See https://github.com/spectrepro-org/spectrepro/issues/10865 and
...
pub const Option = enum {
    ...
    // by SpectrePro with the "last" option. See Redraw the type for more details.
    redraw,
    ...
    // SpectrePro will then send a click event to the shell when the user
    ...
    // SpectrePro may generate a number of fake key events to move the cursor
...
/// started by Kitty[1] and extended by SpectrePro (the "last" option).
///
...
pub const Redraw = enum(u2) {
    ...
    /// The shell does NOT support redrawing. In this case, SpectrePro will NOT
    /// clear any prompt lines on resize.
    false,

    /// The shell supports redrawing only the LAST line of the prompt.
    /// SpectrePro will only clear the last line of the prompt on resize.
    ///
