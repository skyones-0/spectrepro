...
/// WriteState.Options (SpectrePro exposes it as the
...
test "write: basic transaction" {
    ...
    try testing.expectEqualStrings("SpectrePro", committed.contents[0].data);
}
...
test "write: aliases resolve at commit" {
    ...
    try testing.expectEqualStrings("SpectrePro", committed.contents[1].data);
    try testing.expectEqualStrings("UTF8_STRING", committed.contents[2].mime);
    try testing.expectEqualStrings("SpectrePro", committed.contents[2].data);
}
