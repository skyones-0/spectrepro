...
pub const Command = union(Key) {
    ...
    /// well defined, so this is currently ignored by SpectrePro at the time
    ...
    pub const ProgressReport = struct {
        ...
        // sync with spectrepro_action_progress_report_s
        ...
        test "spectrepro.h Command.ProgressReport.State" {
            if (comptime build_options.artifact == .lib) return error.SkipZigTest;
            const CState = LibEnum(.c, state_keys);
            try lib.checkSpectreProHEnum(CState, "SPECTREPRO_PROGRESS_STATE_");
        }
    };
