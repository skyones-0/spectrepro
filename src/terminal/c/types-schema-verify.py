...
def load_native_manifest(path: Path) -> bytes:
    """Call spectrepro_type_json in a native shared library."""
    library = ctypes.CDLL(str(path.resolve()))
    type_json = library.spectrepro_type_json
    type_json.argtypes = ()
    type_json.restype = ctypes.c_char_p

    result = type_json()
    if result is None:
        raise RuntimeError("spectrepro_type_json returned NULL")
    return result


def load_wasm_manifest(path: Path) -> bytes:
    """Call spectrepro_type_json and read its result from WebAssembly memory."""
    try:
        import wasmtime
    except ImportError as error:
        raise SystemExit(
    ...
    type_json = exports["spectrepro_type_json"]

    pointer = type_json(store)
    data = memory.read(store, pointer, memory.data_len(store))
    terminator = data.find(b"\0")
    if terminator < 0:
        raise RuntimeError("spectrepro_type_json result is not NUL terminated")
...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError("spectrepro_type_json did not return a JSON object")
    return value
