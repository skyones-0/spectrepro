...
/// opts in through `SPECTREPRO_TERMINAL_OPT_CONTINUATION_MAX_BYTES`.
...
///
/// C: SpectreProClipboardWriteReply
...
///
/// C: SpectreProClipboardWriteReplyFn
...
///
/// C: SpectreProClipboardReadReply
...
///
/// C: SpectreProClipboardReadReplyFn
...
///
/// C: SpectreProTerminalDesktopNotification
...
///
/// C: SpectreProTerminalUnknownStringSequence
...
///
/// C: SpectreProTerminalUnknownSequence
pub const UnknownSequence = union(Tag) {
    apc: UnknownStringSequence,

    /// C: SpectreProTerminalUnknownSequenceTag
...
///
/// C: SpectreProTerminalModeConfig
...
const Effects = struct {
    ...
    /// Returns the version string (e.g. "spectrepro 1.2.3"). The memory
    /// must remain valid until the callback returns. An empty string
    /// (len=0) causes the default "libspectrepro" to be reported.
...
/// C: SpectreProTerminal
pub const Terminal = ?*TerminalWrapper;

/// C: SpectreProTerminalCompressionMode
pub const CompressionMode = ZigTerminal.CompressionMode;

/// C: SpectreProTerminalCompressionResult
...
fn new_(
    ...
    // libspectrepro-vt embedders don't necessarily install SpectrePro's shell
...
pub fn continuation_alloc(
    ...
    // Resolve NULL to libspectrepro-vt's default allocator before entering the
    ...
    // length with spectrepro_free and the same allocator selection.
...
/// C: SpectreProTerminalCursorStyle
...
/// C: SpectreProTerminalScrollViewport
...
test "xtversion without callback reports default" {
    ...
    // Set write_pty but not xtversion — should get default "libspectrepro"
    try testing.expectEqual(Result.success, set(t, .write_pty, @ptrCast(&S.writePty)));

    vt_write(t, "\x1B[>q", 4);
    try testing.expect(S.last_data != null);
    try testing.expectEqualStrings("\x1BP>|libspectrepro\x1B\\", S.last_data.?);
}
...
test "kitty clipboard write via C effects" {
    ...
    try testing.expectEqualStrings("text/plain", S.last_mimes[0][0..S.last_mime_lens[0]]);
    try testing.expectEqualStrings("SpectrePro", S.last_data[0][0..S.last_data_lens[0]]);
