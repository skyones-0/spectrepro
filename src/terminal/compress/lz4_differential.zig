...
//! unless the environment variable `SPECTREPRO_LZ4_SLOW` is set:
//!
//!     SPECTREPRO_LZ4_SLOW=1 zig build test -Dtest-filter="lz4 differential"
...
test "lz4 differential exhaustive" {
    ...
    //   SPECTREPRO_LZ4_SLOW=1 zig build test -Dtest-filter="lz4 differential"
    if (!std.testing.environ.containsConstant("SPECTREPRO_LZ4_SLOW"))
