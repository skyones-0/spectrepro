...
//
// Must be kept in sync with spectrepro_cursor_shape_e
pub const Shape = enum(c_int) {
    ...
    test "spectrepro.h MouseShape" {
        if (comptime build_options.artifact == .lib) return error.SkipZigTest;
        try lib.checkSpectreProHEnum(Shape, "SPECTREPRO_MOUSE_SHAPE_");
    }
};
