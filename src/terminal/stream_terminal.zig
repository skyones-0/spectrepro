...
pub const Handler = struct {
    ...
    pub const Effects = struct {
        ...
        /// string to report (e.g. "spectrepro 1.2.3"). The returned memory
    ...
    fn dcsCommand(self: *Handler, cmd: *dcs.Command) !void {
        switch (cmd.*) {
            ...
            .xtgettcap => |*gettcap| {
                ...
                const map = comptime terminfo.spectrepro.xtgettcapMap();
    ...
    // The static map also carries this key with SpectrePro's own name, so
    // it is intercepted before the lookup: an embedder that never
    // configured a name must not be reported as SpectrePro's entry.
    ...
    fn reportXtversion(self: *Handler) void {
        ...
        const resp = std.fmt.bufPrintZ(
            &buf,
            "\x1BP>|{s}\x1B\\",
            .{if (version.len > 0) version else "libspectrepro"},
        ) catch return;
        self.writePty(resp);
    }
...
test "kitty clipboard write transaction round trip" {
    ...
    try testing.expectEqualStrings("SpectrePro", S.dataAt(0));
    ...
    try testing.expectEqualStrings("SpectrePro", S.dataAt(2));
    try testing.expectEqualStrings("UTF8_STRING", S.mimeAt(3));
    try testing.expectEqualStrings("SpectrePro", S.dataAt(3));
    try testing.expectEqualStrings(
    ...
    S.read_result = .{
        .success = .{
            .contents = &.{
                ...
                .{ .mime = "text/plain", .data = "SpectrePro" },
            },
...
test "request mode DECRQM with write_pty callback" {
    ...
    // With callback, DECRQM should produce a response
    {
        ...
        // Query DECECM, which SpectrePro recognizes but does not allow changing
...
test "xtversion default" {
    ...
    // Without xtversion effect set, should report "libspectrepro"
    s.nextSlice("\x1b[>0q");
    try testing.expectEqualStrings("\x1bP>|libspectrepro\x1b\\", S.written.?);
}
...
test "xtversion with effect" {
    ...
    s.nextSlice("\x1b[>0q");
    try testing.expectEqualStrings("\x1bP>|spectrepro 1.2.3\x1b\\", S.written.?);
}
...
test "xtversion with empty string effect" {
    ...
    // Empty string from effect should fall back to "libspectrepro"
    s.nextSlice("\x1b[>0q");
    try testing.expectEqualStrings("\x1bP>|libspectrepro\x1b\\", S.written.?);
}
...
test "enquiry with effect" {
    ...
    s.nextSlice("\x05");
    defer testing.allocator.free(S.written.?);
    try testing.expectEqualStrings("spectrepro", S.written.?);
}
