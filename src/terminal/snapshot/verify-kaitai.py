...
Run this from a SpectrePro development shell:
...
def compile_schema(output_dir: Path) -> type[Any]:
    ...
    try:
        module = importlib.import_module("spectrepro_snapshot")
    finally:
        sys.path.pop(0)
    return module.SpectreProSnapshot
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="spectrepro-kaitai-") as temp:
