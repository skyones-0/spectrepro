...
/// SpectrePro's terminfo entry.
pub const spectrepro: Source = .{
    .names = &.{
        ...
        // features.
        // HACK: This is a hack on a hack...we use "xterm-spectrepro" to prevent
    ...
    // verify that SpectrePro does indeed support it and if not we can fix it.
    .capabilities = &.{
        ...
        // actually work with SpectrePro. todo is to validate these!
...
pub const version = version: {
    ...
    var hashing: std.Io.Writer.Hashing(std.hash.Wyhash) =
        .initHasher(.init(0), &.{});
    spectrepro.encode(&hashing.writer) catch unreachable;
