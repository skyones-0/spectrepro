...
/// owned by the caller.
///
/// This is highly SpectrePro-specific and can likely be generalized at
/// some point but we can cross that bridge if we ever need to.
pub fn resourcesDir(alloc: Allocator) !ResourcesDir {
    // Use the SPECTREPRO_RESOURCES_DIR environment variable in release builds.
    //
    // In debug builds we try using terminfo detection first instead, since
    // if debug SpectrePro is launched by an older version of SpectrePro, it
    // would inherit the old, stale resources of older SpectrePro instead of the
    // freshly built ones under zig-out/share/spectrepro.
    //
    ...
    if (comptime builtin.mode != .Debug) env: {
        const dir = global.environ().getAlloc(alloc, "SPECTREPRO_RESOURCES_DIR") catch |err| switch (err) {
    ...
    const sentinels = switch (comptime builtin.target.os.tag) {
        .windows => .{"terminfo/spectrepro.terminfo"},
        .macos => .{"terminfo/78/xterm-spectrepro"},
        .freebsd => .{ "site-terminfo/g/spectrepro", "site-terminfo/x/xterm-spectrepro" },
        else => .{ "terminfo/g/spectrepro", "terminfo/x/xterm-spectrepro" },
    };
    ...
    while (std.fs.path.dirname(exe)) |dir| {
        ...
        if (comptime builtin.target.os.tag.isDarwin()) {
            inline for (sentinels) |sentinel| {
                if (try maybeDir(
                    ...
                    return .{ .app_path = try std.fs.path.join(alloc, &.{ v, "spectrepro" }) };
                }
            }
        }
        ...
        // SpectrePro to be in an app bundle.
        inline for (sentinels) |sentinel| {
            if (try maybeDir(
                &dir_buf,
                dir,
    ...
    if (comptime builtin.mode == .Debug) {
        if (global.environ().getAlloc(alloc, "SPECTREPRO_RESOURCES_DIR")) |dir| {
            if (dir.len > 0) return .{ .app_path = dir };
        } else |err| switch (err) {
