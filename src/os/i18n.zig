...
/// SpectrePro application. This should not be called for libspectrepro users.
pub fn initGlobalDomain() error{OutOfMemory}!void {
    if (comptime !build_config.i18n) return;
    _ = textdomain(build_config.bundle_id) orelse return error.OutOfMemory;
}

/// Translate a message for the SpectrePro domain.
///
...
test "_ returns msgid at comptime" {
    ...
    const msgid = comptime @"_"("SpectrePro");
    try testing.expectEqualStrings("SpectrePro", std.mem.span(msgid));
}
