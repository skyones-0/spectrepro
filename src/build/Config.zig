...
/// SpectrePro exe properties
exe_entrypoint: ExeEntrypoint = .spectrepro,
...
/// Feature gates for libspectrepro-vt artifacts (-Dvt-features). The full
/// SpectrePro application ignores this and always enables everything.
vt_features: TerminalBuildOptions.Features = .{},

/// True when SpectrePro is being built as a dependency of another project
...
pub fn init(b: *std.Build, appVersion: []const u8, libVersion: []const u8) !Config {
    ...
    // Default dependency builds to libspectrepro-vt-only mode. Consumers can
    // still explicitly disable this to request the full SpectrePro build.
    const is_dep = b.dep_prefix.len > 0;
    const emit_lib_vt = b.option(
        bool,
        "emit-lib-vt",
        "Set defaults for a libspectrepro-vt-only build (disables xcframework, macOS app, and docs).",
    ) orelse is_dep;
    const target = target: {
        ...
        // The full SpectrePro build no longer supports iOS; Fail early
        // with a clear message rather than partway through the build.
        if (result.result.os.tag == .ios and !emit_lib_vt) {
            std.log.err(
                "iOS is not a supported target for the full SpectrePro build; " ++
                    "only libspectrepro-vt supports iOS (-Demit-lib-vt)",
                .{},
            );
    ...
    config.version = if (version_string) |v|
        ...
        // Is spectrepro a dependency? If so, skip git detection.
    ...
    // libspectrepro-vt properties

    const lib_version_string = b.option(
        []const u8,
        "lib-version-string",
        "A specific version string to use for the build of libspectrepro-vt. " ++
            "If not specified, git will be used. This must be a semantic version.",
    );
    ...
    config.vt_features = features: {
        const list = b.option(
            []const u8,
            "vt-features",
            "Comma-separated libspectrepro-vt feature modifications applied " ++
    ...
    config.emit_terminfo = b.option(
        bool,
        "emit-terminfo",
        "Install SpectrePro terminfo source file",
    ) orelse switch (target.result.os.tag) {
        .windows => true,
    ...
    config.emit_termcap = b.option(
        bool,
        "emit-termcap",
        "Install SpectrePro termcap file",
    ) orelse switch (optimize) {
        .Debug => true,
        .ReleaseSafe, .ReleaseFast, .ReleaseSmall => false,
    };
    ...
    config.emit_themes = b.option(
        bool,
        "emit-themes",
        "Install bundled iTerm2-Color-Schemes SpectrePro themes",
    ) orelse true;
...
/// SpectrePro executable being built. Callers should modify this as needed.
...
// (see for example https://github.co/skyones/spectrepro/issues/1640).
