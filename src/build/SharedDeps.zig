...
const SpectreProFrameData = @import("SpectreProFrameData.zig");
const DistResource = @import("SpectreProDist.zig").Resource;
...
framedata: SpectreProFrameData,
uucode_tables: std.Build.LazyPath,

/// Singleton uucode module, instantiated once in `init` and reused
/// everywhere so that spectrepro and vaxis share the same compiled tables in
...
pub fn init(b: *std.Build, cfg: *const Config) !SharedDeps {
    ...
    // Instantiate the singleton uucode module that both spectrepro and vaxis
...
pub fn add(
    ...
    self.config.terminalOptions(.spectrepro, optimize).add(b, step.root_module);
    ...
    // Libpng - SpectrePro doesn't actually use this directly, its only used
    ...
    if (step.rootModuleTarget().os.tag.isDarwin()) {
        ...
        step.root_module.addAnonymousImport("spectrepro_metallib", .{
...
fn addGtkNg(
    ...
    spectrepro_resources_translate: {
        // Get our gresource c/h files and add them to our build.
        const dist = gtkNgDistResources(b);
        const translate_c = b.lazyImport(@import("../../build.zig"), "translate_c") orelse
            break :spectrepro_resources_translate;
        const translate_c_dep = b.lazyDependency("translate_c", .{}) orelse
            break :spectrepro_resources_translate;
        const translated: translate_c.Translator = .init(translate_c_dep, .{
            .c_source_file = b.addWriteFiles().add("c.h",
                \\#include <spectrepro_resources.h>
            ),
            .target = target,
            .optimize = optimize,
            .link_system_libs = &.{
        ...
        step.root_module.addImport("spectrepro_gtk_resources", translated.mod);
    }
}
...
pub fn gtkNgDistResources(
    ...
    const resources_c = generate_c.addOutputFileArg("spectrepro_resources.c");
    ...
    const resources_h = generate_h.addOutputFileArg("spectrepro_resources.h");
    ...
    return .{
        .resources_c = .{
            .dist = "src/apprt/gtk/spectrepro_resources.c",
            .generated = resources_c,
        },
        .resources_h = .{
            .dist = "src/apprt/gtk/spectrepro_resources.h",
