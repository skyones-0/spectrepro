% SPECTREPRO(5) Version @@VERSION@@ | SpectrePro terminal emulator configuration file

# NAME

**spectrepro** - SpectrePro terminal emulator configuration file

# DESCRIPTION

To configure SpectrePro, you must use a configuration file. GUI-based configuration
is on the roadmap but not yet supported. The configuration file must be placed
at `$XDG_CONFIG_HOME/spectrepro/config.spectrepro`, which defaults to `~/.config/spectrepro/config.spectrepro`
...
`$HOME/Library/Application Support/co.skyones.spectrepro/config.spectrepro`.** This is the
...
executing the command `spectrepro +show-config --default --docs`. Note that this will
...
the public config files of many SpectrePro users for examples and inspiration.

## Configuration Errors

If your configuration file has any errors, SpectrePro does its best to ignore
...
macOS and Linux (GTK). SpectrePro does not treat configuration errors as fatal and
will fall back to default values for erroneous keys.

You can also view the full configuration SpectrePro is loading using `spectrepro
+show-config` from the command-line. Use the `--help` flag to additional options
for that command.

## Logging

SpectrePro can write logs to a number of destinations. On all platforms, logging to
`stderr` is available. Depending on the platform and how SpectrePro was launched,
logs sent to `stderr` may be stored by the system and made available for later
retrieval.

On Linux if SpectrePro is launched by the default `systemd` user service, you can use
`journald` to see SpectrePro's logs: `journalctl --user --unit app-co.skyones.spectrepro.service`.

On macOS logging to the macOS unified log is available and enabled by default.
--Use the system `log` CLI to view SpectrePro's logs: `sudo log stream --level debug
--predicate 'subsystem=="co.skyones.spectrepro"'`.

SpectrePro's logging can be configured in two ways. The first is by what
optimization level SpectrePro is compiled with. If SpectrePro is compiled with `Debug`
optimizations debug logs will be output to `stderr`. If SpectrePro is compiled with
any other optimization the debug logs will not be output to `stderr`.

SpectrePro also checks the `SPECTREPRO_LOG` environment variable. It can be used
to control which destinations receive logs. SpectrePro currently defines two
destinations:
