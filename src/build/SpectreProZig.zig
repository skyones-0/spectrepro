//! SpectreProZig generates the Zig modules that SpectrePro exports
//! for downstream usage.
const SpectreProZig = @This();
...
pub fn init(
    ...
    return initInner(b, cfg, deps, "spectrepro-vt", "spectrepro-vt-c");
}

/// Create a new SpectreProZig with modules retargeted to a different
/// architecture. Used to produce universal (fat) binaries on macOS.
pub fn retarget(
    self: *const SpectreProZig,
    b: *std.Build,
    cfg: *const Config,
    deps: *const SharedDeps,
    ...
    return initInner(
        ...
        retargeted_config,
        retargeted_deps,
        b.fmt("spectrepro-vt-{s}", .{arch_name}),
        b.fmt("spectrepro-vt-c-{s}", .{arch_name}),
    );
}
