...
pub fn initStatic(
    ...
    const combined = CombineArchivesStep.create(b, deps.config.target, "spectrepro-internal", lib_list.items);
...
pub fn initShared(
    ...
    const dsymutil: ?std.Build.LazyPath = dsymutil: {
        ...
        dsymutil.addArgs(&.{"-o"});
        const output = dsymutil.addOutputFileArg("libspectrepro.dSYM");
...
pub fn initMacOSUniversal(
    ...
    const universal = LipoStep.create(b, .{
        .name = "spectrepro",
        .out_name = "spectrepro-internal.a",
...
pub fn install(self: *const SpectreProLib, name: []const u8) void {
    ...
    if (self.pkg_config) |pc| {
        step.dependOn(&b.addInstallFileWithDir(
            pc,
            .prefix,
            "share/pkgconfig/spectrepro-internal.pc",
        ).step);
    }
    if (self.pkg_config_static) |pc| {
        step.dependOn(&b.addInstallFileWithDir(
            pc,
            .prefix,
            "share/pkgconfig/spectrepro-internal-static.pc",
        ).step);
    }
}

pub fn installHeader(self: *const SpectreProLib) void {
    const b = self.step.owner;
    const header_install = b.addInstallHeaderFile(
        b.path("include/spectrepro.h"),
        "spectrepro.h",
    );
...
fn pkgConfigFiles(
    ...
    return .{
        .shared = wf.add("spectrepro-internal.pc", b.fmt(
            \\prefix={s}
            \\includedir=${{prefix}}/include
            \\libdir=${{prefix}}/lib
            \\
            \\Name: spectrepro-internal
            \\URL: https://github.com/spectrepro-org/spectrepro
            \\Description: SpectrePro internal library (not for external use)
            \\Version: {f}
            \\Cflags: -I${{includedir}}
            \\Libs: ${{libdir}}/{s}
            \\Libs.private:
        ...
        .static = wf.add("spectrepro-internal-static.pc", b.fmt(
            \\prefix={s}
            \\includedir=${{prefix}}/include
            \\libdir=${{prefix}}/lib
            \\
            \\Name: spectrepro-internal-static
            \\URL: https://github.com/spectrepro-org/spectrepro
            \\Description: SpectrePro internal library, static (not for external use)
            \\Version: {f}
            \\Cflags: -I${{includedir}}
            \\Libs: ${{libdir}}/{s}
            \\Libs.private:
...
fn sharedLibraryName(os_tag: std.Target.Os.Tag) []const u8 {
    return if (os_tag == .windows)
        "spectrepro-internal.dll"
    else
        "spectrepro-internal.so";
}

fn staticLibraryName(os_tag: std.Target.Os.Tag) []const u8 {
    return if (os_tag == .windows)
        "spectrepro-internal-static.lib"
    else
        "spectrepro-internal.a";
}
