const SpectrePro = @This();

const std = @import("std");
const builtin = @import("builtin");
const RunStep = std.Build.Step.Run;
const Config = @import("Config.zig");
const Docs = @import("SpectreProDocs.zig");
const I18n = @import("SpectreProI18n.zig");
const Resources = @import("SpectreProResources.zig");
const XCFramework = @import("SpectreProXCFramework.zig");
...
pub fn init(
    ...
    const app_path = b.fmt("macos/build/{s}/SpectrePro.app", .{xc_config});

    // Our step to build the SpectrePro macOS app.
    const build = build: {
    ...
    // Our step to open the resulting SpectrePro app.
    const open = open: {
        ...
        const open = RunStep.create(b, "run SpectrePro app");
        open.has_side_effects = true;
        open.cwd = b.path("");
        open.addArgs(&.{b.fmt(
            "{s}/Contents/MacOS/spectrepro",
            .{app_path},
        )});
        ...
        open.setEnvironmentVariable("SPECTREPRO_LOG", "stderr,macos");

        // Configure how we're launching
        open.setEnvironmentVariable("SPECTREPRO_MAC_LAUNCH_SOURCE", "zig_run");
