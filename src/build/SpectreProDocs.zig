//! SpectreProDocs generates all the on-disk documentation that SpectrePro is
//! installed with (man pages, html, markdown, etc.)
const SpectreProDocs = @This();
...
pub fn init(
    ...
    const manpages = [_]struct {
        ...
        .{ .name = "spectrepro", .section = "1" },
        .{ .name = "spectrepro", .section = "5" },
    };
    ...
    inline for (manpages) |manpage| {
        ...
        try steps.append(b.allocator, &b.addInstallFile(
            markdown_output,
            "share/spectrepro/doc/" ++ manpage.name ++ "." ++ manpage.section ++ ".md",
        ).step);
        ...
        try steps.append(b.allocator, &b.addInstallFile(
            ...
            "share/spectrepro/doc/" ++ manpage.name ++ "." ++ manpage.section ++ ".html",
        ).step);
...
/// app.
pub fn installDummy(self: *const SpectreProDocs, step: *std.Build.Step) void {
    _ = self;
