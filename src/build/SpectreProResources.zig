const SpectreProResources = @This();
...
pub fn init(b: *std.Build, cfg: *const Config, deps: *const SharedDeps) !SpectreProResources {
    ...
    terminfo: {
        ...
        const source = wf.addCopyFile(run.captureStdOut(.{}), "spectrepro.terminfo");

        if (cfg.emit_terminfo) {
            const source_install = b.addInstallFile(
                source,
                if (os_tag == .freebsd)
                    "share/site-terminfo/spectrepro.terminfo"
                else
                    "share/terminfo/spectrepro.terminfo",
            );
        ...
        if (cfg.emit_termcap) {
            ...
            const cap_install = b.addInstallFile(
                out_source,
                if (os_tag == .freebsd)
                    "share/site-terminfo/spectrepro.termcap"
                else
                    "share/terminfo/spectrepro.termcap",
            );
    ...
    // Shell-integration
    {
        const install_step = b.addInstallDirectory(.{
            ...
            .install_subdir = b.pathJoin(&.{ "spectrepro", "shell-integration" }),
    ...
    if (cfg.emit_themes) {
        if (b.lazyDependency("iterm2_themes", .{})) |upstream| {
            const install_step = b.addInstallDirectory(.{
                ...
                .install_dir = .{ .custom = "share" },
                .install_subdir = b.pathJoin(&.{ "spectrepro", "themes" }),
                .exclude_extensions = &.{".md"},
            });
            try steps.append(b.allocator, &install_step.step);
        }
    ...
    // Fish shell completions
    {
        ...
        run.addArg("+fish");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(.{}), "spectrepro.fish");
    ...
    // zsh shell completions
    {
        ...
        run.addArg("+zsh");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(.{}), "_spectrepro");
    ...
    // bash shell completions
    {
        ...
        run.addArg("+bash");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(.{}), "spectrepro.bash");
    ...
    // Vim and Neovim plugin
    {
...

        {
            ...
            _ = wf.addCopyFile(run.captureStdOut(.{}), "syntax/spectrepro.vim");
        }
        {
            const run = b.addRunArtifact(build_data_exe);
            run.addArg("+vim-ftdetect");
            _ = wf.addCopyFile(run.captureStdOut(.{}), "ftdetect/spectrepro.vim");
        }
        {
            const run = b.addRunArtifact(build_data_exe);
            run.addArg("+vim-ftplugin");
            _ = wf.addCopyFile(run.captureStdOut(.{}), "ftplugin/spectrepro.vim");
        }
        {
            const run = b.addRunArtifact(build_data_exe);
            run.addArg("+vim-compiler");
            _ = wf.addCopyFile(run.captureStdOut(.{}), "compiler/spectrepro.vim");
        }
    ...
    // 'spectrepro.sublime-syntax' file from zig-out to the '~.config/bat/syntaxes'
    // directory. The syntax then needs to be mapped to the correct language in
    // the config file within the '~.config/bat' directory
    // (ex: --map-syntax "/Users/user/.config/spectrepro/config.spectrepro:SpectrePro Config").
    {
        const run = b.addRunArtifact(build_data_exe);
        run.addArg("+sublime");
        const wf = b.addWriteFiles();
        _ = wf.addCopyFile(run.captureStdOut(.{}), "spectrepro.sublime-syntax");
...
/// Add the resource files needed to make SpectrePro a proper
...
fn addLinuxAppResources(
    ...
    const app_id = b.fmt("co.skyones.spectrepro{s}", .{
        switch (cfg.optimize) {
            .Debug, .ReleaseSafe => "-debug",
            .ReleaseFast, .ReleaseSmall => "",
        },
    ...
    const templates: []const Template = templates: {
        ...
        // within app stores
        try ts.append(b.allocator, .{
            b.path("dist/linux/co.skyones.spectrepro.metainfo.xml.in"),
    ...
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("dist/linux/spectrepro_dolphin.desktop"),
        "share/kio/servicemenus/co.skyones.spectrepro.desktop",
    ).step);

    // Right click menu action for Nautilus. Note that this _must_ be named
    // `spectrepro.py`. Using the full app id causes problems (see #5468).
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("dist/linux/spectrepro_nautilus.py"),
        "share/nautilus-python/extensions/spectrepro.py",
    ).step);
    ...
    try steps.append(b.allocator, &b.addInstallFile(
        ...
        "share/icons/hicolor/16x16/apps/co.skyones.spectrepro.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/32.png"),
        "share/icons/hicolor/32x32/apps/co.skyones.spectrepro.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/128.png"),
        "share/icons/hicolor/128x128/apps/co.skyones.spectrepro.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/256.png"),
        "share/icons/hicolor/256x256/apps/co.skyones.spectrepro.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/512.png"),
        "share/icons/hicolor/512x512/apps/co.skyones.spectrepro.png",
    ).step);
    // Flatpaks only support icons up to 512x512.
    if (!cfg.flatpak) {
        try steps.append(b.allocator, &b.addInstallFile(
            b.path("images/gnome/1024.png"),
            "share/icons/hicolor/1024x1024/apps/co.skyones.spectrepro.png",
        ).step);
    }

    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/32.png"),
        "share/icons/hicolor/16x16@2/apps/co.skyones.spectrepro.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/64.png"),
        "share/icons/hicolor/32x32@2/apps/co.skyones.spectrepro.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/256.png"),
        "share/icons/hicolor/128x128@2/apps/co.skyones.spectrepro.png",
    ).step);
    try steps.append(b.allocator, &b.addInstallFile(
        b.path("images/gnome/512.png"),
        "share/icons/hicolor/256x256@2/apps/co.skyones.spectrepro.png",
    ).step);
}

pub fn install(self: *const SpectreProResources) void {
    const b = self.steps[0].owner;
    self.addStepDependencies(b.getInstallStep());
}

pub fn addStepDependencies(
    self: *const SpectreProResources,
