...
/// The primary SpectrePro executable.
exe: *std.Build.Step.Compile,

/// The install step for the executable.
install_step: *std.Build.Step.InstallArtifact,

pub fn init(b: *std.Build, cfg: *const Config, deps: *const SharedDeps) !SpectrePro {
    ...
    switch (cfg.target.result.os.tag) {
        .windows => {
            ...
            exe.root_module.addWin32ResourceFile(.{
                .file = b.path("dist/windows/spectrepro.rc"),
            });
        },
...
/// Add the spectrepro exe to the install target.
...
fn checkNixShell(exe: *std.Build.Step.Compile, cfg: *const Config) !void {
    ...
    try exe.step.addError(
        "\x1b[" ++ color_map.get("yellow").? ++
            ...
            \\
            \\The resulting spectrepro binary will likely fail on launch because it is
