...
pub fn init(b: *std.Build, cfg: *const Config) !SpectreProDist {
    // The name prefix used for all paths in the archive.
    const name = if (cfg.emit_lib_vt) "libspectrepro-vt" else "spectrepro";
    ...
    if (!cfg.emit_lib_vt) {
        ...
        {
            const framedata = SpectreProFrameData.distResources(b);
    ...
    // embed the SpectrePro version in the tarball
    {
    ...
    // are not needed to build libspectrepro-vt. This significantly reduces
    ...
    // This is the root SpectrePro source dir of the extracted source tarball.
...
/// test libspectrepro-vt, specified as git pathspec exclude patterns.
...
/// of the source tarball with SpectrePro. These aren't committed to the Git
