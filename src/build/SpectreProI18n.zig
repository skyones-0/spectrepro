...
const domain = "co.skyones.spectrepro";
...
pub fn init(b: *std.Build, cfg: *const Config) !SpectreProI18n {
    _ = cfg;
...
pub fn install(self: *const SpectreProI18n) void {
    self.addStepDependencies(self.owner.getInstallStep());
}

pub fn addStepDependencies(
    self: *const SpectreProI18n,
...
fn createUpdateStep(b: *std.Build) !*std.Build.Step {
    ...
    inline for (gresource.blueprints) |blp| {
        ...
        // a lot of churn because not everyone has the SpectrePro code checked out in
...

    {
        ...
        for (gtk_files.items) |item| {
            ...
            // cause a lot of churn because not everyone has the SpectrePro code
    ...
    const nautilus_script_path = "dist/linux/spectrepro_nautilus.py";
    ...
    const xgettext_merge = b.addSystemCommand(&.{
        ...
        "--copyright-holder=\"Mitchell Hashimoto, SpectrePro contributors\"",
        "-o",
        "-",
    });
