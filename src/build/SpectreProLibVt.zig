...
const SpectreProZig = @import("SpectreProZig.zig");
...
pub fn initWasm(
    ...
    const output = patch_run.addOutputFileArg("spectrepro-vt.wasm");
...
pub const AppleLibs = std.EnumMap(ApplePlatform, SpectreProLibVt);
...
fn initLib(
    ...
    const lib = b.addLibrary(.{
        .name = if (kind == .static) "spectrepro-vt-static" else "spectrepro-vt",
        .linkage = linkage,
        .root_module = zig.vt_c,
        .version = zig.version,
    });
    lib.installHeadersDirectory(
        b.path("include/spectrepro"),
        "spectrepro",
    ...
    const dsymutil: ?std.Build.LazyPath = dsymutil: {
        ...
        dsymutil.addArgs(&.{"-o"});
        const output = dsymutil.addOutputFileArg("libspectrepro-vt.dSYM");
    ...
    // it doesn't change -lspectrepro-vt into an archive-only reference when
...
fn initLibApple(
    ...
    // native link, so explicitly expose only libspectrepro-vt's public C ABI.
    const exports = b.addWriteFiles().add(
        "libspectrepro-vt.exports",
        "_spectrepro_*\n",
    );
    ...
    const real_name = b.fmt("libspectrepro-vt.{d}.{d}.{d}.dylib", .{
        zig.version.major,
        zig.version.minor,
        zig.version.patch,
    });
    const soname = b.fmt("libspectrepro-vt.{d}.dylib", .{zig.version.major});

    const native_link = try @import("apple_sdk").nativeLink.addCommand(
        b,
        "link libspectrepro-vt with Apple ld",
        target,
    );
    ...
    // Restrict exports to libspectrepro-vt's public C ABI.
    ...
    const unversioned_install = b.addSystemCommand(&.{
        ...
        "-sf",
        soname,
        b.getInstallPath(.lib, "libspectrepro-vt.dylib"),
    });
    ...
    dsymutil.addArgs(&.{"-o"});
    const dsym = dsymutil.addOutputFileArg("libspectrepro-vt.dSYM");
...
fn pkgConfigFiles(
    ...
    return .{
        .shared = wf.add("libspectrepro-vt.pc", b.fmt(
            \\prefix={s}
            \\includedir=${{prefix}}/include
            \\libdir=${{prefix}}/lib
            \\
            \\Name: libspectrepro-vt
            \\URL: https://github.com/spectrepro-org/spectrepro
            \\Description: SpectrePro VT library
            \\Version: {f}
            \\Cflags: -I${{includedir}}
            \\Libs: -L${{libdir}} -lspectrepro-vt
            \\Libs.private: {s}
            \\Requires.private: {s}
        , .{ b.install_prefix, zig.version, libs_private, requires_private })),
        .static = wf.add("libspectrepro-vt-static.pc", b.fmt(
            \\prefix={s}
            \\includedir=${{prefix}}/include
            \\libdir=${{prefix}}/lib
            \\
            \\Name: libspectrepro-vt-static
            \\URL: https://github.com/spectrepro-org/spectrepro
            \\Description: SpectrePro VT library (static)
            \\Version: {f}
            \\Cflags: -I${{includedir}}
            \\Libs: ${{libdir}}/{s}
            \\Libs.private: {s}
...
pub fn xcframework(
    ...
    // for SpectreProKit (the macOS app library).
    const wf = b.addWriteFiles();
    ...
    _ = wf.add("module.modulemap",
        \\module SpectreProVt {
        \\    umbrella header "spectrepro/vt.h"
        \\    export *
        \\}
        \\
    );
    ...
    return XCFrameworkStep.create(b, .{
        .name = "spectrepro-vt",
        .out_path = b.pathJoin(&.{ b.install_prefix, "lib/spectrepro-vt.xcframework" }),
...
pub fn install(
    ...
    if (self.pkg_config) |pkg_config| {
        step.dependOn(&b.addInstallFileWithDir(
            pkg_config,
            .prefix,
            "share/pkgconfig/libspectrepro-vt.pc",
        ).step);
    }
    if (self.pkg_config_static) |pkg_config_static| {
        step.dependOn(&b.addInstallFileWithDir(
            pkg_config_static,
            .prefix,
            "share/pkgconfig/libspectrepro-vt-static.pc",
        ).step);
    }
}
