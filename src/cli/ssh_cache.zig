...
///
/// The `+ssh` action installs SpectrePro's terminfo on remote hosts and records
...
/// Examples:
///
///     spectrepro +ssh-cache                           # List all cached destinations
///     spectrepro +ssh-cache user@example.com          # Show that destination
///     spectrepro +ssh-cache example.com               # Show all users on that host
///     spectrepro +ssh-cache --add=user@example.com    # Manually add a destination
///     spectrepro +ssh-cache --remove=user@example.com # Remove a destination
///     spectrepro +ssh-cache --prune=30d               # Remove entries older than 30 days
///     spectrepro +ssh-cache --clear                   # Clear entire cache
pub fn run(alloc_gpa: Allocator) !u8 {
            ...
            if (is_host_flag) {
                try stderr.print(
                    "Warning: --host is deprecated; pass the destination " ++
                        "directly, e.g. `spectrepro +ssh-cache {s}`.\n",
                    .{arg["--host=".len..]},
                );
            }
    ...
    const cache_path = DiskCache.defaultPath(alloc, "spectrepro") catch |err| {
        try stderr.print(
