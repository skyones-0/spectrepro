...
/// SpectrePro. This uses the exact same font discovery mechanism SpectrePro uses to
/// find fonts to use.
///
...
/// same priority order SpectrePro would use to pick a font.
///
/// Flags:
///
...
///     is identical to the `font-family` set of SpectrePro configuration values, so
...
fn runArgs(alloc_gpa: Allocator, argsIter: anytype) !u8 {
    ...
    // Its possible to build SpectrePro without font discovery!
    if (comptime font.Discover == void) {
        var buffer: [1024]u8 = undefined;
        var stderr_writer = std.Io.File.stderr().writer(global.io(), &buffer);
        const stderr = &stderr_writer.interface;
        try stderr.print(
            \\SpectrePro was built without a font discovery mechanism. This is a compile-time
            \\option. Please review how SpectrePro was built from source, contact the
