...
pub const Options = struct {
    ...
    /// If set, open up a new window in a custom instance of SpectrePro.
...
/// running instance of SpectrePro.
///
/// If the `--class` flag is not set, the `new-window` command will try and
/// connect to a running instance of SpectrePro based on what optimizations the
/// SpectrePro CLI was compiled with. Otherwise the `new-window` command will try
/// and contact a running SpectrePro instance that was configured with the same
/// `class` as was given on the command line.
///
/// All of the arguments after the `+new-window` argument (except for the
/// `--class` flag) will be sent to the remote SpectrePro instance and will be
...
/// relative to the current working directory that the `spectrepro +new-window`
...
/// directory that `spectrepro +new-window` is run from will be passed to SpectrePro.
///
