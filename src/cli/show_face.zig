...
/// The `show-face` command shows what font face SpectrePro will use to render a
...
/// SpectrePro uses to render a codepoint in a terminal session.
///
/// Flags:
///
...
fn lookup(
    ...
    const face = font_grid.resolver.collection.getFace(idx) catch |err| switch (err) {
        error.SpecialHasNoFace => {
            try stdout.print("U+{0X:0>2} « {0u} » is handled by SpectrePro's internal sprites.\n", .{cp});
            return null;
        },
        else => {
