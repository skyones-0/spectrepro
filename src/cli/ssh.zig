...
const usage =
    \\Usage: spectrepro +ssh [flags] [--] <ssh args...>
    \\
    \\Flags:
    \\  --forward-env[=bool]  Enable TERM / SendEnv forwarding. Default: true.
    \\  --terminfo[=bool]     Install SpectrePro terminfo on first connect. Default: true.
...
/// Wrap `ssh` to automatically configure SpectrePro terminal integration on
/// remote hosts.
///
...
/// nothing to collide.
///
/// This is typically called via SpectrePro's shell integration. When
/// `shell-integration-features` includes `ssh-env` or `ssh-terminfo`,
/// each shell defines an `ssh` function that runs:
///
///     spectrepro +ssh <flags> -- "$@"
///
/// You can also run `spectrepro +ssh` directly, or alias it yourself (e.g.
/// `alias ssh='spectrepro +ssh --'`) if you prefer not to use the shell
/// integration.
///
...
///      remote shell can still detect that it's running inside SpectrePro.
...
///      given destination, installs SpectrePro's embedded terminfo entry on the
///      remote host using `ssh tic -x -` over a shared `ControlMaster`
///      connection. Successful installs are cached
///      (see `spectrepro +ssh-cache`) so subsequent connections skip this
///      step. When terminfo is successfully installed or already cached,
///      `TERM` is set to `xterm-spectrepro` instead of `xterm-256color`.
///
...
///     host while keeping the cache in use, prefer `spectrepro +ssh-cache
...
///     spectrepro +ssh user@example.com
///
///     # Forward SpectrePro env vars but skip the terminfo install:
///     spectrepro +ssh --terminfo=false user@example.com
///
///     # `ssh` flags (short-form `-p`, etc.) pass through unchanged:
///     spectrepro +ssh -p 2222 -i ~/.ssh/id_ed25519 user@example.com
///
///     # Use `--` explicitly if your ssh args might collide with our flags:
///     spectrepro +ssh -- --some-rare-ssh-arg user@example.com
///
/// Pass `--verbose` to see what `+ssh` is doing. For cache inspection
/// and management, see `spectrepro +ssh-cache`.
///
...
fn runInner(
    ...
    const session: struct {
        ...
        const cache: ?DiskCache = if (opts.cache) cache: {
            const path = DiskCache.defaultPath(alloc, "spectrepro") catch |err| {
                warnPrint(stderr, "spectrepro terminfo cache unavailable: {t}", .{err});
                break :session .{ .term = "xterm-256color" };
            };
        ...
        if (cache) |c| {
            ...
            if (cached) {
                ...
                break :session .{ .term = "xterm-spectrepro" };
            } else {
        ...
        stderr.print("Setting up xterm-spectrepro terminfo on {s}...\n", .{dest}) catch {};
        stderr.flush() catch {};
...
/// Install SpectrePro's terminfo on the remote host over a short-lived SSH
...
fn installRemoteTerminfo(
    ...
    defer buf.deinit();
    try terminfopkg.spectrepro.encode(&buf.writer);
    ...
    // short leaves margin.
    const control_path = try internal_os.randomTmpPath(alloc, "spectrepro-ssh-");
