...
pub const Options = struct {
    ...
    ///
    ///   spectrepro +explain-config --option=font-size
    option: ?[]const u8 = null,

    /// The keybind action to explain. For example:
    ///
    ///   spectrepro +explain-config --keybind=copy_to_clipboard
...
/// SpectrePro configuration option or keybind action.
///
/// Examples:
///
///   spectrepro +explain-config font-size
///   spectrepro +explain-config copy_to_clipboard
///   spectrepro +explain-config --option=font-size
///   spectrepro +explain-config --keybind=copy_to_clipboard
///
/// Flags:
...
pub fn run(alloc: Allocator) !u8 {
    ...
    const name = keybind_name orelse option_name orelse positional orelse {
        ...
        try stderr_writer.interface.writeAll("Usage: spectrepro +explain-config <option>\n");
        try stderr_writer.interface.writeAll("       spectrepro +explain-config --option=<option>\n");
        try stderr_writer.interface.writeAll("       spectrepro +explain-config --keybind=<action>\n");
