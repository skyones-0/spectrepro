...
fn initPager() ?std.process.Child {
    ...
    // Resolve the pager command: $SPECTREPRO_PAGER > $PAGER > `less`.
    // An empty value for either env var disables paging.
    const spectrepro_var = env.get("SPECTREPRO_PAGER");
    const pager_var = env.get("PAGER");
    const cmd: ?[]const u8 = cmd: {
        if (spectrepro_var) |v| break :cmd if (v.len > 0) v else null;
