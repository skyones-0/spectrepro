...
/// The `help` command shows general help about SpectrePro. Recognized as either
...
pub fn run(alloc: Allocator) !u8 {
    ...
    try stdout.writeAll(
        \\Usage: spectrepro [+action] [options]
        \\
        \\Run the SpectrePro terminal emulator or a specific helper action.
        \\
        \\If no `+action` is specified, run the SpectrePro terminal emulator.
        ...
        \\`spectrepro -e top` will run the `top` command inside the terminal.
        \\
        \\On macOS, launching the terminal emulator from the CLI is not
        \\supported and only actions are supported. Use `open -na SpectrePro.app`
        \\instead, or `open -na spectrepro.app --args --foo=bar --baz=quz` to pass
        \\arguments.
        \\
        \\Available actions:
        \\
