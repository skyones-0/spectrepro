...
pub const Action = enum {
    ...
    /// Wrap `ssh` to configure SpectrePro terminal integration on remote hosts
    ssh,
    ...
    // Show which font face SpectrePro loads a codepoint from.
    @"show-face",
    ...
    // Use IPC to tell the running SpectrePro to open a new window.
    @"new-window",

    // Use IPC to tell the running SpectrePro to open a new tab.
    @"new-tab",
    ...
    pub fn detectSpecialCase(arg: []const u8) ?SpecialCase(Action) {
        ...
        // `spectrepro -e spectrepro +command`. If we've seen a command we
        // still want to keep looking because
        // `spectrepro +command -e +command` is invalid.
