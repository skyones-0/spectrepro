...
const Action = @import("../cli.zig").spectrepro.Action;
...
pub const Options = struct {
    ...
    /// If set, open up a new tab in a custom instance of SpectrePro.
...
/// instance of SpectrePro.
///
/// If the `--class` flag is not set, the `new-tab` command will try and connect
/// to a running instance of SpectrePro based on what optimizations the SpectrePro
/// CLI was compiled with. Otherwise the `new-tab` command will try and contact
/// a running SpectrePro instance that was configured with the same `class` as was
/// given on the command line.
///
/// All of the arguments after the `+new-tab` argument (except for the `--class`
/// and `--surface-id` flag) will be sent to the remote SpectrePro instance and
...
/// path relative to the current working directory that the `spectrepro +new-tab`
...
/// directory that `spectrepro +new-tab` is run from will be passed to SpectrePro.
///
/// GTK uses an application ID to identify instances of applications. If SpectrePro
/// is compiled with release optimizations, the default application ID will be
/// `co.skyones.spectrepro`. If SpectrePro is compiled with debug optimizations,
/// the default application ID will be `co.skyones.spectrepro-debug`.  The
...
/// or it will be ignored and SpectrePro will use the default as defined above.
///
/// On GTK, D-Bus activation must be properly configured. SpectrePro does not need
...
/// of SpectrePro if it is not already running. See the SpectrePro website for
...
///     SpectrePro. The class must be a valid GTK application ID.
///
///   * `--surface-id=<surface id>`: If set, specifies a SpectrePro surface ID.
...
///     supported values.
///
///   * `--working-directory=<directory>`: The working directory to pass to SpectrePro.
///
...
fn runArgs(
    ...
    const surface_id = opts.@"surface-id" orelse surface: {
        ...
        defer env.deinit();
        const e = env.get("SPECTREPRO_SURFACE_ID") orelse break :surface 0;
