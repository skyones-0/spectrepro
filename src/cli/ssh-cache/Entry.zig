...
pub fn parse(line: []const u8) ?Entry {
    ...
    const terminfo_version = iter.next() orelse "xterm-spectrepro";
...
test "cache entry parsing valid formats" {
    ...
    const entry = Entry.parse("example.com|1640995200|xterm-spectrepro").?;
    try testing.expectEqualStrings("example.com", entry.hostname);
    try testing.expectEqual(@as(i64, 1640995200), entry.timestamp);
    try testing.expectEqualStrings("xterm-spectrepro", entry.terminfo_version);
    ...
    const complex_entry = Entry.parse("user@server.example.com|1640995200|xterm-spectrepro").?;
    try testing.expectEqualStrings(
...
test "cache entry parsing malformed data resilience" {
    ...
    try testing.expect(
        Entry.parse("host|999999999999999999999999999999999999999999999999|xterm-spectrepro") == null,
    );
}
