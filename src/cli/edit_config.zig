...
/// The `edit-config` command opens the SpectrePro configuration file in the
...
/// application menu, configured keybind, or by restarting SpectrePro. We
/// plan to auto-reload in the future, but SpectrePro isn't capable of
/// this yet.
///
...
fn runInner(alloc: Allocator, stderr: *std.Io.Writer) !u8 {
    ...
    // limitation later but SpectrePro already heavily requires libc
    ...
    if (comptime builtin.os.tag == .windows) {
        try stderr.print(
            \\The `spectrepro +edit-config` command is not supported on Windows.
        ...
        switch (err) {
            error.NoEditorConfigured => {
                try stderr.print(
                    ...
                    \\This environment variable is required to edit the SpectrePro configuration
