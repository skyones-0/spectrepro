// This is the main file for the C API. The C API is used to embed SpectrePro
...
// This is currently used only to embed spectrepro within a macOS app. However,
...
const main = @import("main_spectrepro.zig");
...
comptime {
    ...
    // functions unless we are building libspectrepro.
    if (!builtin.is_test) {
...
comptime {
    ...
    // Any apprt-specific C API, mainly libspectrepro for apprt.embedded.
...
/// Initialize spectrepro global state.
pub export fn spectrepro_init(argc: usize, argv: [*][*:0]u8) c_int {
    ...
    global.init(.{
        ...
        std.log.err("failed to initialize spectrepro error={}", .{err});
        return 1;
    };
...
pub export fn spectrepro_cli_try_action() void {
...
/// Return metadata about SpectrePro, such as version, build mode, etc.
pub export fn spectrepro_info() Info {
    return .{
...
/// Translate a string maintained by libspectrepro into the current
...
/// the function call.
///
/// This should only be used for singular strings maintained by SpectrePro.
pub export fn spectrepro_translate(msgid: [*:0]const u8) [*:0]const u8 {
    return internal_os.i18n._(msgid);
}

/// Free a string allocated by SpectrePro.
pub export fn spectrepro_string_free(str: String) void {
    str.deinit();
}
...
test "spectrepro_string_s empty string" {
...
test "spectrepro_string_s c string" {
    const testing = std.testing;
...
test "spectrepro_string_s zig string" {
    const testing = std.testing;
