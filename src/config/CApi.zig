...
export fn spectrepro_config_new() ?*Config {
...
export fn spectrepro_config_free(ptr: ?*Config) void {
    if (ptr) |v| {
        v.deinit();
...
export fn spectrepro_config_clone(self: *Config) ?*Config {
    const result = global.alloc().create(Config) catch |err| {
        log.err("error allocating config err={}", .{err});
        return null;
    };
...
export fn spectrepro_config_load_cli_args(self: *Config) void {
...
export fn spectrepro_config_load_default_files(self: *Config) void {
...
export fn spectrepro_config_load_file(self: *Config, path: [*:0]const u8) void {
...
export fn spectrepro_config_load_recursive_files(self: *Config) void {
    self.loadRecursiveFiles(global.alloc()) catch |err| {
        log.err("error loading config err={}", .{err});
    };
}

export fn spectrepro_config_finalize(self: *Config) void {
    self.finalize() catch |err| {
        log.err("error finalizing config err={}", .{err});
    };
}

export fn spectrepro_config_get(
    self: *Config,
    ptr: *anyopaque,
    key_str: [*]const u8,
    len: usize,
...
export fn spectrepro_config_trigger(
    self: *Config,
    str: [*]const u8,
    len: usize,
...
export fn spectrepro_config_diagnostics_count(self: *Config) u32 {
    return @intCast(self._diagnostics.items().len);
}

export fn spectrepro_config_get_diagnostic(self: *Config, idx: u32) Diagnostic {
...
export fn spectrepro_config_open_path() String {
...
/// Sync with spectrepro_diagnostic_s
const Diagnostic = extern struct {
    message: [*:0]const u8 = "",
};

test "spectrepro_config_get: bool" {
    ...
    var out = false;
    const key = "maximize";
    try testing.expect(spectrepro_config_get(&cfg, &out, key, key.len));
    try testing.expect(out);
}

test "spectrepro_config_get: enum" {
    const testing = std.testing;
    const alloc = testing.allocator;
    ...
    try testing.expect(spectrepro_config_get(&cfg, @ptrCast(&out), key, key.len));
    const str = std.mem.sliceTo(out, 0);
    try testing.expectEqualStrings("dark", str);
}

test "spectrepro_config_get: optional null returns false" {
    const testing = std.testing;
    const alloc = testing.allocator;
...
test "spectrepro_config_get: unknown key returns false" {
    const testing = std.testing;
    const alloc = testing.allocator;
...
test "spectrepro_config_get: optional string null returns true" {
    const testing = std.testing;
    const alloc = testing.allocator;
...
test "spectrepro_config_get: float" {
    const testing = std.testing;
    const alloc = testing.allocator;
...
test "spectrepro_config_get: struct cval conversion" {
    const testing = std.testing;
    const alloc = testing.allocator;
...
test "spectrepro_config_trigger: default keybind" {
    const testing = std.testing;
