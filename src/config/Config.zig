...
// like the `spectrepro-build-data` binary depend on the Config but don't
...
pub const compatibility = std.StaticStringMap(
    ...
    // SpectrePro 1.1 introduced background-blur support for Linux which
    ...
    // SpectrePro 1.2 renamed all our adw options to gtk because we now have
    // a hard dependency on libadwaita.
    .{ "adw-toolbar-style", cli.compatibilityRenamed(Config, "gtk-toolbar-style") },

    // SpectrePro 1.2 removed the `hidden` value from `gtk-tabs-location` and
    // moved it to `window-show-tab-bar`.
    .{ "gtk-tabs-location", compatGtkTabsLocation },

    // SpectrePro 1.2 lets you set `cell-foreground` and `cell-background`
    ...
    // SpectrePro 1.2 merged `bold-is-bright` into the new `bold-color`
    // by setting the value to "bright".
    .{ "bold-is-bright", compatBoldIsBright },

    // SpectrePro 1.2 removed the "desktop" option and renamed it to "detect".
    // The semantics also changed slightly but this is the correct mapping.
    .{ "gtk-single-instance", compatGtkSingleInstance },

    // SpectrePro 1.3 rename the "window" option to "new-window".
    // See: https://github.com/spectrepro-org/spectrepro/pull/9764
    .{ "macos-dock-drop-behavior", compatMacOSDockDropBehavior },

    // SpectrePro 1.4 renamed `scrollback-limit` to `scrollback-limit-bytes`
    ...
    // SpectrePro 1.4 updated "copy-on-select", allow copying to the selection
    ...
    // See: https://github.com/spectrepro-org/spectrepro/pull/12604
    .{ "copy-on-select", compatCopyOnSelect },
});

/// Set SpectrePro's graphical user interface language to a language other than the
/// system default language. For example:
///
///     language = de
///
/// will force the strings in SpectrePro's graphical user interface to be in German
/// rather than the system default.
///
/// This will not affect the language used by programs run _within_ SpectrePro.
/// Those will continue to use the default system language. There are also many
/// non-GUI elements in SpectrePro that are not translated - this setting will have
/// no effect on those.
///
/// Warning: This setting cannot be reloaded at runtime. To change the language
/// you must fully restart SpectrePro.
///
/// GTK only.
...
/// etc.
///
/// Notes on emoji specifically: On macOS, SpectrePro by default will always use
...
/// found, SpectrePro will use the regular style. This prevents falling back to a
...
/// set `font-family-bold = FooBar` and "FooBar" cannot be found, SpectrePro will
/// use whatever font is set for `font-family` for the bold style.
///
/// Finally, some styles may be synthesized if they are not supported.
/// For example, if a font does not have an italic style and no alternative
/// italic font is specified, SpectrePro will synthesize an italic style by
...
/// Control whether SpectrePro should synthesize a style if the requested style is
/// not available in the specified font-family.
///
/// SpectrePro can synthesize bold, italic, and bold italic styles if the font
...
///
/// The font feature will apply to all fonts rendered by SpectrePro. A future
...
///
/// SpectrePro breaks runs at certain points to improve readability or usability.
/// For example, SpectrePro by default will break runs under the cursor so that
...
///
/// This configuration only applies to SpectrePro builds that use FreeType.
...
///     This is the most common setting in GTK apps and therefore also SpectrePro's
...
/// name, or an absolute path to a custom theme file. SpectrePro also supports
/// specifying a different theme to use for light and dark mode. Each
/// option is documented below.
///
/// If the theme is an absolute pathname, SpectrePro will attempt to load that
...
///
/// The first directory is the `themes` subdirectory of your SpectrePro
/// configuration directory. This is `$XDG_CONFIG_HOME/spectrepro/themes` or
/// `~/.config/spectrepro/themes`.
///
/// The second directory is the `themes` subdirectory of the SpectrePro resources
/// directory. SpectrePro ships with a multitude of themes that will be installed
/// into this directory. On macOS, this list is in the
/// `SpectrePro.app/Contents/Resources/spectrepro/themes` directory. On Linux, this
/// list is in the `share/spectrepro/themes` directory (wherever you installed the
/// SpectrePro "share" directory.
///
/// To see a list of available themes, run `spectrepro +list-themes`.
///
/// A theme file is simply another SpectrePro configuration file. They share
...
/// `config-file`. At the time of writing this, SpectrePro will not show any
...
/// splits. A future improvement to SpectrePro will address this.
///
...
/// memory usage (specifically VRAM usage). A future SpectrePro improvement
...
///
/// If this is not set at all (`null`), then SpectrePro will respect DEC Mode 12
...
/// additional configuration or SpectrePro's shell integration features to be
/// enabled.
///
...
///   anything but modifiers or keybinds that are processed by SpectrePro).
///
...
/// to all scrolling devices. Specifying a prefix was introduced in SpectrePro
/// 1.2.1.
///
...
///
/// On macOS, changing this configuration requires restarting SpectrePro completely.
...
/// or `true` value will enable background blur. On Wayland, SpectrePro uses
...
/// Control when SpectrePro preserves a zoomed split. Under normal circumstances,
...
/// created when SpectrePro starts, use the `initial-command` configuration.
///
/// SpectrePro supports the common `-e` flag for executing a command with
/// arguments. For example, `spectrepro -e fish --with --custom --args`.
/// This flag sets the `initial-command` configuration, see that for more
/// information.
command: ?Command = null,

/// This is the same as "command", but only applies to the first terminal
/// surface created when SpectrePro starts. Subsequent terminal surfaces will use
...
///
/// If you're using the `spectrepro` CLI there is also a shortcut to set this
/// with arguments directly: you can use the `-e` flag. For example: `spectrepro -e
...
///     the `spectrepro -e` command).
///
///   * `gtk-single-instance=false` - This ensures that a new instance is
///     launched and the CLI args are respected.
///
///   * `quit-after-last-window-closed=true` - This ensures that the SpectrePro
...
/// variables set by SpectrePro. For example, if you set `SPECTREPRO_RESOURCES_DIR`
/// then the value you set here will override the value SpectrePro typically
/// automatically injects.
///
/// These environment variables _will not_ be passed to commands run by SpectrePro
...
/// memory. On supported systems with scrollback compression enabled, SpectrePro
...
/// pages. Compression does not allow SpectrePro to retain more history than the
...
///
/// This limit is an estimate. Internally, SpectrePro will only trim lines
...
///
/// SpectrePro does its best to only compress when idle and decompress
...
/// content-dependent.
///
/// Note that the way SpectrePro works is that we compress and discard the
...
/// The title SpectrePro will use for the window. This will force the title of the
/// window to be this title at all times and SpectrePro will ignore any set title
...
/// bus name that SpectrePro uses to connect to DBus.
///
/// Note that changing this value between invocations will create new, separate
/// instances, of SpectrePro when running with `gtk-single-instance=true`. See that
/// option for more details.
///
/// Changing this value may break launching SpectrePro from `.desktop` files, via
/// DBus activation, or systemd user services as the system is expecting SpectrePro
/// to connect to DBus using the default `class` when it is launched.
///
/// The class name must follow the requirements defined [in the GTK
/// documentation](https://docs.gtk.org/gio/type_func.Application.id_is_valid.html).
///
/// The default is `co.skyones.spectrepro`.
///
...
/// setting. If a previous SpectrePro terminal exists in the same process,
...
/// if SpectrePro can detect it is launched from launchd (double-clicked) or
/// `open`, then it defaults to `home`. On Linux with GTK, if SpectrePro can detect
...
/// the documentation or using the `spectrepro +list-actions` command.
///
...
/// otherwise bound. When looking up a binding, SpectrePro first tries to match
...
///
/// Warning: If you define a sequence as a CLI argument to `spectrepro`,
/// you probably have to quote the keybind since `>` is a special character
/// in most shells. Example: spectrepro --keybind='ctrl+a>n=new_window'
///
/// A trigger sequence has some special handling:
///
///   * SpectrePro will wait an indefinite amount of time for the next key in
...
///     `spectrepro +list-actions` command.
///
...
///  * `global:`
///
///    Make the keybind global. By default, keybinds only work within SpectrePro
...
///    (e.g. even when SpectrePro is not focused), specify this prefix.
...
///    if there is a selection to copy. If there is no selection, SpectrePro
...
/// to SpectrePro. When a `global:` keybind is specified and SpectrePro is launched
/// or reloaded, SpectrePro will attempt to request these permissions.
...
/// -> Accessibility.
///
/// Since SpectrePro 1.4.0, global shortcuts on Linux are primarily supported
...
/// sequence.
///
/// Chained keybinds are available since SpectrePro 1.3.0.
///
/// ## Key Tables
///
...
///
/// Key tables are available since SpectrePro 1.3.0.
keybind: Keybinds = .{},

/// Remap modifier keys within SpectrePro. This allows you to swap or reassign
...
///
/// This will NOT change keyboard layout or key encodings outside of SpectrePro.
...
/// before SpectrePro receives the key events.
///
/// Example:
///
...
///   prior to SpectrePro receiving them. For example, `option+a` on macOS
...
///   this happens before SpectrePro receives the key event. To workaround
...
/// may override this preference. SpectrePro will do its best to respect this
...
///    desktop environment. This option usually makes SpectrePro look the most
...
///   * `spectrepro` - Use the background and foreground colors specified in the
///     SpectrePro configuration. This is only supported on Linux builds.
///
/// On macOS, if `macos-titlebar-style` is `tabs` or `transparent`, the window theme will be
/// automatically set based on the luminosity of the terminal background color.
/// This only applies to terminal windows. This setting will still apply to
/// non-terminal windows within SpectrePro.
///
...
/// certain size to fit within the grid. There is nothing SpectrePro will do about
...
/// to be a certain position to fit within the grid. There is nothing SpectrePro
...
///
///   * `always` will always save window state whenever SpectrePro is exited.
///
/// If you change this value to `never` while SpectrePro is not running, the next
/// SpectrePro launch will NOT restore the window state.
///
/// If you change this value to `default` while SpectrePro is not running and the
/// previous exit saved state, the next SpectrePro launch will still restore the
/// window state. This is because SpectrePro cannot know if the previous exit was
/// due to a forced save or not (macOS doesn't provide this information).
///
/// If you change this value so that window state is saved while SpectrePro is not
/// running, the previous window state will not be restored because SpectrePro only
...
/// window-theme is set to spectrepro. Currently only supported in the GTK app
/// runtime.
///
/// Specified as either hex (`#RRGGBB` or `RRGGBB`) or a named X11 color.
...
/// of an escape sequence hardcoded into SpectrePro for now.
///
...
/// this limit bounds how much memory a program can make SpectrePro
...
///
/// On Linux, if this is `true`, SpectrePro can delay quitting fully until a
/// configurable amount of time has passed after the last window is closed.
/// See the documentation of `quit-after-last-window-closed-delay`.
@"quit-after-last-window-closed": bool = builtin.os.tag == .linux or builtin.os.tag == .macos,

/// Controls how long SpectrePro will stay running after the last open surface has
...
/// SpectrePro will quit immediately after the last window is closed if
/// `quit-after-last-window-closed` is `true`.
///
/// Only implemented on Linux.
@"quit-after-last-window-closed-delay": ?Duration = null,

/// This controls whether an initial window is created when SpectrePro
/// is run. Note that if `quit-after-last-window-closed` is `true` and
/// `quit-after-last-window-closed-delay` is set, setting `initial-window` to
/// `false` will mean that SpectrePro will quit after the configured delay if no
...
///
/// On macOS, changing this configuration requires restarting SpectrePro
/// completely.
///
...
///
/// Available since: 1.2.0
@"gtk-quick-terminal-namespace": [:0]const u8 = "spectrepro-quick-terminal",
...
/// SpectrePro uses the compositor-reported primary output when available and
...
///     converts TERM from `xterm-spectrepro` to `xterm-256color` when connecting to
...
///     Attempts to install SpectrePro's embedded terminfo entry using `tic` on local
...
///     (Available since: 1.2.0)
///
///   * `path` - Add SpectrePro's binary directory to PATH. This ensures the `spectrepro`
...
/// when both `ssh-env` and `ssh-terminfo` are enabled, SpectrePro will install its
/// terminfo on remote hosts and use `xterm-spectrepro` as TERM, falling back to
...
/// command-palette-entry = title:"SpectrePro",description:"Add a little SpectrePro to your terminal.",action:"text:\xf0\x9f\x91\xbb"
/// ```
///
/// There are some additional special values that can be specified for
...
/// SpectrePro currently supports OSC 10 (foreground), OSC 11 (background), and
...
///
/// Warning: Invalid shaders can cause SpectrePro to become unusable such as by
...
/// largely match Shadertoy, with some caveats and SpectrePro-specific extensions.
///
...
///
/// SpectrePro-specific extensions:
///
...
///
///    Request the user's attention when SpectrePro is unfocused, until it has
...
///      bring the SpectrePro window into focus;
///
...
/// Control the in-app notifications that SpectrePro shows.
///
...
/// and not something SpectrePro enables.
///
...
/// onto the SpectrePro icon in the macOS dock.
///
/// Valid values are:
///
...
/// If true, SpectrePro on macOS will automatically enable the "Secure Input"
/// feature when it detects that a password prompt is being displayed.
///
/// "Secure Input" is a macOS security feature that prevents applications from
/// reading keyboard events. This can always be enabled manually using the
/// `SpectrePro > Secure Keyboard Entry` menu item.
///
...
/// If true, SpectrePro will show a graphical indication when secure input is
...
/// If true, SpectrePro exposes and handles the built-in AppleScript dictionary
/// on macOS.
///
...
/// Valid values:
///
///  * `official` - Use the official SpectrePro icon.
///  * `blueprint`, `chalkboard`, `microchip`, `glass`, `holographic`,
///    `paper`, `retro`, `xray` - Official variants of the SpectrePro icon
...
///  * `custom-style` - Use the official SpectrePro icon but with custom
...
///
/// Defaults to `~/.config/spectrepro/SpectrePro.icns`
...
/// Whether macOS Shortcuts are allowed to control SpectrePro.
///
/// SpectrePro exposes a number of actions that allow Shortcuts to
/// control and interact with SpectrePro. This includes creating new
...
/// Valid values are:
///
/// * `ask` - Ask the user whether for permission. SpectrePro will remember
///   this choice and never ask again. This is similar to other macOS
///   permissions such as microphone access, camera access, etc.
///
/// * `allow` - Allow Shortcuts to control SpectrePro without asking.
///
/// * `deny` - Deny Shortcuts from controlling SpectrePro.
///
/// Available since: 1.2.0
...
/// instead of the entire SpectrePro process. Similarly, if a shell program is
/// using too much CPU, only that surface will be CPU-throttled.
///
/// This will cause startup times to be slower (a hundred milliseconds or so),
/// so the default value is "single-instance." In single-instance mode, only
/// one instance of SpectrePro is running (see gtk-single-instance) so the startup
/// time is a one-time cost. Additionally, single instance SpectrePro is much
/// more likely to have many windows, tabs, etc. so cgroup isolation is a
/// big benefit.
///
/// This feature requires `systemd`. If `systemd` is unavailable, cgroup
/// initialization will fail. By default, this will not prevent SpectrePro from
...
///   * `single-instance` - Enable cgroups only for SpectrePro instances launched
...
/// If `true`, the SpectrePro GTK application will run in single-instance mode:
/// each new `spectrepro` process launched will result in a new window if there is
/// already a running process.
///
/// If `false`, each new spectrepro process will launch a separate application.
///
/// If `detect`, SpectrePro will assume true (single instance) unless one of
/// the following scenarios is found:
///
/// 1. TERM_PROGRAM environment variable is a non-empty value. In this
/// case, we assume SpectrePro is being launched from a graphical terminal
/// session and you want a dedicated instance.
///
/// 2. Any CLI arguments exist. In this case, we assume you are passing
/// custom SpectrePro configuration. Single instance mode inherits the
...
/// The default value is `detect`.
///
/// Note that debug builds of SpectrePro have a separate single-instance ID
...
/// If `true` (default), then the SpectrePro GTK tabs will be "wide." Wide tabs
...
///
/// Launch SpectrePro with `env GTK_DEBUG=interactive spectrepro` to tweak SpectrePro's
/// CSS in real time using the GTK Inspector. Errors in your CSS files would
/// also be reported in the terminal you started SpectrePro from. See
...
/// escape sequences such as OSC 4. (Since SpectrePro 1.2.0, the previous
/// configuration `bold-is-bright` is deprecated and replaced by this
/// usage).
///
/// Available since SpectrePro 1.2.0.
...
/// keyprotocol=spectrepro:kitty`, however a bug in the implementation prevents it
/// from working properly. https://github.com/vim/vim/pull/13211 fixes this.
term: []const u8 = "xterm-spectrepro",
...
///
/// If the selected backend is not available on the platform, SpectrePro will
...
/// Control the auto-update functionality of SpectrePro. This is only supported
/// on macOS currently, since Linux builds are distributed via package
/// managers that are not centrally controlled by SpectrePro.
///
...
/// underlying protocols. To put it another way: SpectrePro doesn't explicitly
...
/// running SpectrePro version. If you download a pre-release version of SpectrePro
/// then this will be set to `tip` and you will receive pre-release updates.
/// If you download a stable version of SpectrePro then this will be set to
...
/// Set to true if SpectrePro was executed as xdg-terminal-exec on Linux.
...
test "handle bom in config files" {
...

    {
        ...
        try cfg.loadReader(
            alloc,
            &reader,
            "/home/spectrepro/.config/spectrepro/config.spectrepro",
        );
        try cfg.finalize();
...
pub fn finalize(self: *Config) !void {
    ...
    if (self.theme) |*theme| {
        ...
        if (different) {
            ...
            // because it'll force the theme based on the SpectrePro theme.
    ...
    // short value because it can cause SpectrePro to quit before the first
...
/// SpectrePro in a CLI environment. We need this to change some behaviors.
...
/// If this is changed, you must also update spectrepro.h
...
pub const WorkingDirectory = union(enum) {
    ...
    test "WorkingDirectory parseCLI" {
        ...
        try wd.parseCLI(alloc, "~/projects/spectrepro");
        try testing.expectEqualStrings("~/projects/spectrepro", wd.path);
    ...
    test "WorkingDirectory finalize" {
        ...
        {
            var wd: Self = .{ .path = "~/projects/spectrepro" };
            try wd.finalize(alloc);
...
pub const Keybinds = struct {
    ...
    pub fn init(self: *Keybinds, alloc: Allocator) !void {
        ...
        }
        {
            ...
            inline while (i <= end) : (i += 1) {
                ...
                // libspectrepro trigger API returns this one for the action,
    ...
    // Regression test for https://github.com/spectrepro-org/spectrepro/issues/2734
    test "formatConfig multiple items" {
        const testing = std.testing;
        var buf: std.Io.Writer.Allocating = .init(testing.allocator);
        defer buf.deinit();
...
/// See macos-icon
///
/// Note: future versions of SpectrePro can support a custom icon with
...
pub const GtkTitlebarStyle = enum(c_int) {
    ...
    pub const getGObjectType = switch (build_config.app_runtime) {
        .gtk => @import("gobject").ext.defineEnum(
            GtkTitlebarStyle,
            .{ .name = "SpectreProGtkTitlebarStyle" },
        ),
...
pub const FreetypeLoadFlags = packed struct {
    ...
    // for Freetype itself. SpectrePro hasn't made any opinionated changes
    // to these defaults. (Strictly speaking, `light` isn't FreeType's
    // own default, but appears to be the effective default with most
    // Fontconfig-aware software using FreeType, so until SpectrePro
...
pub const WindowDecoration = enum(c_int) {
    ...
    pub const getGObjectType = switch (build_config.app_runtime) {
        .gtk => @import("gobject").ext.defineEnum(
            WindowDecoration,
            .{ .name = "SpectreProConfigWindowDecoration" },
        ),
...
test "working-directory expands tilde" {
    ...
    defer cfg.deinit();
    var it: TestIterator = .{ .data = &.{
        "--working-directory=~/projects/spectrepro",
    } };
    try cfg.loadIter(alloc, &it);
    try cfg.finalize();
