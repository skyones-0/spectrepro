...
/// SPECTREPRO_SURFACE_ID. It must not be zero as zero is used to incicate a null
...
/// This is managed completely by SpectrePro core but an apprt action
...
/// wasn't handled in any way by SpectrePro.
pub const InputEffect = enum {
    /// The input was not handled in any way by SpectrePro and should be
    /// forwarded to other subsystems (i.e. the OS) for further
    /// processing.
    ignored,

    /// The input was handled and consumed by SpectrePro.
    consumed,
...
pub fn init(
    ...
    rt_app: *apprt.runtime.App,
    rt_surface: *apprt.runtime.Surface,
    serial_config: ?termio.Serial.Config,
) !void {
    ...
    // be scoped here to be valid.
    {
        ...
        // don't leak SPECTREPRO_LOG to any subprocesses
        _ = env.orderedRemove("SPECTREPRO_LOG");

        var buf: [18]u8 = undefined;
        try env.put(
            "SPECTREPRO_SURFACE_ID",
            std.fmt.bufPrint(&buf, "0x{x:0>16}", .{self.id}) catch unreachable,
        );

        // Initialize our IO backend. Serial sessions deliberately bypass a
        // subprocess and own their callout device directly.
        var io_backend: termio.Backend = if (serial_config) |serial| .{ .serial = try termio.Serial.init(alloc, serial) } else .{ .exec = try termio.Exec.init(alloc, .{
            .command = command,
            .env = env,
...
/// Request a hardware BREAK on a native serial backend. The request is
/// serialized through the IO mailbox so it uses the same descriptor as the
/// active terminal session.
pub fn serialBreak(self: *Surface, duration_ms: u32) void {
    self.io.queueMessage(.{ .serial_break = duration_ms }, .unlocked);
}
...
fn childExitedAbnormally(
    ...
    try t.printString("SpectrePro failed to launch the requested command:");
    try t.setAttribute(.{ .unset = {} });
...
pub fn updateConfig(
    ...
    try self.setFontSize(font_size: {
        ...
        // assumed that SpectrePro was broken! This logic makes more sense.
...
fn resize(self: *Surface, size: rendererpkg.ScreenSize) !void {
    ...
    // Recalculate our grid size. Because SpectrePro supports fluid resizing,
...
fn encodeKeyOpts(self: *const Surface) input.key_encode.Options {
    ...
    opts.macos_option_as_alt = self.config.macos_option_as_alt orelse detect: {
        ...
        // here and assume libspectrepro for ease...
...
/// Call to notify SpectrePro that the color scheme for the terminal has
/// changed.
...
test "queueIo frees allocated writes in readonly mode" {
    ...
    const data = try testing.allocator.dupe(u8, "\x1b]lSpectrePro\x1b\\");
