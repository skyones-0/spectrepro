...
fn writeBashCompletions(writer: *std.Io.Writer) !void {
    ...
    try writer.writeAll(
        ...
        \\    local IFS=$'\n'
        \\    COMPREPLY=()
        \\    while read -r line; do COMPREPLY+=("$line"); done < <( compgen -P '"' -S '"' -W "$($spectrepro +list-fonts | grep '^[A-Z]' )" -- "$cur")
        \\  }
        \\
        \\  _themes() {
        \\    local IFS=$'\n'
        \\    COMPREPLY=()
        \\    while read -r line; do COMPREPLY+=("$line"); done < <( compgen -P '"' -S '"' -W "$($spectrepro +list-themes | sed -E 's/^(.*) \(.*$/\1/')" -- "$cur")
        \\  }
        \\
        \\  _files() {
    ...
    try writer.writeAll(
        ...
        \\  # eg: --key=
        \\  # this can be improved if needed see: https://github.com/spectrepro-org/spectrepro/discussions/2994
        \\
        \\  if [ "$2" = "=" ]; then cur=""
        \\  else                    cur="$2"
        \\  fi
        ...
        \\}
        \\
        \\complete -o nospace -o bashdefault -F _spectrepro spectrepro
        \\
    );
}
