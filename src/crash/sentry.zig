...
///
/// PRIVACY NOTE: I want to make it very clear that SpectrePro by default does
...
pub const Transport = struct {
    ...
    fn sendInternal(envelope: *sentry.Envelope) !void {
        ...
        const path = try std.fs.path.join(alloc, &.{
            state_dir,
            try std.fmt.allocPrint(alloc, "{s}.spectreprocrash", .{uuid.string()}),
        });
