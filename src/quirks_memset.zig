...
//! Zig 0.15 produced.
//!
//! In benchmarks this made `spectrepro-bench +terminal-stream` 2.8x slower on
...
comptime {
    ...
    if (enabled) @export(&memset, .{
        ...
        // Hidden so that shared library builds (libspectrepro,
        // libspectrepro-vt) resolve this internally without exporting
...
fn memset(dest: ?[*]u8, c: c_int, len: usize) callconv(.c) ?[*]u8 {
    ...
    if (len >= vec_bytes) {
        ...
        while (i + vec_bytes <= len) : (i += vec_bytes) {
            ...
            // this by being built with -fno-builtin; SpectrePro is not.
