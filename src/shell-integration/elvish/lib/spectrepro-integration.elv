{
  ...
  if (and (has-env SPECTREPRO_SHELL_INTEGRATION_XDG_DIR) (has-env XDG_DATA_DIRS)) {
    set-env XDG_DATA_DIRS (str:replace $E:SPECTREPRO_SHELL_INTEGRATION_XDG_DIR":" "" $E:XDG_DATA_DIRS)
    unset-env SPECTREPRO_SHELL_INTEGRATION_XDG_DIR
  }

  # List of enabled shell integration features
  var features = [(str:split ',' $E:SPECTREPRO_SHELL_FEATURES)]

  # State tracking for semantic prompt sequences
  # Values: 'prompt-start', 'pre-exec', 'post-exec'
  fn set-prompt-state {|new| set-env __spectrepro_prompt_state $new }

  fn mark-prompt-start {
    if (not-eq $E:__spectrepro_prompt_state 'prompt-start') {
  ...
  fn ssh-integration {|@args|
    var spectrepro = $E:SPECTREPRO_BIN_DIR/"spectrepro"
    var flags = []
  ...
  if (str:contains $E:SPECTREPRO_SHELL_FEATURES "cursor") {
  ...
  if (and (has-value $features path) (has-env SPECTREPRO_BIN_DIR)) {
    if (not (has-value $paths $E:SPECTREPRO_BIN_DIR)) {
        set paths = [$@paths $E:SPECTREPRO_BIN_DIR]
    }
  }
  if (and (has-value $features sudo) (not-eq "" $E:TERMINFO) (has-external sudo)) {
    edit:add-var sudo~ $sudo-with-terminfo~
  }
  if (and (str:contains $E:SPECTREPRO_SHELL_FEATURES ssh-) (has-external ssh)) {
