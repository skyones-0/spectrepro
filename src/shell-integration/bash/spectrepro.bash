...
if [ -n "$SPECTREPRO_BASH_INJECT" ]; then
  ...
  builtin declare __spectrepro_bash_flags="$SPECTREPRO_BASH_INJECT"
  builtin unset ENV SPECTREPRO_BASH_INJECT

  # Restore an existing ENV that was replaced by the shell integration code.
  if [[ -n "$SPECTREPRO_BASH_ENV" ]]; then
    builtin export ENV=$SPECTREPRO_BASH_ENV
    builtin unset SPECTREPRO_BASH_ENV
  fi
  ...
  if [[ -n "$SPECTREPRO_BASH_UNEXPORT_HISTFILE" ]]; then
    builtin export -n HISTFILE
    builtin unset SPECTREPRO_BASH_UNEXPORT_HISTFILE
  fi
  ...
  if builtin shopt -q login_shell; then
    if [[ $__spectrepro_bash_flags != *"--noprofile"* ]]; then
      [ -r /etc/profile ] && builtin source "/etc/profile"
      for __spectrepro_rcfile in "$HOME/.bash_profile" "$HOME/.bash_login" "$HOME/.profile"; do
        [ -r "$__spectrepro_rcfile" ] && {
          builtin source "$__spectrepro_rcfile"
          break
        }
      done
    fi
  else
    if [[ $__spectrepro_bash_flags != *"--norc"* ]]; then
      ...
      for __spectrepro_rcfile in /etc/bash.bashrc /etc/bash/bashrc /etc/bashrc; do
        [ -r "$__spectrepro_rcfile" ] && {
          builtin source "$__spectrepro_rcfile"
          break
        }
      done
      if [[ -z "$SPECTREPRO_BASH_RCFILE" ]]; then SPECTREPRO_BASH_RCFILE="$HOME/.bashrc"; fi
      [ -r "$SPECTREPRO_BASH_RCFILE" ] && builtin source "$SPECTREPRO_BASH_RCFILE"
    fi
  fi

  builtin unset __spectrepro_rcfile
  builtin unset __spectrepro_bash_flags
  builtin unset SPECTREPRO_BASH_RCFILE
fi

# Add SpectrePro binary to PATH if the path feature is enabled
if [[ "$SPECTREPRO_SHELL_FEATURES" == *"path"* && -n "$SPECTREPRO_BIN_DIR" ]]; then
  if [[ ":$PATH:" != *":$SPECTREPRO_BIN_DIR:"* ]]; then
    export PATH="$PATH:$SPECTREPRO_BIN_DIR"
  fi
fi

# Sudo
if [[ "$SPECTREPRO_SHELL_FEATURES" == *"sudo"* && -n "$TERMINFO" ]]; then
  # Wrap `sudo` command to ensure SpectrePro terminfo is preserved.
  #
...
if [[ "$SPECTREPRO_SHELL_FEATURES" == *ssh-* ]]; then
  function ssh() {
    builtin local -a flags
    flags=()
    [[ "$SPECTREPRO_SHELL_FEATURES" != *ssh-env* ]] && flags+=(--forward-env=false)
    [[ "$SPECTREPRO_SHELL_FEATURES" != *ssh-terminfo* ]] && flags+=(--terminfo=false)
    "$SPECTREPRO_BIN_DIR/spectrepro" +ssh "${flags[@]}" -- "$@"
  }
fi

# This is set to 1 when we're executing a command so that we don't
# send prompt marks multiple times.
_spectrepro_executing=""
_spectrepro_last_reported_cwd=""

function __spectrepro_precmd() {
  local ret="$?"
  if test "$_spectrepro_executing" != "0"; then
    _SPECTREPRO_SAVE_PS1="$PS1"
    _SPECTREPRO_SAVE_PS2="$PS2"
    ...
    # Cursor
    if [[ "$SPECTREPRO_SHELL_FEATURES" == *"cursor"* ]]; then
      builtin local cursor=5  # blinking bar
      [[ "$SPECTREPRO_SHELL_FEATURES" == *"cursor:steady"* ]] && cursor=6  # steady bar
    ...
    if [[ "$SPECTREPRO_SHELL_FEATURES" == *"title"* ]]; then
      PS1=$PS1'\[\e]2;\w\a\]'
    fi
  fi

  if test "$_spectrepro_executing" != ""; then
  ...
  if [[ "$_spectrepro_last_reported_cwd" != "$PWD" ]]; then
    _spectrepro_last_reported_cwd="$PWD"
    builtin printf "\e]7;kitty-shell-cwd://%s%s\a" "$HOSTNAME" "$PWD"
  fi

  _spectrepro_executing=0
}

function __spectrepro_preexec() {
  builtin local cmd="$1"

  PS1="$_SPECTREPRO_SAVE_PS1"
  PS2="$_SPECTREPRO_SAVE_PS2"

  # Title (current command)
  if [[ -n $cmd && "$SPECTREPRO_SHELL_FEATURES" == *"title"* ]]; then
    builtin printf "\e]2;%s\a" "${cmd//[[:cntrl:]]/}"
  fi

  # End of input, start of output.
  builtin printf "\e]133;C;\a"
  _spectrepro_executing=1
}

if (( BASH_VERSINFO[0] > 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] >= 4) )); then
  __spectrepro_preexec_hook() {
    builtin local cmd
    cmd=$(LC_ALL=C HISTTIMEFORMAT='' builtin history 1)
    cmd="${cmd#*[[:digit:]][* ] }"  # remove leading history number
    [[ -n "$cmd" ]] && __spectrepro_preexec "$cmd"
  }

  __spectrepro_hook() {
    builtin local ret=$?
    __spectrepro_precmd "$ret"

    # Append preexec hook to PS0 if not already present.
    # Use function substitution in 5.3+, otherwise command substitution.
    if [[ "$PS0" != *"__spectrepro_preexec_hook"* ]]; then
      if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 3) )); then
        # shellcheck disable=SC2016
        PS0+='${ __spectrepro_preexec_hook; }'
      else
        # shellcheck disable=SC2016
        PS0+='$(__spectrepro_preexec_hook >/dev/tty)'
      fi
    fi
  }
  ...
  # errors from inside __spectrepro_hook itself, but those are all terminal escape
  # sequences and non-actionable.
  #
  # shellcheck disable=SC2128,SC2178,SC2179
  if [[ ";${PROMPT_COMMAND[*]:-};" != *";__spectrepro_hook 2>/dev/null;"* ]]; then
    if [[ -z "${PROMPT_COMMAND[*]}" ]]; then
      if (( BASH_VERSINFO[0] > 5 || (BASH_VERSINFO[0] == 5 && BASH_VERSINFO[1] >= 1) )); then
        PROMPT_COMMAND=("__spectrepro_hook 2>/dev/null")
      else
        PROMPT_COMMAND="__spectrepro_hook 2>/dev/null"
      fi
  ...
  preexec_functions+=(__spectrepro_preexec)
  precmd_functions+=(__spectrepro_precmd)
fi
