...
function spectrepro_restore_xdg_data_dir -d "restore the original XDG_DATA_DIR value"
    # If we don't have our own data dir then we don't need to do anything.
    if not set -q SPECTREPRO_SHELL_INTEGRATION_XDG_DIR
        return
    end
    ...
    if set --function index (contains --index "$SPECTREPRO_SHELL_INTEGRATION_XDG_DIR" $xdg_data_dirs)
    ...
    set --erase SPECTREPRO_SHELL_INTEGRATION_XDG_DIR
end

function spectrepro_exit -d "exit the shell integration setup"
    functions -e spectrepro_restore_xdg_data_dir
    functions -e spectrepro_exit
    exit 0
end

# We always try to restore the XDG data dir
spectrepro_restore_xdg_data_dir

# If we aren't interactive or we've already run, don't run.
status --is-interactive || spectrepro_exit
...
function __spectrepro_setup --on-event fish_prompt -d "Setup spectrepro integration"
    functions -e __spectrepro_setup

    set --local features (string split , $SPECTREPRO_SHELL_FEATURES)
    ...
    set -g __spectrepro_prompt_start_mark "\e]133;A\a"
    if test "$fish_major" -gt 4; or test "$fish_major" -eq 4 -a "$fish_minor" -ge 1
        set -g __spectrepro_prompt_start_mark "\e]133;A;click_events=1\a"
    end
    ...
    if string match -q 'cursor*' -- $features
        ...
        function __spectrepro_set_cursor_beam --on-event fish_prompt -V cursor -d "Set cursor shape"
            if not functions -q fish_vi_cursor_handle
                echo -en "\e[$cursor q"
            end
        end
        function __spectrepro_reset_cursor --on-event fish_preexec -d "Reset cursor shape"
            if not functions -q fish_vi_cursor_handle
                echo -en "\e[0 q"
            end
        end
    ...
    if contains path $features; and test -n "$SPECTREPRO_BIN_DIR"
        fish_add_path --global --path --append "$SPECTREPRO_BIN_DIR"
    end
    ...
    set -l features (string split ',' -- "$SPECTREPRO_SHELL_FEATURES")
    if contains ssh-env $features; or contains ssh-terminfo $features
        function ssh --wraps=ssh --description "SSH wrapper with SpectrePro integration"
            set -l features (string split ',' -- "$SPECTREPRO_SHELL_FEATURES")
            set -l flags
            contains ssh-env $features; or set -a flags --forward-env=false
            contains ssh-terminfo $features; or set -a flags --terminfo=false
            "$SPECTREPRO_BIN_DIR/spectrepro" +ssh $flags -- $argv
        end
    end

    # Setup prompt marking
    function __spectrepro_mark_prompt_start --on-event fish_prompt --on-event fish_posterror
        # If we never got the output end event, then we need to send it now.
        if test "$__spectrepro_prompt_state" != prompt-start
            echo -en "\e]133;D\a"
        end

        set --global __spectrepro_prompt_state prompt-start
        echo -en $__spectrepro_prompt_start_mark
    end

    function __spectrepro_mark_output_start --on-event fish_preexec
        set --global __spectrepro_prompt_state pre-exec
        echo -en "\e]133;C\a"
    end

    function __spectrepro_mark_output_end --on-event fish_postexec
        set --global __spectrepro_prompt_state post-exec
    ...
    # Enable fish to handle reflow because SpectrePro clears the prompt on resize.
    set --global fish_handle_reflow 1

    # Initial calls for first prompt
    if string match -q 'cursor*' -- $features
        __spectrepro_set_cursor_beam
    end
    __spectrepro_mark_prompt_start
    __update_cwd_osc
end
