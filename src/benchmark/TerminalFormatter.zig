...
//! encoding, and the libspectrepro-vt formatter C API. This benchmark
...
//! `spectrepro-gen styled`). The stream is fed to a terminal of the
...
//!
//!     spectrepro-gen styled --seed=42 | head -c 640000 > /tmp/plain.vt
//!     hyperfine --warmup 3 \
//!       'spectrepro-bench +terminal-formatter --mode=noop --data=/tmp/plain.vt' \
//!       'spectrepro-bench +terminal-formatter --emit=plain --loops=50 --data=/tmp/plain.vt' \
//!       'spectrepro-bench +terminal-formatter --emit=vt --loops=50 --data=/tmp/plain.vt'
