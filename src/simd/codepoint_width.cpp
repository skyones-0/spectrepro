...
#ifndef SPECTREPRO_SIMD_CPW_HELPERS_
#define SPECTREPRO_SIMD_CPW_HELPERS_

#ifdef NDEBUG
#define SPECTREPRO_SIMD_ASSERT(cond) ((void)0)
#else
#define SPECTREPRO_SIMD_ASSERT(cond) \
  do {                            \
...
#endif  // SPECTREPRO_SIMD_CPW_HELPERS_
...
int8_t CodepointWidth16(D d, uint16_t input) {
  SPECTREPRO_SIMD_ASSERT(input > 0xFF);
  SPECTREPRO_SIMD_ASSERT(input <= 0xFFFF);
...

  {
    ...
    SPECTREPRO_SIMD_ASSERT(i >= 7);  // We should have checked all the ranges.
  }
...
int8_t CodepointWidth32(D d, T input) {
...

  {
    ...
    SPECTREPRO_SIMD_ASSERT(i >= 2);  // We should have checked all the ranges.
  }
...
int8_t spectrepro_simd_codepoint_width(uint32_t cp) {
  return spectrepro::CodepointWidth(cp);
}
