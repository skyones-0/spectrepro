...
pub fn maxLen(input: []const u8) usize {
    if (comptime options.simd) return spectrepro_simd_base64_max_length(
        input.ptr,
        input.len,
    );
...
pub fn decode(input: []const u8, output: []u8) error{Base64Invalid}![]const u8 {
    if (comptime options.simd) {
        const res = spectrepro_simd_base64_decode(
            input.ptr,
            input.len,
            output.ptr,
        );
...
// base64.cpp
extern "c" fn spectrepro_simd_base64_max_length(
    input: [*]const u8,
    len: usize,
) usize;
extern "c" fn spectrepro_simd_base64_decode(
    input: [*]const u8,
    len: usize,
    output: [*]u8,
) isize;
