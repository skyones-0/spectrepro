...
pub const Target = union(Key) {
    ...
    // Sync with: spectrepro_target_tag_e
    pub const Key = enum(c_int) {
        app,
        surface,

        test "spectrepro.h Target.Key" {
            try lib.checkSpectreProHEnum(Key, "SPECTREPRO_TARGET_");
        }
    };

    // Sync with: spectrepro_target_u
    pub const CValue = extern union {
        app: void,
        surface: *apprt.Surface,
    };

    // Sync with: spectrepro_target_s
    pub const C = extern struct {
        key: Key,
        value: CValue,
    };

    /// Convert to spectrepro_target_s.
...
pub const Action = union(Key) {
    ...
    //    because it maps directly to the libspectrepro C enum. For ABI
    ...
    //
    // 4. Update `include/spectrepro.h`: add the new key, value, and union
    ...
    /// Toggle the visibility of all SpectrePro terminal windows.
    toggle_visibility,
    ...
    /// Open the SpectrePro configuration. This is platform-specific about
    ...
    ///
    /// Despite the name, this is the notification that libspectrepro sends
    ...
    /// Sync with: spectrepro_action_tag_e
    pub const Key = enum(c_int) {
        quit,
        new_window,
        new_tab,
        ...
        test "spectrepro.h Action.Key" {
            try lib.checkSpectreProHEnum(Key, "SPECTREPRO_ACTION_");
        }
    };

    /// Sync with: spectrepro_action_u
    ...
    /// Convert to spectrepro_action_s.
...
pub const SplitDirection = enum(c_int) {
    ...
    test "spectrepro.h SplitDirection" {
        try lib.checkSpectreProHEnum(SplitDirection, "SPECTREPRO_SPLIT_DIRECTION_");
    }
};
...
pub const GotoSplit = enum(c_int) {
    ...
    test "spectrepro.h GotoSplit" {
        try lib.checkSpectreProHEnum(GotoSplit, "SPECTREPRO_GOTO_SPLIT_");
    }
};
...
pub const GotoWindow = enum(c_int) {
    ...
    test "spectrepro.h GotoWindow" {
        try lib.checkSpectreProHEnum(GotoWindow, "SPECTREPRO_GOTO_WINDOW_");
    }
};
...
pub const ResizeSplit = extern struct {
    ...
    pub const Direction = enum(c_int) {
        ...
        test "spectrepro.h ResizeSplit.Direction" {
            try lib.checkSpectreProHEnum(Direction, "SPECTREPRO_RESIZE_SPLIT_");
        }
    };
};
...
pub const GotoTab = enum(c_int) {
    ...
    // test "spectrepro.h GotoTab" {
    //     try lib.checkSpectreProHEnum(GotoTab, "SPECTREPRO_GOTO_TAB_");
    // }
};
...
pub const Fullscreen = enum(c_int) {
    ...
    test "spectrepro.h Fullscreen" {
        try lib.checkSpectreProHEnum(Fullscreen, "SPECTREPRO_FULLSCREEN_");
    }
};
...
pub const FloatWindow = enum(c_int) {
    ...
    test "spectrepro.h FloatWindow" {
        try lib.checkSpectreProHEnum(FloatWindow, "SPECTREPRO_FLOAT_WINDOW_");
    }
};
...
pub const SecureInput = enum(c_int) {
    ...
    test "spectrepro.h SecureInput" {
        try lib.checkSpectreProHEnum(SecureInput, "SPECTREPRO_SECURE_INPUT_");
    }
};
...
pub const Inspector = enum(c_int) {
    ...
    test "spectrepro.h Inspector" {
        try lib.checkSpectreProHEnum(Inspector, "SPECTREPRO_INSPECTOR_");
    }
};
...
pub const ExportTerminalIO = struct {
    ...
    // Sync with: spectrepro_action_export_terminal_io_s
    pub const C = extern struct {
        contents: [*]const u8,
        len: usize,
    };
...
pub const QuitTimer = enum(c_int) {
    ...
    test "spectrepro.h QuitTimer" {
        try lib.checkSpectreProHEnum(QuitTimer, "SPECTREPRO_QUIT_TIMER_");
    }
};
...
pub const Readonly = enum(c_int) {
    ...
    test "spectrepro.h Readonly" {
        try lib.checkSpectreProHEnum(Readonly, "SPECTREPRO_READONLY_");
    }
};
...
pub const MouseVisibility = enum(c_int) {
    ...
    test "spectrepro.h MouseVisibility" {
        try lib.checkSpectreProHEnum(MouseVisibility, "SPECTREPRO_MOUSE_");
    }
};
...
pub const PromptTitle = enum(c_int) {
    ...
    test "spectrepro.h PromptTitle" {
        try lib.checkSpectreProHEnum(PromptTitle, "SPECTREPRO_PROMPT_TITLE_");
    }
};
...
pub const MouseOverLink = struct {
    ...
    // Sync with: spectrepro_action_mouse_over_link_s
    pub const C = extern struct {
        url: [*]const u8,
        len: usize,
    };
...
pub const InitialSize = extern struct {
    ...
    pub const getGObjectType = switch (build_config.app_runtime) {
        .gtk => @import("gobject").ext.defineBoxed(
            InitialSize,
            .{ .name = "SpectreProApprtInitialSize" },
        ),
...
pub const SetTitle = struct {
    ...
    // Sync with: spectrepro_action_set_title_s
    pub const C = extern struct {
        title: [*:0]const u8,
    };
...
pub const Pwd = struct {
    ...
    // Sync with: spectrepro_action_set_pwd_s
    pub const C = extern struct {
        pwd: [*:0]const u8,
    };
...
pub const DesktopNotification = struct {
    ...
    // Sync with: spectrepro_action_desktop_notification_s
    pub const C = extern struct {
        title: [*:0]const u8,
        body: [*:0]const u8,
    };
...
pub const KeySequence = union(enum) {
    ...
    // Sync with: spectrepro_action_key_sequence_s
    pub const C = extern struct {
        active: bool,
...
pub const KeyTable = union(enum) {
    ...
    // Sync with: spectrepro_action_key_table_tag_e
    ...
    // Sync with: spectrepro_action_key_table_u
    pub const CValue = extern union {
    ...
    // Sync with: spectrepro_action_key_table_s
    pub const C = extern struct {
        tag: Tag,
        value: CValue,
    };
...
pub const ColorKind = enum(c_int) {
    ...
    // test "spectrepro.h ColorKind" {
    //     try lib.checkSpectreProHEnum(ColorKind, "SPECTREPRO_COLOR_KIND_");
    // }
};

pub const ReloadConfig = extern struct {
    /// A soft reload means that the configuration doesn't need to be
    /// read off disk, but libspectrepro needs the full config again so call
...
pub const ConfigChange = struct {
    ...
    // Sync with: spectrepro_action_config_change_s
    pub const C = extern struct {
        config: *const configpkg.Config,
    };
...
pub const OpenUrl = struct {
    ...
    ///
    /// Sync with: spectrepro_action_open_url_kind_e
    pub const Kind = enum(c_int) {
        ...
        test "spectrepro.h OpenUrl.Kind" {
            try lib.checkSpectreProHEnum(Kind, "SPECTREPRO_ACTION_OPEN_URL_KIND_");
        }
    };

    // Sync with: spectrepro_action_open_url_s
    pub const C = extern struct {
        kind: Kind,
        url: [*]const u8,
        len: usize,
...
/// sync with spectrepro_action_close_tab_mode_e in spectrepro.h
pub const CloseTabMode = enum(c_int) {
    ...
    test "spectrepro.h CloseTabMode" {
        try lib.checkSpectreProHEnum(CloseTabMode, "SPECTREPRO_ACTION_CLOSE_TAB_MODE_");
    }
};
...
pub const CommandFinished = struct {
    ...
    /// sync with spectrepro_action_command_finished_s in spectrepro.h
    pub const C = extern struct {
        exit_code: i16,
        duration: u64,
    };
...
pub const StartSearch = struct {
    ...
    // Sync with: spectrepro_action_start_search_s
    pub const C = extern struct {
        needle: [*:0]const u8,
    };
...
pub const SearchTotal = struct {
    ...
    // Sync with: spectrepro_action_search_total_s
    pub const C = extern struct {
        total: isize,
    };
...
pub const SearchSelected = struct {
    ...
    // Sync with: spectrepro_action_search_selected_s
    pub const C = extern struct {
        selected: isize,
    };
...
pub const OpenConfig = enum(c_int) {
    ...
    test "spectrepro.h OpenConfig" {
        try lib.checkSpectreProHEnum(OpenConfig, "SPECTREPRO_ACTION_OPEN_CONFIG_");
    }
};
