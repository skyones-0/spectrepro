...
//
// `spectrepro +new-tab` is equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest co.skyones.spectrepro --object-path /co/skyones/spectrepro --method org.gtk.Actions.Activate new-tab '[<@(tas) (0, [])>]' []
// ```
//
// `spectrepro +new-tab -e echo hello` would be equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest co.skyones.spectrepro --object-path /co/skyones/spectrepro --method org.gtk.Actions.Activate new-tab '[<@(tas) (0, ["-e" "echo" "hello"])>]' []
// ```
pub fn newTab(alloc: Allocator, target: apprt.ipc.Target, value: apprt.ipc.Action.NewTab) (Allocator.Error || std.Io.Writer.Error || apprt.ipc.Errors)!bool {
...

    {
        ...
        // main SpectrePro instance and interpreted as CLI arguments.
