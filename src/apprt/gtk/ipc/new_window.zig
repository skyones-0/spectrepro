...
//
// `spectrepro +new-window` is equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest co.skyones.spectrepro --object-path /co/skyones/spectrepro --method org.gtk.Actions.Activate new-window [] []
// ```
//
// `spectrepro +new-window -e echo hello` would be equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest co.skyones.spectrepro --object-path /co/skyones/spectrepro --method org.gtk.Actions.Activate new-window-command '[<@as ["-e" "echo" "hello"]>]' []
// ```
pub fn newWindow(alloc: Allocator, target: apprt.ipc.Target, value: apprt.ipc.Action.NewWindow) (Allocator.Error || std.Io.Writer.Error || apprt.ipc.Errors)!bool {
    ...
    if (value.arguments) |arguments| {
        ...
        // will be sent to the main SpectrePro instance and interpreted as CLI
        // arguments.
