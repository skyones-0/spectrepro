...
/// The bus name of the SpectrePro instance that we are calling.
bus_name: [:0]const u8,

/// The object path of the SpectrePro instance that we are calling.
...
pub fn init(alloc: Allocator, target: apprt.ipc.Target, action: [:0]const u8) InitError!Self {
    ...
    // SpectrePro instance we're interested in.
...
/// Send the IPC to the remote SpectrePro. Once it completes, nothing further
