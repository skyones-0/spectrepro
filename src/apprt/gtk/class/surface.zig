...
const log = std.log.scoped(.gtk_spectrepro_surface);
...
pub const Surface = extern struct {
    ...
    const Private = struct {
        ...
        /// usually by shell integration which then talks to libspectrepro
        ...
        /// Cached metrics for libspectrepro callbacks
    ...
    /// Force the surface to redraw itself. SpectrePro often will only redraw
    ...
    pub fn setScrollbar(self: *Self, scrollbar: terminal.Scrollbar) void {
        ...
        // During manual scrollbar changes from SpectrePro core we don't
    ...
    pub fn keyEvent(
        ...
        // the block for more specifics.
        {
            ...
            // never proceed with key encoding for SpectrePro but that is not the
    ...
    pub fn childExited(
        ...
        // for child exited. The false return will force libspectrepro to show
    ...
    pub fn defaultTermioEnv(self: *Self) !std.process.Environ.Map {
        ...
        // Remove some environment variables that are set when SpectrePro is launched
        ...
        // This allows SpectrePro to further launch additional snaps.
        ...
        if (ext.getAncestor(Window, self.as(gtk.Widget))) |window| {
            ...
            if (window.isQuickTerminal()) {
                try env.put("SPECTREPRO_QUICK_TERMINAL", "1");
            }
        }
    ...
    fn filterSnapPaths(gpa: std.mem.Allocator, env_map: *std.process.Environ.Map) !void {
        ...
        while (it.next()) |entry| {
            ...
            if (std.mem.startsWith(u8, key, "SPECTREPRO")) continue;
    ...
    fn snapshot(self: *Self, snap: *gtk.Snapshot) callconv(.c) void {
        ...
        if (blur) |b| blur: {
            ...
            // SpectrePro on an older GTK version, you won't get blur.
            // Sorry.
    ...
    /// there are actions within libspectrepro callbacks (such as showing close
    ...
    fn imPreeditChanged(
        ...
        // Update our preedit state in SpectrePro core
    ...
    fn imPreeditEnd(
        ...
        // End our preedit state in SpectrePro core
        const surface = priv.core_surface orelse return;
    ...
    fn imCommit(
        ...
        switch (priv.in_keyevent) {
            ...
            // that SpectrePro can be aware of the key event alongside
            // the text.
            .not_composing => {
    ...
    fn glareaRealize(
        ...
        if (priv.gl_area.getError()) |err| {
            ...
            log.warn("this is a common cause of this issue: https://spectrepro.org/docs/help/gtk-opengl-context", .{});
            self.setError(true);
            return;
        }
