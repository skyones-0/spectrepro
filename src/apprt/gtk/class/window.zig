...
const log = std.log.scoped(.gtk_spectrepro_window);
...
pub const Window = extern struct {
    ...
    fn syncAppearance(self: *Self) void {
        ...
        // Apply class to color headerbar if window-theme is set to `spectrepro` and
        // GTK version is before 4.16. The conditional is because above 4.16
        // we use GTK CSS color variables.
        self.toggleCssClass(
            "window-theme-spectrepro",
            !gtk_version.atLeast(4, 16, 0) and
                config.@"window-theme" == .spectrepro,
        );
    ...
    fn closureTitle(
        ...
        return glib.ext.dupeZ(u8, "SpectrePro");
    }
    ...
    fn tabViewNPages(
        ...
        if (priv.tab_view.getNPages() == 0) {
            ...
            // won't cause SpectrePro to exit.
    ...
    fn actionAbout(
        ...
        const name = "SpectrePro";
        const icon = "co.skyones.spectrepro";
        const website = "https://spectrepro.org";
        ...
        if (adw_version.supportsDialogs()) {
            adw.showAboutDialog(
                ...
                name,
                "developer-name",
                i18n._("SpectrePro Developers"),
                "application-icon",
                icon,
                "version",
                build_config.version_string.ptr,
                "issue-url",
                "https://github.com/spectrepro-org/spectrepro/issues",
                "website",
                website,
                @as(?*anyopaque, null),
            );
    ...
    /// Toggle the SpectrePro inspector for the active surface.
    fn toggleInspector(self: *Self) void {
        const surface = self.getActiveSurface() orelse return;
        _ = surface.controlInspector(.toggle);
    }

    /// React to a GTK action requesting that the SpectrePro inspector be toggled.
