...
const log = std.log.scoped(.gtk_spectrepro_application);
...
/// The primary entrypoint for the SpectrePro GTK application.
///
/// This requires a `spectrepro.App` and `spectrepro.Config` and takes
...
pub const Application = extern struct {
    ...
    const Private = struct {
        ...
        /// nicer to just make THIS the apprt app but the current libspectrepro
        /// API doesn't allow that.
        rt_app: *ApprtApp,

        /// The libspectrepro App instance.
        core_app: *CoreApp,
        ...
        /// CSS Provider for any styles based on SpectrePro configuration values.
        css_provider: *gtk.CssProvider,

        /// Providers for loading custom stylesheets defined by user
        custom_css_providers: std.ArrayListUnmanaged(*gtk.CssProvider) = .empty,

        /// A copy of the LANG environment variable that was provided to SpectrePro
        /// by the system. If this is null, the LANG environment variable did
        /// not exist in SpectrePro's environment variable.
    ...
    /// default application is a SpectreProApplication. The program would have
    ...
    pub fn new(
        ...
        // can develop SpectrePro in SpectrePro.
    ...
    /// Get the original language that SpectrePro was launched with. This returns a
    ...
    /// integrate it with libspectrepro.
    pub fn run(self: *Self) !void {
        ...
        // for launching SpectrePro in the "background" without immediately opening
        // a window).
        //
        ...
        while (priv.running) {
            ...
            // Tick the core SpectrePro terminal app
    ...
    fn loadRuntimeCss416(
        ...
        switch (window_theme) {
            .spectrepro => try writer.print(
                \\:root {{
                \\  --spectrepro-fg: rgb({d},{d},{d});
                \\  --spectrepro-bg: rgb({d},{d},{d});
                \\  --headerbar-fg-color: var(--spectrepro-fg);
                \\  --headerbar-bg-color: var(--spectrepro-bg);
                \\  --headerbar-backdrop-color: oklab(from var(--headerbar-bg-color) calc(l * 0.9) a b / alpha);
                \\  --overview-fg-color: var(--spectrepro-fg);
                \\  --overview-bg-color: var(--spectrepro-bg);
                \\  --popover-fg-color: var(--spectrepro-fg);
                \\  --popover-bg-color: var(--spectrepro-bg);
                \\  --window-fg-color: var(--spectrepro-fg);
                \\  --window-bg-color: var(--spectrepro-bg);
                \\}}
                \\windowhandle {{
    ...
    fn startupStyleManager(self: *Self) void {
        ...
        // if our current theme matches what libspectrepro has so its safe to
        // call.
    ...
    pub const Class = extern struct {
        ...
        fn init(class: *Class) callconv(.c) void {
            ...
            {
                const spectrepro_gtk_resources = @import("spectrepro_gtk_resources");
                if (spectrepro_gtk_resources.spectrepro_get_resource()) |ptr| {
...
const Action = struct {
    ...
    pub fn exportTerminalIO(
        ...
        chooser.setCurrentName("spectrepro-terminal-io.txt");
    ...
    pub fn desktopNotification(
        ...
        const icon = gio.ThemedIcon.new("co.skyones.spectrepro");
        defer icon.unref();
    ...
    fn gotoWindowMaybe(gtk_window: *gtk.Window) bool {
        ...
        // If it isn't a SpectrePro window, skip it.
...
fn setGtkEnv(config: *const CoreConfig) std.Io.Writer.Error!void {
    ...
    var gdk_debug: struct {
        ...
        opengl: bool = false,
        /// disable GLES, SpectrePro can't use GLES
    ...
    environment: {
        ...
        // Versions prior to 4.14 are a bit of an unknown for SpectrePro. It
