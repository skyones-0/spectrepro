...
const log = std.log.scoped(.gtk_spectrepro_global_shortcuts);
...
pub const GlobalShortcuts = extern struct {
    ...
    pub const getGObjectType = gobject.ext.defineClass(Self, .{
        .name = "SpectreProGlobalShortcuts",
        .instanceInit = &init,
    ...
    pub const BindFailed = struct {
        ...
        pub const getGObjectType = gobject.ext.defineBoxed(
            BindFailed,
            .{ .name = "SpectreProGlobalShortcutsBindFailed" },
        );
    };
    ...
    /// Get the XDG portal request path for the current SpectrePro instance.
    ///
...
fn generateToken(buf: *Token) [:0]const u8 {
    // u28 takes up 7 bytes in hex, 8 bytes for "spectrepro_" and 1 byte for NUL
    // 7 + 8 + 1 = 16
