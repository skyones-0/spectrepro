...
const log = std.log.scoped(.gtk_spectrepro_surface_child_exited);
...
const SurfaceChildExitedBanner = extern struct {
    ...
    pub const getGObjectType = gobject.ext.defineClass(Self, .{
        .name = "SpectreProSurfaceChildExited",
        .instanceInit = &init,
    ...
    fn propData(
        ...
        const data = priv.data orelse {
            ...
            banner.as(adw.Banner).setTitle("This is a bug in SpectrePro. Please report it.");
            return;
        };
        if (data.exit_code == 0) {
