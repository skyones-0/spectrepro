...
pub const blueprint_compiler_help =
    \\
    \\When building from a Git checkout, SpectrePro requires
    \\version {f} or newer of `blueprint-compiler` as a
    \\build-time dependency. Please install it, ensure that it
    \\is available on your PATH, and then retry building SpectrePro.
    \\See `HACKING.md` for more details.
    \\
    \\This message should *not* appear for normal users, who
    \\should build SpectrePro from official release tarballs instead.
    \\Please consult https://spectrepro.org/docs/install/build for
...
pub fn main(init: std.process.Init) !void {
    ...
    if (adwaita_version.order(required_adwaita_version) == .lt) {
        std.debug.print(
            ...
            \\
            \\SpectrePro requires a version {f} or newer of `libadwaita` to
            \\compile this blueprint. Please install it, ensure that it is
            \\available on your PATH, and then retry building SpectrePro.
