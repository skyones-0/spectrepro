//! Application runtime for the embedded version of SpectrePro. The embedded
//! version is when SpectrePro is embedded within a parent host application,
//! rather than owning the application lifecycle itself. This is used for
//! example for the macOS build of SpectrePro so that we can use a native
...
pub const App = struct {
    ...
    ///
    /// C type: spectrepro_runtime_config_s
    ...
    /// This is the key event sent for spectrepro_surface_key and
    /// spectrepro_app_key.
    pub const KeyEvent = struct {
        ...
        /// Convert a libspectrepro key event into a core key event.
    ...
    pub fn keyEvent(
        ...
        // Invoke the core SpectrePro logic to handle this input.
    ...
    /// Send the given IPC to a running SpectrePro. Returns `true` if the action was
    /// able to be performed, `false` otherwise.
    ///
    /// Note that this is a static function. Since this is called from a CLI app (or
    /// some other process that is not SpectrePro) there is no full-featured apprt App
    /// to use.
    pub fn performIpc(
        _: Allocator,
        _: apprt.ipc.Target,
...
/// Platform-specific configuration for libspectrepro.
pub const Platform = union(PlatformTag) {
    macos: MacOS,
    ios: IOS,

    // If our build target for libspectrepro is not darwin then we do
...
pub const Surface = struct {
    ...
    pub const Options = extern struct {
        ...
        /// despite SpectrePro allowing directly executed commands via config.
    ...
    pub fn defaultTermioEnv(self: *const Surface) !std.process.Environ.Map {
        ...
        if (comptime builtin.target.os.tag.isDarwin()) {
            ...
            // Remove this so that running `spectrepro` within SpectrePro works.
            _ = env.orderedRemove("SPECTREPRO_MAC_LAUNCH_SOURCE");

            // If we were launched from the desktop then we want to
            // remove the LANGUAGE env var so that we don't inherit
            // our translation settings for SpectrePro. If we aren't from
...
pub const Inspector = struct {
    ...
    pub fn init(surface: *Surface) !Inspector {
        ...
        io.BackendPlatformName = "spectrepro_embedded";
...
pub const CAPI = struct {
    ...
    export fn spectrepro_app_tick(v: *App) void {
    ...
    export fn spectrepro_app_userdata(v: *App) ?*anyopaque {
        return v.opts.userdata;
    }

    export fn spectrepro_app_free(v: *App) void {
    ...
    export fn spectrepro_app_set_focus(
        app: *App,
        focused: bool,
    ) void {
    ...
    export fn spectrepro_config_key_is_binding(
        config: *Config,
        event: KeyEvent,
    ) bool {
    ...
    export fn spectrepro_app_keyboard_changed(v: *App) void {
    ...
    export fn spectrepro_app_open_config(v: *App) void {
    ...
    export fn spectrepro_app_update_config(
        v: *App,
        config: *const Config,
    ) void {
    ...
    export fn spectrepro_app_needs_confirm_quit(v: *App) bool {
        return v.core_app.needsConfirmQuit();
    }

    /// Returns true if the app has global keybinds.
    export fn spectrepro_app_has_global_keybinds(v: *App) bool {
        return v.hasGlobalKeybinds();
    }

    /// Update the color scheme of the app.
    export fn spectrepro_app_set_color_scheme(v: *App, scheme_raw: c_int) void {
    ...
    export fn spectrepro_surface_config_new() apprt.Surface.Options {
        return .{};
    }

    /// Create a new surface as part of an app.
    export fn spectrepro_surface_new(
        app: *App,
    ...
    export fn spectrepro_surface_free(ptr: *Surface) void {
        ptr.app.closeSurface(ptr);
    }

    /// Returns the userdata associated with the surface.
    export fn spectrepro_surface_userdata(surface: *Surface) ?*anyopaque {
        return surface.userdata;
    }

    /// Returns the app associated with a surface.
    export fn spectrepro_surface_app(surface: *Surface) *App {
        return surface.app;
    }

    /// Returns the config to use for surfaces that inherit from this one.
    export fn spectrepro_surface_inherited_config(
        surface: *Surface,
    ...
    export fn spectrepro_surface_update_config(
        surface: *Surface,
        config: *const Config,
    ) void {
    ...
    export fn spectrepro_surface_needs_confirm_quit(surface: *Surface) bool {
        return surface.core_surface.needsConfirmQuit();
    }

    /// Returns true if the surface process has exited.
    export fn spectrepro_surface_process_exited(surface: *Surface) bool {
        return surface.core_surface.child_exited;
    }

    /// Returns true if the surface has a selection.
    export fn spectrepro_surface_has_selection(surface: *Surface) bool {
        return surface.core_surface.hasSelection();
    }

    /// Same as spectrepro_surface_read_text but reads from the user selection,
    /// if any.
    export fn spectrepro_surface_read_selection(
        surface: *Surface,
        result: *Text,
    ) bool {
    ...
    export fn spectrepro_surface_read_text(
        surface: *Surface,
        sel: Selection,
        result: *Text,
    ) bool {
    ...
    export fn spectrepro_surface_free_text(_: *Surface, ptr: *Text) void {
        ptr.deinit();
    }

    /// Tell the surface that it needs to schedule a render
    export fn spectrepro_surface_refresh(surface: *Surface) void {
        surface.refresh();
    }

    /// Tell the surface that it needs to schedule a render
    /// call as soon as possible (NOW if possible).
    export fn spectrepro_surface_draw(surface: *Surface) void {
        surface.draw();
    }

    /// Update the size of a surface. This will trigger resize notifications
    /// to the pty and the renderer.
    export fn spectrepro_surface_set_size(surface: *Surface, w: u32, h: u32) void {
        surface.updateSize(w, h);
    }

    /// Return the size information a surface has.
    export fn spectrepro_surface_size(surface: *Surface) SurfaceSize {
    ...
    export fn spectrepro_surface_foreground_pid(surface: *Surface) u64 {
    ...
    /// freed by the caller via spectrepro_string_free.
    export fn spectrepro_surface_tty_name(surface: *Surface) String {
    ...
    export fn spectrepro_surface_set_color_scheme(surface: *Surface, scheme_raw: c_int) void {
        const scheme = std.enums.fromInt(apprt.ColorScheme, scheme_raw) orelse return;
        surface.colorSchemeCallback(scheme);
    }

    /// Update the content scale of the surface.
    export fn spectrepro_surface_set_content_scale(surface: *Surface, x: f64, y: f64) void {
        surface.updateContentScale(x, y);
    }

    /// Update the focused state of a surface.
    export fn spectrepro_surface_set_focus(surface: *Surface, focused: bool) void {
        surface.focusCallback(focused);
    }

    /// Update the occlusion state of a surface.
    export fn spectrepro_surface_set_occlusion(surface: *Surface, visible: bool) void {
    ...
    export fn spectrepro_surface_key_translation_mods(
        surface: *Surface,
        mods_raw: c_int,
    ) c_int {
    ...
    /// key and char events.
    export fn spectrepro_surface_key(
        surface: *Surface,
        event: KeyEvent,
    ) bool {
    ...
    /// is important because things like trigger sequences are only
    /// valid until the next key event.
    export fn spectrepro_surface_key_is_binding(
        surface: *Surface,
        event: KeyEvent,
    ...
    export fn spectrepro_surface_text(
        surface: *Surface,
        ptr: [*]const u8,
        len: usize,
    ) void {
    ...
    export fn spectrepro_surface_preedit(
        surface: *Surface,
        ptr: [*]const u8,
        len: usize,
    ) void {
    ...
    /// enabled.
    export fn spectrepro_surface_mouse_captured(surface: *Surface) bool {
        return surface.core_surface.mouseCaptured();
    }

    /// Tell the surface that it needs to schedule a render
    export fn spectrepro_surface_mouse_button(
        surface: *Surface,
        action: input.MouseButtonState,
        button: input.MouseButton,
        mods: c_int,
    ...
    export fn spectrepro_surface_mouse_pos(
        surface: *Surface,
        x: f64,
        y: f64,
        mods: c_int,
    ...
    export fn spectrepro_surface_mouse_scroll(
        surface: *Surface,
        x: f64,
        y: f64,
        scroll_mods: c_int,
    ...
    export fn spectrepro_surface_mouse_pressure(
        surface: *Surface,
        stage_raw: u32,
        pressure: f64,
    ) void {
    ...
    export fn spectrepro_surface_ime_point(
        surface: *Surface,
        x: *f64,
        y: *f64,
        width: *f64,
    ...
    export fn spectrepro_surface_request_close(ptr: *Surface) void {
        ptr.core_surface.close();
    }

    /// Request that the surface split in the given direction.
    export fn spectrepro_surface_split(ptr: *Surface, direction: apprt.action.SplitDirection) void {
    ...
    export fn spectrepro_surface_split_focus(
        ptr: *Surface,
    ...
    export fn spectrepro_surface_split_resize(
        ptr: *Surface,
    ...
    export fn spectrepro_surface_split_equalize(ptr: *Surface) void {
        _ = ptr.app.performAction(
            .{ .surface = &ptr.core_surface },
            .equalize_splits,
            {},
    ...
    export fn spectrepro_surface_binding_action(
        ptr: *Surface,
        action_ptr: [*]const u8,
        action_len: usize,
    ) bool {
    ...
    /// be invalidated.
    ///
    /// To deny a request use spectrepro_surface_deny_clipboard_request
    /// instead.
    export fn spectrepro_surface_complete_clipboard_request(
        ptr: *Surface,
    ...
    export fn spectrepro_surface_deny_clipboard_request(
        ptr: *Surface,
        state: *apprt.ClipboardRequest,
    ) void {
    ...
    export fn spectrepro_surface_inspector(ptr: *Surface) ?*Inspector {
    ...
    export fn spectrepro_inspector_free(ptr: *Surface) void {
        ptr.freeInspector();
    }

    export fn spectrepro_inspector_set_size(ptr: *Inspector, w: u32, h: u32) void {
        ptr.updateSize(w, h);
    }

    export fn spectrepro_inspector_set_content_scale(ptr: *Inspector, x: f64, y: f64) void {
        ptr.updateContentScale(x, y);
    }

    export fn spectrepro_inspector_mouse_button(
        ptr: *Inspector,
        action: input.MouseButtonState,
        button: input.MouseButton,
        mods: c_int,
    ...
    export fn spectrepro_inspector_mouse_pos(ptr: *Inspector, x: f64, y: f64) void {
        ptr.cursorPosCallback(x, y);
    }

    export fn spectrepro_inspector_mouse_scroll(
        ptr: *Inspector,
        x: f64,
        y: f64,
        scroll_mods: c_int,
    ...
    export fn spectrepro_inspector_key(
        ptr: *Inspector,
        action: input.Action,
        key: input.Key,
        c_mods: c_int,
    ...
    export fn spectrepro_inspector_text(
        ptr: *Inspector,
        str: [*:0]const u8,
    ) void {
    ...
    export fn spectrepro_inspector_set_focus(ptr: *Inspector, focused: bool) void {
    ...
    export fn spectrepro_set_window_background_blur(
        app: *App,
        window: *anyopaque,
    ) void {
    ...
    /// See spectrepro_set_window_background_blur
    ...
    // Darwin-only C APIs.
    const Darwin = struct {
        export fn spectrepro_surface_set_display_id(ptr: *Surface, display_id: u32) void {
        ...
        export fn spectrepro_surface_quicklook_font(ptr: *Surface) ?*anyopaque {
        ...
        export fn spectrepro_surface_quicklook_word(
            ptr: *Surface,
            result: *Text,
        ) bool {
            const surface = &ptr.core_surface;
        ...
        export fn spectrepro_inspector_metal_init(ptr: *Inspector, device: objc.c.id) bool {
            return ptr.initMetal(.fromId(device));
        }

        export fn spectrepro_inspector_metal_render(
            ptr: *Inspector,
        ...
        export fn spectrepro_inspector_metal_shutdown(ptr: *Inspector) void {
            if (ptr.backend) |v| {
                v.deinit();
                ptr.backend = null;
            }
