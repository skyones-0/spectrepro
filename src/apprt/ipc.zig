//! Inter-process Communication to a running SpectrePro instance from a separate
//! process.
...
pub const Target = union(Key) {
    /// Open up a new window in a custom instance of SpectrePro.
    class: [:0]const u8,

    /// Detect which instance to open a new window in.
    detect,

    // Sync with: spectrepro_ipc_target_tag_e
    pub const Key = enum(c_int) {
        class,
        detect,

        test "spectrepro.h Target.Key" {
            try lib.checkSpectreProHEnum(Key, "SPECTREPRO_IPC_TARGET_");
        }
    };

    // Sync with: spectrepro_ipc_target_u
    pub const CValue = extern union {
        class: [*:0]const u8,
        detect: void,
    };

    // Sync with: spectrepro_ipc_target_s
    pub const C = extern struct {
        key: Key,
        value: CValue,
    };

    /// Convert to spectrepro_ipc_target_s.
...
pub const Action = union(enum) {
    ...
    /// Sync with: spectrepro_ipc_action_tag_e
    pub const Key = enum(c_int) {
        new_window,
        new_tab,
        ...
        test "spectrepro.h Action.Key" {
            try lib.checkSpectreProHEnum(Key, "SPECTREPRO_IPC_ACTION_");
        }
    };

    /// Sync with: spectrepro_ipc_action_u
    ...
    /// Sync with: spectrepro_ipc_action_s
    pub const C = extern struct {
        key: Key,
        value: CValue,
    };
    ...
    /// Convert to spectrepro_ipc_action_s.
