...
def main [
  ...
  if $install_name != "@rpath/libspectrepro-vt.dylib" {
  ...
  # passed to Apple's linker should leave only libspectrepro-vt's public C API.
  let exports = (
    ^nm -gjU $binary
    | lines
  ...
  let unexpected_exports = (
    $exports
    | where { not ($in | str starts-with "_spectrepro_") }
  )
