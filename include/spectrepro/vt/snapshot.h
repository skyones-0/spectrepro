...
#ifndef SPECTREPRO_VT_SNAPSHOT_H
#define SPECTREPRO_VT_SNAPSHOT_H
 ...
 * malformed, truncated snapshot data and returns SPECTREPRO_INVALID_VALUE.
 * SPECTREPRO_IO_ERROR is reserved for a reader callback that returns false.
 *
 * ## Examples
 *
 ...
 *
 * ### Adapt a byte source to SpectreProReader
 ...
 *
 * @see <a href="https://github.com/spectrepro-org/spectrepro/blob/main/src/terminal/snapshot/main.zig">Snapshot format and Zig codec documentation</a>
 *
 * @{
 */
 ...
 * spectrepro_snapshot_decoder_set() after spectrepro_snapshot_decoder_ready() or
 * spectrepro_snapshot_decoder_decode() returns SPECTREPRO_INVALID_VALUE.
 */
typedef enum SPECTREPRO_ENUM_TYPED {
   ...
   * SPECTREPRO_SNAPSHOT_DECODER_OPT_RETAIN_CONTINUATION is true, the same value
   * also becomes the continuation tracking limit on the returned terminal.
   *
   * Input type: size_t *
   */
  SPECTREPRO_SNAPSHOT_DECODER_OPT_MAX_CONTINUATION_BYTES = 0,

  /**
   * Retain the decoded continuation on the returned terminal.
   *
   * When true, terminals returned by spectrepro_snapshot_decoder_ready() and
   * spectrepro_snapshot_decoder_decode() use
   * SPECTREPRO_SNAPSHOT_DECODER_OPT_MAX_CONTINUATION_BYTES as their continuation
   * tracking limit. The existing spectrepro_terminal_continuation_* APIs can then
   ...
   * set SPECTREPRO_TERMINAL_OPT_CONTINUATION_MAX_BYTES to zero after export and
   * before writing post-snapshot input.
   *
   * Input type: bool *
   */
  SPECTREPRO_SNAPSHOT_DECODER_OPT_RETAIN_CONTINUATION = 1,

  SPECTREPRO_SNAPSHOT_DECODER_OPT_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProSnapshotDecoderOption;

/**
 * Queryable snapshot decoder data.
 *
 * Each variant documents the output pointer type expected by
 * spectrepro_snapshot_decoder_get().
 */
typedef enum SPECTREPRO_ENUM_TYPED {
  /** Invalid data type. Never results in data extraction. */
  SPECTREPRO_SNAPSHOT_DECODER_DATA_INVALID = 0,
   ...
   * Output type: size_t *
   */
  SPECTREPRO_SNAPSHOT_DECODER_DATA_MAX_CONTINUATION_BYTES = 1,
   ...
   * Output type: size_t *
   */
  SPECTREPRO_SNAPSHOT_DECODER_DATA_SOURCE_OFFSET = 2,
   ...
   */
  SPECTREPRO_SNAPSHOT_DECODER_DATA_HISTORY_ROWS_PRIMARY = 3,

  /**
   * Advisory complete logical history extent for the alternate screen.
   *
   * The value has the same semantics and lifetime as
   * SPECTREPRO_SNAPSHOT_DECODER_DATA_HISTORY_ROWS_PRIMARY. Querying it returns
   * SPECTREPRO_NO_VALUE when the snapshot does not declare an alternate screen.
   *
   * Output type: uint64_t *
   */
  SPECTREPRO_SNAPSHOT_DECODER_DATA_HISTORY_ROWS_ALTERNATE = 4,

  /**
   * Screen associated with the most recently decoded history page.
   *
   * This value is available only after spectrepro_snapshot_decoder_next()
   * returns SPECTREPRO_SUCCESS. A later call to next replaces it or clears it
   * when FINISH is reached or an error occurs.
   *
   * Output type: SpectreProTerminalScreen *
   */
  SPECTREPRO_SNAPSHOT_DECODER_DATA_PROGRESS_SCREEN = 5,
   ...
   * Output type: size_t *
   */
  SPECTREPRO_SNAPSHOT_DECODER_DATA_PROGRESS_ROWS = 6,
   ...
   */
  SPECTREPRO_SNAPSHOT_DECODER_DATA_PROGRESS_REMAINING = 7,
   ...
   * Output type: bool *
   */
  SPECTREPRO_SNAPSHOT_DECODER_DATA_RETAIN_CONTINUATION = 8,

  SPECTREPRO_SNAPSHOT_DECODER_DATA_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProSnapshotDecoderData;
 ...
 * otherwise this returns SPECTREPRO_INVALID_VALUE.
 *
 ...
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_encode(SpectreProTerminal terminal,
 ...
 * buffer is too small, this returns SPECTREPRO_OUT_OF_SPACE and stores the
 ...
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_encode_buf(
 ...
 * when allocator is NULL. The caller must release it with spectrepro_free(),
 ...
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_encode_alloc(
    SpectreProTerminal terminal,
 ...
 * not call APIs, including spectrepro_snapshot_decoder_free(), on the decoder
 * that owns it. Returning false reports SPECTREPRO_IO_ERROR; returning true with
 ...
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_decoder_new(
    const SpectreProAllocator* allocator,
    SpectreProSnapshotDecoder* decoder,
    SpectreProReader reader);
 ...
 * query SPECTREPRO_SNAPSHOT_DECODER_DATA_SOURCE_OFFSET to locate them.
 *
 * @param allocator Allocator for decoder and decoded terminal state, or NULL
 *                  for the default allocator
 * @param decoder Pointer to receive the decoder handle (must not be NULL)
 ...
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_decoder_new_buf(
    const SpectreProAllocator* allocator,
    SpectreProSnapshotDecoder* decoder,
    const uint8_t* ptr,
    size_t len);
 ...
 * @ingroup snapshot
 */
SPECTREPRO_API void spectrepro_snapshot_decoder_free(SpectreProSnapshotDecoder decoder);
 ...
 * @return SPECTREPRO_SUCCESS on success, SPECTREPRO_INVALID_VALUE if decoding has
 *         started or an argument is invalid, or another error code on failure
 *
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_decoder_set(
    SpectreProSnapshotDecoder decoder,
    SpectreProSnapshotDecoderOption option,
    const void* value);
 ...
 * to be restored with spectrepro_snapshot_decoder_next().
 *
 * The restored parser state may be unfinished. By default, terminal
 * continuation tracking is disabled and
 * SPECTREPRO_TERMINAL_DATA_CONTINUATION_MAX_BYTES returns zero. When
 * SPECTREPRO_SNAPSHOT_DECODER_OPT_RETAIN_CONTINUATION is true, the decoder's
 ...
 * SPECTREPRO_TERMINAL_OPT_CONTINUATION_MAX_BYTES to zero after export and before
 ...
 * restores history; spectrepro_snapshot_decoder_next() uses it automatically.
 *
 ...
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_decoder_ready(
    SpectreProSnapshotDecoder decoder,
    SpectreProTerminal* terminal);
 ...
 *
 * Each SPECTREPRO_SUCCESS consumes and validates one PAGE record. Query the
 * SPECTREPRO_SNAPSHOT_DECODER_DATA_PROGRESS_* values before calling next again.
 * SPECTREPRO_NO_VALUE means FINISH was validated; repeated calls after FINISH
 * also return SPECTREPRO_NO_VALUE.
 *
 ...
 * spectrepro_snapshot_decoder_free() may subsequently be called on the decoder.
 *
 * @param decoder Decoder handle (must not be NULL)
 * @return SPECTREPRO_SUCCESS for one page, SPECTREPRO_NO_VALUE after FINISH, or an
 *         error code on failure
 *
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_decoder_next(
    SpectreProSnapshotDecoder decoder);
 ...
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_decoder_decode(
    SpectreProSnapshotDecoder decoder,
    SpectreProTerminal* terminal);
 ...
 * value that is not currently available returns SPECTREPRO_NO_VALUE.
 *
 * @param decoder Decoder handle (must not be NULL)
 ...
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_decoder_get(
    SpectreProSnapshotDecoder decoder,
    SpectreProSnapshotDecoderData data,
    void* out);
 ...
 * @return SPECTREPRO_SUCCESS if every query succeeds, or the first error
 *
 * @ingroup snapshot
 */
SPECTREPRO_API SpectreProResult spectrepro_snapshot_decoder_get_multi(
    SpectreProSnapshotDecoder decoder,
    size_t count,
    const SpectreProSnapshotDecoderData* keys,
    void** values,
    size_t* out_written);
...
#endif /* SPECTREPRO_VT_SNAPSHOT_H */
