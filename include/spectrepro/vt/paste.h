...
#ifndef SPECTREPRO_VT_PASTE_H
#define SPECTREPRO_VT_PASTE_H
 ...
 * the recommended way to paste is spectrepro_terminal_paste(). The embedder
 * hands over the MIME types the clipboard holds (just `text/plain` for
 * an ordinary paste), a SpectreProMimeReader that produces the data of
 ...
 *   SPECTREPRO_MODE_PASTE_EVENTS) are enabled, the paste was user-initiated
 *   (SPECTREPRO_PASTE_SOURCE_CLIPBOARD), and a clipboard_read callback is
 ...
 *   sequences if mode 2004 (SPECTREPRO_MODE_BRACKETED_PASTE) is enabled, or
 *   has its newlines converted to carriage returns if not.
 *
 * The data is pulled through SpectreProPaste::reader only when a
 ...
 * stream to the write_pty callback (SPECTREPRO_TERMINAL_OPT_WRITE_PTY)
 ...
 * SPECTREPRO_REJECTED and nothing written unless SpectreProPaste::allow_unsafe
 * is set. The usual flow is to call once, confirm with the user on
 * SPECTREPRO_REJECTED, and call again with `allow_unsafe` set. Each call
 ...
 * ## Building Blocks
 *
 * For embedders that encode without a terminal, spectrepro_paste_is_safe()
 * checks if paste data contains potentially dangerous sequences
 * (conservatively, regardless of terminal state) and
 * spectrepro_paste_encode() encodes paste data for writing to the pty,
...
typedef enum SPECTREPRO_ENUM_TYPED {
  ...
  SPECTREPRO_PASTE_SOURCE_CLIPBOARD = 0,
   ...
   */
  SPECTREPRO_PASTE_SOURCE_TEXT = 1,
  SPECTREPRO_PASTE_SOURCE_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProPasteSource;

/**
 * A paste of clipboard contents into the terminal.
 *
 * This is a sized struct; set `size` to `sizeof(SpectreProPaste)`. The
 * MIME type array and the strings it points to are borrowed only for
 * the duration of the spectrepro_terminal_paste() call, as is everything
 * the reader produces.
 */
typedef struct {
   ...
   *
   * Called at most once per spectrepro_terminal_paste() call: for the
   ...
   * paste with SPECTREPRO_IO_ERROR.
   */
  SpectreProMimeReader reader;

  /**
   * Write text that could inject commands. Call with false, confirm
   * with the user on SPECTREPRO_REJECTED, and call again with true.
   */
  bool allow_unsafe;
} SpectreProPaste;
 ...
 * @return SPECTREPRO_SUCCESS on success (see @p out_written);
 *         SPECTREPRO_REJECTED if the text could inject commands and
 *         SpectreProPaste::allow_unsafe is false (nothing was written);
 *         SPECTREPRO_INVALID_VALUE for a NULL terminal or paste, MIME
 *         types without a reader, or when no write_pty callback is
 *         installed; SPECTREPRO_OUT_OF_MEMORY; SPECTREPRO_IO_ERROR if the
 *         reader failed or there is no secure entropy source to mint a
 *         paste event password (wasm32-freestanding without
 *         SPECTREPRO_SYS_OPT_RANDOM_SECURE set). Errors write nothing.
 */
SPECTREPRO_API SpectreProResult spectrepro_terminal_paste(
    SpectreProTerminal terminal,
    const SpectreProPaste* paste,
    bool* out_written);
 ...
 * current terminal state. spectrepro_terminal_paste() applies the
 ...
 */
SPECTREPRO_API bool spectrepro_paste_is_safe(const char* data, size_t len);
 ...
 * SPECTREPRO_OUT_OF_SPACE and sets the required size in @p out_written.
 * The caller can then retry with a sufficiently sized buffer.
 *
 * This is the encoder spectrepro_terminal_paste() uses for a text paste;
 ...
 *         is too small
 */
SPECTREPRO_API SpectreProResult spectrepro_paste_encode(
    char* data,
    size_t data_len,
    bool bracketed,
    char* buf,
