...
#ifndef SPECTREPRO_VT_SCREEN_H
#define SPECTREPRO_VT_SCREEN_H
 ...
 * Terminal screen cell and row types.
 *
 * These types represent the contents of a terminal screen. A SpectreProCell
 * is a single grid cell and a SpectreProRow is a single row. Cell fields can
 * be accessed via spectrepro_cell_get() or decoded from the packed layout in
 * spectrepro_type_json(). Rows are opaque and accessed via spectrepro_row_get().
 *
 * @{
 */
 ...
 * spectrepro_cell_get(). Boundary-sensitive callers can decode the packed value
 * using the SpectreProCell descriptor returned by spectrepro_type_json(). The
 * manifest is authoritative for the linked build; hardcoding bit positions
 * is unsupported.
 *
 * @ingroup screen
 */
typedef uint64_t SpectreProCell;

/**
 * Opaque row value.
 *
 * Represents a single terminal row. The internal layout is opaque and
 * must be queried via spectrepro_row_get(). Obtain row values from
 * terminal query APIs.
 *
 * @ingroup screen
 */
typedef uint64_t SpectreProRow;
 ...
 * can be queried via spectrepro_cell_get() or decoded using the SpectreProCell
 * packed descriptor returned by spectrepro_type_json().
 *
 * @ingroup screen
 */
typedef struct {
  /** Pointer to len contiguous cell values. */
  const SpectreProCell* ptr;
...
typedef enum SPECTREPRO_ENUM_TYPED {
  ...
  SPECTREPRO_CELL_CONTENT_CODEPOINT = 0,

  /** A codepoint that is part of a multi-codepoint grapheme cluster. */
  SPECTREPRO_CELL_CONTENT_CODEPOINT_GRAPHEME = 1,

  /** No text; background color from palette. */
  SPECTREPRO_CELL_CONTENT_BG_COLOR_PALETTE = 2,

  /** No text; background color as RGB. */
  SPECTREPRO_CELL_CONTENT_BG_COLOR_RGB = 3,
  SPECTREPRO_CELL_CONTENT_TAG_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProCellContentTag;
...
typedef enum SPECTREPRO_ENUM_TYPED {
  ...
  SPECTREPRO_CELL_WIDE_NARROW = 0,

  /** Wide character, cell width 2. */
  SPECTREPRO_CELL_WIDE_WIDE = 1,

  /** Spacer after wide character. Do not render. */
  SPECTREPRO_CELL_WIDE_SPACER_TAIL = 2,

  /** Spacer at end of soft-wrapped line for a wide character. */
  SPECTREPRO_CELL_WIDE_SPACER_HEAD = 3,
  SPECTREPRO_CELL_WIDE_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProCellWide;
...
typedef enum SPECTREPRO_ENUM_TYPED {
  ...
  SPECTREPRO_CELL_SEMANTIC_OUTPUT = 0,

  /** Content that is part of user input. */
  SPECTREPRO_CELL_SEMANTIC_INPUT = 1,

  /** Content that is part of a shell prompt. */
  SPECTREPRO_CELL_SEMANTIC_PROMPT = 2,
  SPECTREPRO_CELL_SEMANTIC_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProCellSemanticContent;

/**
 * Cell data types.
 *
 * These values specify what type of data to extract from a cell
 * using `spectrepro_cell_get`.
 *
 * @ingroup screen
 */
typedef enum SPECTREPRO_ENUM_TYPED {
  /** Invalid data type. Never results in any data extraction. */
  SPECTREPRO_CELL_DATA_INVALID = 0,
   ...
   */
  SPECTREPRO_CELL_DATA_CODEPOINT = 1,

  /**
   * The content tag describing what kind of content is in the cell.
   *
   * Output type: SpectreProCellContentTag *
   */
  SPECTREPRO_CELL_DATA_CONTENT_TAG = 2,

  /**
   * The wide property of the cell.
   *
   * Output type: SpectreProCellWide *
   */
  SPECTREPRO_CELL_DATA_WIDE = 3,
   ...
   * Output type: bool *
   */
  SPECTREPRO_CELL_DATA_HAS_TEXT = 4,
   ...
   * Output type: bool *
   */
  SPECTREPRO_CELL_DATA_HAS_STYLING = 5,
   ...
   */
  SPECTREPRO_CELL_DATA_STYLE_ID = 6,
   ...
   * Output type: bool *
   */
  SPECTREPRO_CELL_DATA_HAS_HYPERLINK = 7,
   ...
   * Output type: bool *
   */
  SPECTREPRO_CELL_DATA_PROTECTED = 8,

  /**
   * The semantic content type of the cell (from OSC 133).
   *
   * Output type: SpectreProCellSemanticContent *
   */
  SPECTREPRO_CELL_DATA_SEMANTIC_CONTENT = 9,

  /**
   * The palette index for the cell's background color.
   * Only valid when content_tag is SPECTREPRO_CELL_CONTENT_BG_COLOR_PALETTE.
   *
   * Output type: SpectreProColorPaletteIndex *
   */
  SPECTREPRO_CELL_DATA_COLOR_PALETTE = 10,

  /**
   * The RGB value for the cell's background color.
   * Only valid when content_tag is SPECTREPRO_CELL_CONTENT_BG_COLOR_RGB.
   *
   * Output type: SpectreProColorRgb *
   */
  SPECTREPRO_CELL_DATA_COLOR_RGB = 11,
  SPECTREPRO_CELL_DATA_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProCellData;
...
typedef enum SPECTREPRO_ENUM_TYPED {
  ...
  SPECTREPRO_ROW_SEMANTIC_NONE = 0,

  /** Prompt cells exist and this is a primary prompt line. */
  SPECTREPRO_ROW_SEMANTIC_PROMPT = 1,

  /** Prompt cells exist and this is a continuation line. */
  SPECTREPRO_ROW_SEMANTIC_PROMPT_CONTINUATION = 2,
  SPECTREPRO_ROW_SEMANTIC_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProRowSemanticPrompt;

/**
 * Row data types.
 *
 * These values specify what type of data to extract from a row
 * using `spectrepro_row_get`.
 *
 * @ingroup screen
 */
typedef enum SPECTREPRO_ENUM_TYPED {
  /** Invalid data type. Never results in any data extraction. */
  SPECTREPRO_ROW_DATA_INVALID = 0,
   ...
   * Output type: bool *
   */
  SPECTREPRO_ROW_DATA_WRAP_CONTINUATION = 2,
   ...
   * Output type: bool *
   */
  SPECTREPRO_ROW_DATA_GRAPHEME = 3,
   ...
   * Output type: bool *
   */
  SPECTREPRO_ROW_DATA_HYPERLINK = 5,

  /**
   * The semantic prompt state of this row.
   *
   * Output type: SpectreProRowSemanticPrompt *
   */
  SPECTREPRO_ROW_DATA_SEMANTIC_PROMPT = 6,
   ...
   * Output type: bool *
   */
  SPECTREPRO_ROW_DATA_KITTY_VIRTUAL_PLACEHOLDER = 7,
   ...
   * Output type: bool *
   */
  SPECTREPRO_ROW_DATA_DIRTY = 8,
  SPECTREPRO_ROW_DATA_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProRowData;
 ...
 * in the `SpectreProCellData` enum.
 *
 ...
 * @ingroup screen
 */
SPECTREPRO_API SpectreProResult spectrepro_cell_get(SpectreProCell cell,
                                SpectreProCellData data,
                                void *out);
 ...
 * @ingroup screen
 */
SPECTREPRO_API SpectreProResult spectrepro_cell_get_multi(SpectreProCell cell,
                                     size_t count,
                                     const SpectreProCellData* keys,
                                     void** values,
                                     size_t* out_written);
 ...
 * @ingroup screen
 */
SPECTREPRO_API SpectreProResult spectrepro_row_get(SpectreProRow row,
                               SpectreProRowData data,
                               void *out);
 ...
 * @ingroup screen
 */
SPECTREPRO_API SpectreProResult spectrepro_row_get_multi(SpectreProRow row,
                                    size_t count,
                                    const SpectreProRowData* keys,
                                    void** values,
                                    size_t* out_written);
