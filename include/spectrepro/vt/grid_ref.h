...
#ifndef SPECTREPRO_VT_GRID_REF_H
#define SPECTREPRO_VT_GRID_REF_H
 ...
 * terminal. Obtain a grid reference from `spectrepro_terminal_grid_ref`
 * for untracked or `spectrepro_terminal_grid_ref_track` for tracked. Untracked
 ...
 * spectrepro_tracked_grid_ref_has_value() returns false and
 * spectrepro_tracked_grid_ref_snapshot() / spectrepro_tracked_grid_ref_point() return
 * SPECTREPRO_NO_VALUE. The handle remains valid, and callers may move it to a new
 * point with spectrepro_tracked_grid_ref_set().
 *
 * To read cell data from a tracked reference, first snapshot it with
 * spectrepro_tracked_grid_ref_snapshot(). The returned `SpectreProGridRef` is again
 ...
 * alternate screens. Calling spectrepro_tracked_grid_ref_set() resolves the new
 ...
 * spectrepro_tracked_grid_ref_free(). If the terminal that created a tracked
 ...
 * @return SPECTREPRO_SUCCESS on success, SPECTREPRO_INVALID_VALUE if the ref's
 *         node is NULL
 *
 * @ingroup grid_ref
 */
SPECTREPRO_API SpectreProResult spectrepro_grid_ref_cell(const SpectreProGridRef *ref,
                                    SpectreProCell *out_cell);
 ...
 * @ingroup grid_ref
 */
SPECTREPRO_API SpectreProResult spectrepro_grid_ref_row(const SpectreProGridRef *ref,
                                   SpectreProRow *out_row);
 ...
 * out_len is set to 0 and SPECTREPRO_SUCCESS is returned.
 *
 * If the buffer is too small (or NULL), the function returns
 * SPECTREPRO_OUT_OF_SPACE and writes the required number of codepoints to
 ...
 *             SPECTREPRO_OUT_OF_SPACE, the required buffer size in codepoints.
 * @return SPECTREPRO_SUCCESS on success, SPECTREPRO_INVALID_VALUE if the ref's
 *         node is NULL, SPECTREPRO_OUT_OF_SPACE if the buffer is too small
 *
 * @ingroup grid_ref
 */
SPECTREPRO_API SpectreProResult spectrepro_grid_ref_graphemes(const SpectreProGridRef *ref,
                                         uint32_t *buf,
                                         size_t buf_len,
                                         size_t *out_len);
 ...
 * hyperlink, out_len is set to 0 and SPECTREPRO_SUCCESS is returned.
 *
 * If the buffer is too small (or NULL), the function returns
 * SPECTREPRO_OUT_OF_SPACE and writes the required number of bytes to
 * out_len. The caller can then retry with a sufficiently sized buffer.
 *
 * @param ref Pointer to the grid reference
 ...
 *             SPECTREPRO_OUT_OF_SPACE, the required buffer size in bytes.
 * @return SPECTREPRO_SUCCESS on success, SPECTREPRO_INVALID_VALUE if the ref's
 *         node is NULL, SPECTREPRO_OUT_OF_SPACE if the buffer is too small
 *
 * @ingroup grid_ref
 */
SPECTREPRO_API SpectreProResult spectrepro_grid_ref_hyperlink_uri(
 ...
 * @ingroup grid_ref
 */
SPECTREPRO_API SpectreProResult spectrepro_grid_ref_style(const SpectreProGridRef *ref,
                                     SpectreProStyle *out_style);
...
#endif /* SPECTREPRO_VT_GRID_REF_H */
