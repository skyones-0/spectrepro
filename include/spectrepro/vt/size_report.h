...
#ifndef SPECTREPRO_VT_SIZE_REPORT_H
#define SPECTREPRO_VT_SIZE_REPORT_H
 ...
 * ## Basic Usage
 *
 * Use spectrepro_size_report_encode() to encode a size report into a
 * caller-provided buffer. If the buffer is too small, the function
 * returns SPECTREPRO_OUT_OF_SPACE and sets the required size in the
 * output parameter.
 *
 * ## Example
 *
...
typedef enum SPECTREPRO_ENUM_TYPED {
    ...
    SPECTREPRO_SIZE_REPORT_MODE_2048 = 0,
    /** XTWINOPS text area size in pixels: ESC [ 4 ; height ; width t */
    SPECTREPRO_SIZE_REPORT_CSI_14_T = 1,
    /** XTWINOPS cell size in pixels: ESC [ 6 ; height ; width t */
    SPECTREPRO_SIZE_REPORT_CSI_16_T = 2,
    /** XTWINOPS text area size in characters: ESC [ 8 ; rows ; cols t */
    SPECTREPRO_SIZE_REPORT_CSI_18_T = 3,
    SPECTREPRO_SIZE_REPORT_STYLE_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProSizeReportStyle;
 ...
 *         is too small
 */
SPECTREPRO_API SpectreProResult spectrepro_size_report_encode(
    SpectreProSizeReportStyle style,
...
#endif /* SPECTREPRO_VT_SIZE_REPORT_H */
