...
#ifndef SPECTREPRO_VT_COLOR_SCHEME_H
#define SPECTREPRO_VT_COLOR_SCHEME_H
 ...
 * ## Basic Usage
 *
 * Use spectrepro_color_scheme_report_encode() to encode a color scheme report
 * into a caller-provided buffer. If the buffer is too small, the function
 * returns SPECTREPRO_OUT_OF_SPACE and sets the required size in the output
 * parameter.
 *
 * ## Example
 *
 ...
 * query response.
 *
 * Hosts should gate unsolicited sends on SPECTREPRO_MODE_COLOR_SCHEME_REPORT
 ...
 *         is too small
 */
SPECTREPRO_API SpectreProResult spectrepro_color_scheme_report_encode(
    SpectreProColorScheme scheme,
    char* buf,
    size_t buf_len,
    size_t* out_written);
...
#endif /* SPECTREPRO_VT_COLOR_SCHEME_H */
