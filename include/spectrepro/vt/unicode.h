...
#ifndef SPECTREPRO_VT_UNICODE_H
#define SPECTREPRO_VT_UNICODE_H
 ...
 * ## Basic Usage
 *
 * Use spectrepro_unicode_codepoint_width() to determine how many terminal
 ...
 * spectrepro_unicode_grapheme_width() to segment and measure full grapheme
 ...
 * spectrepro_unicode_grapheme_width(). Summing per-codepoint widths is only
 ...
 */
SPECTREPRO_API uint8_t spectrepro_unicode_codepoint_width(uint32_t cp);
 ...
 * spectrepro_unicode_codepoint_width(), this accounts for cluster-level
 ...
 *   uint8_t width;
 *   i += spectrepro_unicode_grapheme_width(cps + i, len - i, &width);
 *   total += width;
 * }
 * @endcode
 *
 ...
 *   spectrepro_unicode_codepoint_width().
 ...
 * which is SpectrePro's recommended configuration. When mode 2027 is
 * disabled, clusters never combine and variation selectors never change
 * width; predict layout in that case by summing
 * spectrepro_unicode_codepoint_width() over each codepoint instead.
 *
 * Edge cases:
 ...
 */
SPECTREPRO_API size_t spectrepro_unicode_grapheme_width(const uint32_t *cps,
                                                  size_t len,
                                                  uint8_t *width);
