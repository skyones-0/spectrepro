 ...
 * @file allocator.h
 *
 * Memory management interface for libspectrepro-vt.
 */

#ifndef SPECTREPRO_VT_ALLOCATOR_H
#define SPECTREPRO_VT_ALLOCATOR_H
 ...
 *
 * libspectrepro-vt does require memory allocation for various operations,
 ...
 *
 * libspectrepro-vt uses explicit memory allocation via an allocator
 * interface provided by SpectreProAllocator. The interface is based on the
 ...
 * function that accepts one,** and libspectrepro will use a default allocator.
 ...
 * 1. Implement the SpectreProAllocatorVtable function pointers
 * 2. Create a SpectreProAllocator struct with your vtable and context
 * 3. Pass the allocator to functions that accept one
 *
 * ## Alloc/Free Helpers
 *
 * spectrepro_alloc() and spectrepro_free() provide a simple malloc/free-style
 * interface for allocating and freeing byte buffers through the library's
 * allocator. These are useful when:
 *
 * - You need to allocate a buffer to pass into a libspectrepro-vt function
 *   (e.g. preparing input data for spectrepro_terminal_vt_write()).
 * - You need to free a buffer returned by a libspectrepro-vt function
 *   (e.g. the output of spectrepro_formatter_format_alloc()).
 ...
 * Usage example:
 * @code
 * SpectreProAllocator allocator = {
 *     .vtable = &my_allocator_vtable,
 *     .ctx = my_allocator_state
 * };
 * @endcode
 */
typedef struct SpectreProAllocator {
    /**
     ...
     */
    const SpectreProAllocatorVtable *vtable;
} SpectreProAllocator;
 ...
 * The returned buffer must be freed with spectrepro_free() using the same
 * allocator.
 *
 ...
 * @ingroup allocator
 */
SPECTREPRO_API uint8_t* spectrepro_alloc(const SpectreProAllocator* allocator, size_t len);

/**
 * Free memory that was allocated by a libspectrepro-vt function.
 *
 * Use this to free buffers returned by functions such as
 * spectrepro_formatter_format_alloc(). Pass the same allocator that was
 ...
 * @ingroup allocator
 */
SPECTREPRO_API void spectrepro_free(const SpectreProAllocator* allocator, uint8_t* ptr, size_t len);

/** @} */

#endif /* SPECTREPRO_VT_ALLOCATOR_H */
