...
#ifndef SPECTREPRO_VT_KITTY_GRAPHICS_H
#define SPECTREPRO_VT_KITTY_GRAPHICS_H
 ...
 *
 * The central object is @ref SpectreProKittyGraphics, an opaque handle to
 ...
 *
 * A @ref SpectreProKittyGraphics handle is obtained from a terminal via
 * spectrepro_terminal_get() with @ref SPECTREPRO_TERMINAL_DATA_KITTY_GRAPHICS.
 * The handle is borrowed from the terminal and remains valid until the
 * next mutating terminal call (e.g. spectrepro_terminal_vt_write() or
 ...
 * @ref SPECTREPRO_TERMINAL_OPT_KITTY_IMAGE_STORAGE_LIMIT, and a PNG
 * decoder callback must be installed via spectrepro_sys_set() with
 * @ref SPECTREPRO_SYS_OPT_DECODE_PNG.
 *
 * @snippet c-vt-kitty-graphics/src/main.c kitty-graphics-decode-png
 *
 * ## Iterating Placements
 *
 * Placements are inspected through a @ref SpectreProKittyGraphicsPlacementIterator.
 * The typical workflow is:
 *
 *   1. Create an iterator with spectrepro_kitty_graphics_placement_iterator_new().
 *   2. Populate it from the storage with spectrepro_kitty_graphics_get() using
 *      @ref SPECTREPRO_KITTY_GRAPHICS_DATA_PLACEMENT_ITERATOR.
 *   3. Optionally filter by z-layer with
 *      spectrepro_kitty_graphics_placement_iterator_set().
 *   4. Advance with spectrepro_kitty_graphics_placement_next() and read
 *      per-placement data with spectrepro_kitty_graphics_placement_get().
 *   5. For each placement, look up its image with
 *      spectrepro_kitty_graphics_image() to access pixel data and dimensions.
 *   6. Free the iterator with spectrepro_kitty_graphics_placement_iterator_free().
 *
 * ## Looking Up Images
 *
 * Given an image ID (obtained from a placement via
 * @ref SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_IMAGE_ID), call
 * spectrepro_kitty_graphics_image() to get a @ref SpectreProKittyGraphicsImage
 * handle. From this handle, spectrepro_kitty_graphics_image_get() provides
 ...
 *
 * - spectrepro_kitty_graphics_placement_pixel_size() — rendered pixel
 *   dimensions accounting for source rect and aspect ratio.
 * - spectrepro_kitty_graphics_placement_grid_size() — number of grid
 *   columns and rows the placement occupies.
 * - spectrepro_kitty_graphics_placement_viewport_pos() — viewport-relative
 *   grid position (may be negative for partially scrolled placements).
 * - spectrepro_kitty_graphics_placement_source_rect() — resolved source
 *   rectangle in pixels, clamped to image bounds.
 * - spectrepro_kitty_graphics_placement_rect() — bounding rectangle as a
 *   @ref SpectreProSelection.
 *
 * ## Change Detection
 *
 ...
 *
 * - @ref SPECTREPRO_KITTY_GRAPHICS_DATA_GENERATION is a storage-wide stamp
 ...
 *   placements), so spectrepro_kitty_graphics_placement_render_info()
 *   should still be recomputed on dirty frames.
 * - @ref SPECTREPRO_KITTY_IMAGE_DATA_GENERATION is a per-image stamp
 ...
 *
 * All handles borrowed from the terminal (SpectreProKittyGraphics,
 * SpectreProKittyGraphicsImage) are invalidated by any mutating terminal
...
/**
 * Queryable data kinds for spectrepro_kitty_graphics_get().
 *
 * @ingroup kitty_graphics
 */
typedef enum SPECTREPRO_ENUM_TYPED {
  /** Invalid / sentinel value. */
  SPECTREPRO_KITTY_GRAPHICS_DATA_INVALID = 0,
   ...
   *
   * Output type: SpectreProKittyGraphicsPlacementIterator *
   */
  SPECTREPRO_KITTY_GRAPHICS_DATA_PLACEMENT_ITERATOR = 1,
   ...
   * geometry such as spectrepro_kitty_graphics_placement_render_info()
   ...
   */
  SPECTREPRO_KITTY_GRAPHICS_DATA_GENERATION = 2,
  SPECTREPRO_KITTY_GRAPHICS_DATA_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProKittyGraphicsData;

/**
 * Queryable data kinds for spectrepro_kitty_graphics_placement_get().
 *
 * @ingroup kitty_graphics
 */
typedef enum SPECTREPRO_ENUM_TYPED {
  /** Invalid / sentinel value. */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_INVALID = 0,
   ...
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_IMAGE_ID = 1,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_PLACEMENT_ID = 2,
   ...
   * Output type: bool *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_IS_VIRTUAL = 3,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_X_OFFSET = 4,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_Y_OFFSET = 5,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_SOURCE_X = 6,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_SOURCE_Y = 7,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_SOURCE_WIDTH = 8,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_SOURCE_HEIGHT = 9,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_COLUMNS = 10,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_ROWS = 11,
   ...
   * Output type: int32_t *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_Z = 12,

  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_DATA_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProKittyGraphicsPlacementData;
 ...
 */
typedef enum SPECTREPRO_ENUM_TYPED {
  SPECTREPRO_KITTY_PLACEMENT_LAYER_ALL = 0,
  SPECTREPRO_KITTY_PLACEMENT_LAYER_BELOW_BG = 1,
  SPECTREPRO_KITTY_PLACEMENT_LAYER_BELOW_TEXT = 2,
  SPECTREPRO_KITTY_PLACEMENT_LAYER_ABOVE_TEXT = 3,
  SPECTREPRO_KITTY_PLACEMENT_LAYER_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProKittyPlacementLayer;

/**
 * Settable options for spectrepro_kitty_graphics_placement_iterator_set().
 *
 * @ingroup kitty_graphics
 */
typedef enum SPECTREPRO_ENUM_TYPED {
   ...
   *
   * Input type: SpectreProKittyPlacementLayer *
   */
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_ITERATOR_OPTION_LAYER = 0,
  SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_ITERATOR_OPTION_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProKittyGraphicsPlacementIteratorOption;

/**
 * Pixel format of a Kitty graphics image.
 *
 * Note that stored images are always fully decoded:
 * SPECTREPRO_KITTY_IMAGE_FORMAT_PNG is never returned by
 * spectrepro_kitty_graphics_image_get() because PNG payloads are decoded
 * to SPECTREPRO_KITTY_IMAGE_FORMAT_RGBA before storage. The PNG value
 * exists only for protocol-level completeness.
 *
 * @ingroup kitty_graphics
 */
typedef enum SPECTREPRO_ENUM_TYPED {
  SPECTREPRO_KITTY_IMAGE_FORMAT_RGB = 0,
  SPECTREPRO_KITTY_IMAGE_FORMAT_RGBA = 1,
  SPECTREPRO_KITTY_IMAGE_FORMAT_PNG = 2,
  SPECTREPRO_KITTY_IMAGE_FORMAT_GRAY_ALPHA = 3,
  SPECTREPRO_KITTY_IMAGE_FORMAT_GRAY = 4,
  SPECTREPRO_KITTY_IMAGE_FORMAT_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProKittyImageFormat;

/**
 * Compression of a Kitty graphics image.
 *
 * Note that stored images are always decompressed:
 * SPECTREPRO_KITTY_IMAGE_COMPRESSION_ZLIB_DEFLATE payloads are inflated
 * before storage, so spectrepro_kitty_graphics_image_get() always reports
 * SPECTREPRO_KITTY_IMAGE_COMPRESSION_NONE. Consumers never need to
 * inflate image data themselves.
 *
 * @ingroup kitty_graphics
 */
typedef enum SPECTREPRO_ENUM_TYPED {
  SPECTREPRO_KITTY_IMAGE_COMPRESSION_NONE = 0,
  SPECTREPRO_KITTY_IMAGE_COMPRESSION_ZLIB_DEFLATE = 1,
  SPECTREPRO_KITTY_IMAGE_COMPRESSION_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProKittyImageCompression;

/**
 * Queryable data kinds for spectrepro_kitty_graphics_image_get().
 *
 * @ingroup kitty_graphics
 */
typedef enum SPECTREPRO_ENUM_TYPED {
  /** Invalid / sentinel value. */
  SPECTREPRO_KITTY_IMAGE_DATA_INVALID = 0,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_IMAGE_DATA_ID = 1,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_IMAGE_DATA_NUMBER = 2,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_IMAGE_DATA_WIDTH = 3,
   ...
   * Output type: uint32_t *
   */
  SPECTREPRO_KITTY_IMAGE_DATA_HEIGHT = 4,

  /**
   * Pixel format of the image. Never SPECTREPRO_KITTY_IMAGE_FORMAT_PNG;
   * PNG payloads are decoded to RGBA before storage.
   *
   * Output type: SpectreProKittyImageFormat *
   */
  SPECTREPRO_KITTY_IMAGE_DATA_FORMAT = 5,

  /**
   * Compression of the image. Always
   * SPECTREPRO_KITTY_IMAGE_COMPRESSION_NONE; compressed payloads are
   * inflated before storage.
   *
   * Output type: SpectreProKittyImageCompression *
   */
  SPECTREPRO_KITTY_IMAGE_DATA_COMPRESSION = 6,

  /**
   * Borrowed pointer to the raw pixel data. Valid as long as the
   * underlying terminal is not mutated. Returns SPECTREPRO_NO_VALUE when
   ...
   * format reported by SPECTREPRO_KITTY_IMAGE_DATA_FORMAT: zlib payloads
   ...
   * image's SPECTREPRO_KITTY_IMAGE_DATA_GENERATION changes whenever the
   * current frame changes, so generation-keyed caches remain
   * coherent.
   *
   * Output type: const uint8_t **
   */
  SPECTREPRO_KITTY_IMAGE_DATA_DATA_PTR = 7,
   ...
   * Output type: size_t *
   */
  SPECTREPRO_KITTY_IMAGE_DATA_DATA_LEN = 8,
   ...
   * SPECTREPRO_KITTY_GRAPHICS_DATA_GENERATION. Never zero for a stored
   ...
   * detect that completion through SPECTREPRO_KITTY_GRAPHICS_DATA_GENERATION
   * and retry DATA_PTR.
   *
   * Output type: uint64_t *
   */
  SPECTREPRO_KITTY_IMAGE_DATA_GENERATION = 9,

  SPECTREPRO_KITTY_IMAGE_DATA_MAX_VALUE = SPECTREPRO_ENUM_MAX_VALUE,
} SpectreProKittyGraphicsImageData;

/**
 * Combined rendering geometry for a placement in a single sized struct.
 *
 * Combines the results of spectrepro_kitty_graphics_placement_pixel_size(),
 * spectrepro_kitty_graphics_placement_grid_size(),
 * spectrepro_kitty_graphics_placement_viewport_pos(), and
 * spectrepro_kitty_graphics_placement_source_rect() into one call. This is
 ...
 * SPECTREPRO_INIT_SIZED(SpectreProKittyGraphicsPlacementRenderInfo) before calling
 * spectrepro_kitty_graphics_placement_render_info().
 *
 * @ingroup kitty_graphics
 */
typedef struct {
  /** Size of this struct in bytes. Must be set to sizeof(SpectreProKittyGraphicsPlacementRenderInfo). */
  size_t size;
...
} SpectreProKittyGraphicsPlacementRenderInfo;
 ...
 * @return SPECTREPRO_SUCCESS on success
 *
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_get(
    SpectreProKittyGraphics graphics,
    SpectreProKittyGraphicsData data,
    void* out);
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProKittyGraphicsImage spectrepro_kitty_graphics_image(
    SpectreProKittyGraphics graphics,
    uint32_t image_id);
 ...
 * data kind.
 *
 * @param image The image handle (NULL returns SPECTREPRO_INVALID_VALUE)
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_image_get(
    SpectreProKittyGraphicsImage image,
    SpectreProKittyGraphicsImageData data,
    void* out);

/**
 * Get multiple data fields from a Kitty graphics image in a single call.
 *
 * This is an optimization over calling spectrepro_kitty_graphics_image_get()
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_image_get_multi(
    SpectreProKittyGraphicsImage image,
    size_t count,
    const SpectreProKittyGraphicsImageData* keys,
    void** values,
    size_t* out_written);
 ...
 * via spectrepro_kitty_graphics_get() with
 * SPECTREPRO_KITTY_GRAPHICS_DATA_PLACEMENT_ITERATOR.
 *
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_placement_iterator_new(
    const SpectreProAllocator* allocator,
    SpectreProKittyGraphicsPlacementIterator* out_iterator);
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API void spectrepro_kitty_graphics_placement_iterator_free(
    SpectreProKittyGraphicsPlacementIterator iterator);

/**
 * Set an option on a placement iterator.
 *
 * Use SPECTREPRO_KITTY_GRAPHICS_PLACEMENT_ITERATOR_OPTION_LAYER with a
 * SpectreProKittyPlacementLayer value to filter placements by z-layer.
 * The filter is applied during iteration: spectrepro_kitty_graphics_placement_next()
 * will skip placements that do not match the configured layer.
 *
 * The default layer is SPECTREPRO_KITTY_PLACEMENT_LAYER_ALL (no filtering).
 *
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_placement_iterator_set(
    SpectreProKittyGraphicsPlacementIterator iterator,
    SpectreProKittyGraphicsPlacementIteratorOption option,
    const void* value);
 ...
 * spectrepro_kitty_graphics_placement_iterator_set(), only placements
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API bool spectrepro_kitty_graphics_placement_next(
    SpectreProKittyGraphicsPlacementIterator iterator);

/**
 * Get data from the current placement in a placement iterator.
 *
 * Call spectrepro_kitty_graphics_placement_next() at least once before
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_placement_get(
    SpectreProKittyGraphicsPlacementIterator iterator,
    SpectreProKittyGraphicsPlacementData data,
    void* out);

/**
 * Get multiple data fields from the current placement in a single call.
 *
 * This is an optimization over calling spectrepro_kitty_graphics_placement_get()
 * repeatedly, particularly useful in environments with high per-call
 * overhead such as FFI or Cgo.
 *
 * Each element in the keys array specifies a data kind, and the
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_placement_get_multi(
    SpectreProKittyGraphicsPlacementIterator iterator,
    size_t count,
    const SpectreProKittyGraphicsPlacementData* keys,
    void** values,
    size_t* out_written);
 ...
 * placements (unicode placeholders) return SPECTREPRO_NO_VALUE.
 *
 ...
 * @return SPECTREPRO_SUCCESS on success, SPECTREPRO_INVALID_VALUE if any handle
 *         is NULL or the iterator is not positioned, SPECTREPRO_NO_VALUE for
 *         virtual placements or when Kitty graphics are disabled
 *
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_placement_rect(
    SpectreProKittyGraphicsPlacementIterator iterator,
    SpectreProKittyGraphicsImage image,
 ...
 * @return SPECTREPRO_SUCCESS on success, SPECTREPRO_INVALID_VALUE if any handle
 *         is NULL or the iterator is not positioned, SPECTREPRO_NO_VALUE when
 *         Kitty graphics are disabled
 *
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_placement_pixel_size(
    SpectreProKittyGraphicsPlacementIterator iterator,
    SpectreProKittyGraphicsImage image,
    SpectreProTerminal terminal,
    uint32_t* out_width,
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_placement_grid_size(
    SpectreProKittyGraphicsPlacementIterator iterator,
    SpectreProKittyGraphicsImage image,
    SpectreProTerminal terminal,
    uint32_t* out_cols,
 ...
 *
 * Returns SPECTREPRO_SUCCESS for any placement that is at least
 * partially visible in the viewport. Returns SPECTREPRO_NO_VALUE when
 ...
 * @return SPECTREPRO_SUCCESS on success, SPECTREPRO_NO_VALUE if fully
 *         off-screen or virtual, SPECTREPRO_INVALID_VALUE if any handle
 *         is NULL or the iterator is not positioned
 *
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_placement_viewport_pos(
    SpectreProKittyGraphicsPlacementIterator iterator,
    SpectreProKittyGraphicsImage image,
    SpectreProTerminal terminal,
    int32_t* out_col,
 ...
 * @return SPECTREPRO_SUCCESS on success, SPECTREPRO_INVALID_VALUE if any
 *         handle is NULL or the iterator is not positioned
 *
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_placement_source_rect(
    SpectreProKittyGraphicsPlacementIterator iterator,
    SpectreProKittyGraphicsImage image,
    uint32_t* out_x,
    uint32_t* out_y,
 ...
 * SPECTREPRO_INIT_SIZED(SpectreProKittyGraphicsPlacementRenderInfo).
 *
 ...
 * @ingroup kitty_graphics
 */
SPECTREPRO_API SpectreProResult spectrepro_kitty_graphics_placement_render_info(
    SpectreProKittyGraphicsPlacementIterator iterator,
    SpectreProKittyGraphicsImage image,
    SpectreProTerminal terminal,
    SpectreProKittyGraphicsPlacementRenderInfo* out_info);
...
#endif /* SPECTREPRO_VT_KITTY_GRAPHICS_H */
